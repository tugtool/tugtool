 ...
 * The single source of truth for canonical L values is roadmap/tug-hvv-canonical.json,
...
/** Build the preset formula for a given hue, vib, and val. */
function presetFormula(H: string, vib: number, val: number): string {
  const lPart = lightnessPart(H, val);
  const cPart = `calc(${vib} / 100 * var(--tug-${H}-peak-c))`;
  const hPart = `var(--tug-${H}-h)`;
  return `oklch(${lPart} ${cPart} ${hPart})`;
}

/**
 * Build the CSS lightness expression for a given val.
 *
 * val=50 (and intense/muted/canonical) resolves to var(--tug-H-canonical-l) directly
 ...
 *
 * For other val values we emit the full piecewise calc()+clamp() formula.
 */
...
lines.push(`/**`);
lines.push(` * tug-palette.css — HVV Palette Engine (Pure CSS)`);
lines.push(` *`);
lines.push(` * HVV color model: Hue, Vibrancy, Value — a perceptual coordinate system`);
lines.push(` * mapped to OKLCH. Each hue has three constants (h, canonical-l, peak-c).`);
lines.push(` * Vibrancy (0-100) controls chroma as a fraction of peak-c. Value (0-100)`);
lines.push(` * controls lightness via a piecewise linear formula between three anchors:`);
lines.push(` *   val=0  → L_DARK (${fmt(L_DARK)})`);
lines.push(` *   val=50 → canonical-l (per-hue)`);
lines.push(` *   val=100 → L_LIGHT (${fmt(L_LIGHT)})`);
lines.push(` *`);
lines.push(` * Piecewise formula (calc + clamp):`);
lines.push(` *   L = L_DARK`);
lines.push(` *       + clamp(0, val, 50) * (canonical-l - L_DARK) / 50`);
lines.push(` *       + (clamp(50, val, 100) - 50) * (L_LIGHT - canonical-l) / 50`);
lines.push(` *   C = vib / 100 * peak-c`);
lines.push(` *`);
lines.push(` * Five convenience presets per hue (120 = 24 hues x 5 presets):`);
lines.push(` *   canonical  vib=50, val=50   The crayon color — reference point`);
lines.push(` *   light      vib=20, val=85   Background-safe, airy`);
lines.push(` *   dark       vib=50, val=20   Contrast text, dark surfaces`);
lines.push(` *   intense    vib=90, val=50   Pops, draws attention`);
lines.push(` *   muted      vib=20, val=50   Subdued, secondary`);
lines.push(` *`);
...
lines.push(` *   --tug-{hue}-canonical-l  Canonical lightness at vib=50, val=50`);
lines.push(` *   --tug-{hue}-peak-c       Peak chroma = MAX_CHROMA_FOR_HUE * PEAK_C_SCALE (${PEAK_C_SCALE})`);
lines.push(` *`);
lines.push(` * Global constants:`);
lines.push(` *   --tug-l-dark   Lightness at val=0 (${fmt(L_DARK)})`);
lines.push(` *   --tug-l-light  Lightness at val=100 (${fmt(L_LIGHT)})`);
lines.push(` */`);
lines.push(``);
...
lines.push(`   *`);
lines.push(`   * Each preset uses literal vib/val numbers in the calc()+clamp() formula.`);
lines.push(`   * Five presets: canonical (vib=50,val=50), light (vib=20,val=85),`);
lines.push(`   *               dark (vib=50,val=20), intense (vib=90,val=50),`);
lines.push(`   *               muted (vib=20,val=50)`);
lines.push(`   *`);
lines.push(`   * Formula for L (piecewise via clamp):`);
lines.push(`   *   L = var(--tug-l-dark)`);
lines.push(`   *       + clamp(0, val, 50) * (var(--tug-H-canonical-l) - var(--tug-l-dark)) / 50`);
lines.push(`   *       + (clamp(50, val, 100) - 50) * (var(--tug-l-light) - var(--tug-H-canonical-l)) / 50`);
lines.push(`   * Formula for C (linear):`);
lines.push(`   *   C = vib / 100 * var(--tug-H-peak-c)`);
lines.push(`   *`);
lines.push(`   * Canonical (val=50): clamp(0,50,50)=50 and clamp(50,50,100)-50=0, so`);
...
const { canonical, light, dark, intense, muted } = HVV_PRESETS;

for (const hue of HUE_ORDER) {
  lines.push(`  /* ${hue} */`);
  lines.push(`  --tug-${hue}: ${presetFormula(hue, canonical.vib, canonical.val)};`);
  lines.push(`  --tug-${hue}-light: ${presetFormula(hue, light.vib, light.val)};`);
  lines.push(`  --tug-${hue}-dark: ${presetFormula(hue, dark.vib, dark.val)};`);
  lines.push(`  --tug-${hue}-intense: ${presetFormula(hue, intense.vib, intense.val)};`);
  lines.push(`  --tug-${hue}-muted: ${presetFormula(hue, muted.vib, muted.val)};`);
  lines.push(``);
}
...
lines.push(`   *   val=50: L = canonical-L = 0.555  (canonical, intense, muted — no chroma)`);
lines.push(`   *   val=85: L = 0.555 + 35*(${fmt(L_LIGHT)}-0.555)/50 = 0.555 + 0.284 = 0.839  (light)`);
lines.push(`   *   val=20: L = ${fmt(L_DARK)} + 20*(0.555-${fmt(L_DARK)})/50 = ${fmt(L_DARK)} + 0.162 = 0.312    (dark)`);
