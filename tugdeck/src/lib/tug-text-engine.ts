 ...
 *
 * contentEditable as input capture surface with an owned document model.
 * Inspired by Lexical/CM6 architecture with a UITextInput-inspired API layer.
 *
 * Architecture:
...
export interface CompletionItem {
  ...
  label: string;
  /** The atom to insert when this item is accepted. */
  atom: AtomSegment;
}
...
export class TugTextEngine {
  ...
  private reconcile(): void {
    ...
    for (let i = 0; i < this.segments.length; i++) {
      ...
      if (seg.kind === "text") {
        ...
        if (old instanceof HTMLSpanElement && old.dataset.slot === "tug-atom" &&
  ...
  private rebuildFromDOM(): void {
    ...
    for (let i = 0; i < this.root.childNodes.length; i++) {
      ...
      } else if (child instanceof HTMLElement && child.dataset.slot === "tug-atom") {
        newSegs.push({
          kind: "atom",
    ...
    for (let i = 0; i < this.root.childNodes.length && segIdx < this.segments.length; i++) {
      const child = this.root.childNodes[i];
      if (child instanceof Text ||
          (child instanceof HTMLElement && child.dataset.slot === "tug-atom")) {
   ...
   * The card's selectAll action checks document.activeElement for
