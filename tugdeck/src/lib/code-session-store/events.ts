...
export interface SendActionEvent {
  type: "send";
  /**
   * Raw substrate text — contains `U+FFFC` (object replacement) at
   * every atom position. Goes onto the `UserMessage` Message in the
   * scratch entry so the transcript chip renderer can walk `text`
   * looking for `U+FFFC` and read the corresponding atom for chip
   * placement. NEVER forwarded to the wire — see
   * [D01](../../../roadmap/tide-atoms.md#d01-ffc-substitution-at-submit).
   */
  text: string;
  /**
   * Parallel atom array — one entry per `U+FFFC` in `text`. Same
   * data the substrate stores on its `TugTextEditingState` snapshot.
   * Goes onto `UserMessage.attachments` for the substrate's atom
   * lookup at render / replay time.
   */
  atoms: AtomSegment[];
  /**
   * Wire-ready text — `U+FFFC` placeholders substituted with each
   * atom's `value`. This is what reaches claude on the wire. Built
   * by `buildWirePayload` in the impure store wrapper before
   * dispatch; the reducer just forwards it to the `send-frame`
   * effect verbatim.
   *
   * Per [D01](../../../roadmap/tide-atoms.md#d01-ffc-substitution-at-submit)
   * and [Spec S03](../../../roadmap/tide-atoms.md#s03-build-wire-payload).
   */
  wireText: string;
  /**
   * Wire-ready Attachment array — one entry per image atom whose
   * bytes are in the per-card `AtomBytesStore`. Order matches the
   * atoms' document order. Built by `buildWirePayload` alongside
   * `wireText`. Per [D02](../../../roadmap/tide-atoms.md#d02-image-attach-text-rest).
   */
