 ...
 *
 *  - `SessionRecentsDataSource` enumerates recent project paths in the
 ...
 *    elision.
 *
 *  - `SessionsDataSource` enumerates session-choice rows for the
 ...
 * the path.
 */
export class SessionRecentsDataSource implements TugListViewDataSource {
...
/**
 * Hook — mint a stable `SessionRecentsDataSource` per hook lifetime
 * and feed it the latest `(recents, query)` snapshot each render.
 */
export function useSessionRecentsDataSource(
  recents: ReadonlyArray<string>,
  query: string,
): SessionRecentsDataSource {
  const ref = useRef<SessionRecentsDataSource | null>(null);
  if (ref.current === null) {
    ref.current = new SessionRecentsDataSource({ recents, query });
  }
