/**
 * theme-css-generator.ts
 *
 * Shared CSS generation module for --tug-* design tokens.
 *
 * Used by:
 *   - scripts/generate-tug-tokens.ts  (build-time token generation)
 *   - vite.config.ts                  (dev-server middleware, hot reload)
 *
...
// ---------------------------------------------------------------------------
// buildTokenCssLines — formats token entries into CSS lines
//
...
function buildTokenCssLines(tokens: Record<string, string>): string[] {
  ...
  for (const entry of Object.entries(tokens)) {
    ...
    groups[group].push(entry as TokenEntry);
  }
...
export function generateThemeCSS(recipe: ThemeRecipe): string {
  ...
  lines.push(`body {`);
  lines.push(...buildTokenCssLines(tokens));
  lines.push(`}`);
