 ...
 *    `data-slot="tide-error-block"`, this docstring.
 *  - [L20] owns the `--tugx-err-*` slot family.
 *
 * @module components/tugways/chrome/tide-error-block
 */

import "./tide-error-block.css";
...
export const TideErrorBlock: React.FC<TideErrorBlockProps> = ({
  ...
  return (
    <div
      data-slot="tide-error-block"
      data-tugx-error-tone={tone}
      className="tide-error-block"
    >
      <span className="tide-error-block-icon" aria-hidden="true">
        {input.recoverable ? (
          <AlertCircle size={14} />
        ) : (
      ...
      <span className="tide-error-block-message">{input.message}</span>
      <div className="tide-error-block-actions">
        {input.recoverable && onRetry !== undefined ? (
          <button
            type="button"
            className="tide-error-block-button"
            onClick={onRetry}
            data-slot="tide-error-block-retry"
          >
        ...
        {!input.recoverable ? (
          <button
            ...
            data-slot="tide-error-block-copy"
          >
