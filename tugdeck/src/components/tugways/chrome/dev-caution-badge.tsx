 ...
 * ([#step-21](roadmap/dev-assistant-rendering.md#step-21)); both
 ...
 *  - [L19] file pair (`.tsx` + `.css`), `data-slot="dev-caution-badge"`,
 *    this docstring.
 *  - [L20] owns the `--tugx-caut-*` slot family; consumes the shared
 *    `--tugx-block-tone-caution-*` tones for the chip surface.
 *
 * @module components/tugways/chrome/dev-caution-badge
 */

import "./dev-caution-badge.css";
...
export function DevCautionBadge({
  ...
  return (
    <span
      data-slot="dev-caution-badge"
      data-caution-reason={caution.reason}
      className={
        className === undefined
          ? "dev-caution-badge"
          : `dev-caution-badge ${className}`
      }
      title={title}
    >
      ⚠ {label}
