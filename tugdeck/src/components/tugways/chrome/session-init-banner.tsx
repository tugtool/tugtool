/**
 * `SessionInitBanner` — top-of-card chrome that surfaces the
 ...
 *    `data-slot="session-init-banner"`, this docstring.
 ...
 *    `caution` prop.
 *
 * @module components/tugways/chrome/session-init-banner
 */

import "./session-init-banner.css";
 ...
 * `RenderInput`.
 */
export interface SessionInitBannerInputProps {
 ...
 */
export interface SessionInitBannerProps {
  input: SessionInitBannerInputProps;
  caution?: CautionFlag;
}

export const SessionInitBanner: React.FC<SessionInitBannerProps> = ({
  input,
  caution,
}) => {
  ...
  return (
    <div
      data-slot="session-init-banner"
      className="session-init-banner"
    >
      <Cpu size={14} aria-hidden="true" className="session-init-banner-icon" />
      {current.model !== undefined ? (
        <span
          className="session-init-banner-field"
          data-slot="session-init-banner-model"
        >
          <span className="session-init-banner-field-label">model</span>
          <code className="session-init-banner-field-value">
            {current.model}
          </code>
        </span>
      ) : null}
      {current.permissionMode !== undefined ? (
        <span
          className="session-init-banner-field"
          data-slot="session-init-banner-permission-mode"
        >
          <span className="session-init-banner-field-label">
            permission
          </span>
          <code className="session-init-banner-field-value">
      ...
      {current.cwd !== undefined ? (
        <span
          className="session-init-banner-field"
          data-slot="session-init-banner-cwd"
        >
          <span className="session-init-banner-field-label">cwd</span>
          <code className="session-init-banner-field-value">
            {current.cwd}
          </code>
        </span>
