/**
 * TugCreateSessionCard — the app-modal empty-deck affordance. A sibling of
 ...
 * back to work: a default double-ringed "Create Session Card" button that opens a
 * fresh Session card (which becomes first responder and presents the Choose
 ...
 * open state: an effect opens the chooser when {@link deriveCreateSessionCardOpen}
 * says so. "Create Session Card" resolves into `deck.addCard("session")`; "Open Text
 ...
 * create-session-card. During a genuine first run the setup wizard owns the empty
 * deck (its "Start a Claude Code session" step) until the first card has
 * existed; see {@link deriveCreateSessionCardOpen}. Suppressed under the same
...
export function TugCreateSessionCard(): ReactElement {
  ...
  const wantOpen = deriveCreateSessionCardOpen({
    gateOpen,
    suppressed,
    loggedIn,
    cardCount,
