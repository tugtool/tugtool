 ...
 *  - `BlockChrome` owns the frame: a `Zap` icon + the composed
 *    tool-name (`Remote Trigger · <action>`), the status stripe,
 *    the inline `SessionCautionBadge` (when the dispatch flagged
 ...
 *
 * @module components/tugways/cards/blocks/remote-trigger-tool-block
 */
...
function renderRemoteTriggerBody({
  ...
  return (
    <BlockBody>
      {hasAction ? (
        <BlockFieldRow label="action">
          <code>{input.action}</code>
        </BlockFieldRow>
      ) : null}
      {hasBody ? (
        <BlockFieldRow label="body" layout="stacked">
          <BlockPre>
