 ...
 *  - `BlockChrome` owns the frame: the tool
 *    name `Monitor` + an args summary that names what's being
 *    watched, the status stripe, the inline `SessionCautionBadge`
 ...
 *    arrived; body is `null` (the header dot is the in-flight signal).
 ...
 *    `rootSlot`).
 *  - [L20] reuses the chrome's `--tugx-block-*`; the wrapper's
 ...
 *
 * @module components/tugways/cards/blocks/monitor-tool-block
 */
...
export const MonitorToolBlock: React.FC<ToolBlockProps> = ({
          ...
          {tailComp !== null ? (
            ...
            >
              {tailComp.droppedLineCount > 0 ? (
                ...
                >
                  <BlockPre>{tailComp.head}</BlockPre>
                </BlockDisclosure>
              ) : null}
              <BlockPre>{tailComp.tail}</BlockPre>
            </div>
          ) : null}
        </BlockBody>
      );
  ...
  // Fold + copy affordances. Default OPEN — Monitor's body (until +
