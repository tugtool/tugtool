 ...
 *  - `BlockChrome` owns the frame: a `GitBranch` icon + a
 *    composed tool-name string (`Worktree · enter` / `Worktree ·
 *    exit`), the status stripe, the inline `SessionCautionBadge` (when
 ...
 *    every visible rule lives in `block-chrome.css` (the
 ...
 *
 * @module components/tugways/cards/blocks/worktree-tool-block
 */
...
export const WorktreeToolBlock: React.FC<ToolBlockProps> = ({
  ...
  if (status === "streaming") {
    ...
    if (bodyField === null) {
      ...
      body = (
        <BlockBody>
          <BlockFieldRow label={bodyField.label}>
