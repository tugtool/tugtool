 ...
 *  - `BlockChrome` owns the frame: a `Clock` icon + a
 *    composed tool-name string (`Cron · create` / `Cron · delete` /
 *    `Cron · list`), the status stripe, the inline `SessionCautionBadge`
 ...
 *    same change.
 *
 * @module components/tugways/cards/blocks/cron-tool-block
 */
...
function renderCreateBody(
  ...
  return (
    <BlockBody>
      {hasCron ? (
        <BlockFieldRow label="cron">
          <code>{input.cron}</code>
        </BlockFieldRow>
      ) : null}
      {hasPrompt ? (
        <BlockFieldRow label="prompt" layout="stacked">
          <BlockPre>{input.prompt}</BlockPre>
        </BlockFieldRow>
      ) : null}
      {hasRecurring ? (
        <BlockFieldRow label="recurring">
          <code>{String(input.recurring)}</code>
        </BlockFieldRow>
      ) : null}
      {hasDurable ? (
        <BlockFieldRow label="durable">
