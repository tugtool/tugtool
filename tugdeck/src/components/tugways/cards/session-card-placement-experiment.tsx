/**
 * session-card-placement-experiment.tsx — dev-only A/B harness for the
 * session card's display-only placement zones (Z0, Z1-assistant, Z2, Z3,
 ...
 *
 *   ```js
 *   window.tugSessionPlacement.set({ Z2: "window", Z4B: "phase" })
 *   window.tugSessionPlacement.set({ Z1: "perTurnDuration" })
 *   window.tugSessionPlacement.clear()                  // wipe to empty defaults
 *   window.tugSessionPlacement.get()                    // read current mapping
 *   window.tugSessionPlacement.datums                   // list available datums
 *   window.tugSessionPlacement.zones                    // list configurable zones
 *   ```
 *
 ...
 *
 * Conformance:
 *  - [L02] `useSessionPlacementSlots` subscribes to the tugbank-backed
 *    store via `useTugbankValue`; no React-state mirror.
 *  - [L06] Slot content is React nodes computed from the mapping. The
 *    renderer components themselves (in `session-card-telemetry-renderers`)
 *    own all DOM. No appearance state in React beyond the slot node
 *    identity.
 *
 * @module components/tugways/cards/session-card-placement-experiment
 */
...
import {
  SessionTelemetryCumulativeActiveMs,
  SessionTelemetryCumulativeTokens,
  SessionTelemetryPerTurnCost,
  SessionTelemetryPerTurnDuration,
  SessionTelemetryPerTurnTtft,
  SessionTelemetryPhase,
  SessionTelemetryStatusRow,
  SessionTelemetryWindowUtilization,
} from "./session-card-telemetry-renderers";
import type { SessionTelemetryStatusRowHandle } from "./session-card-telemetry-renderers";
import type { ScrollToRowHandler } from "./session-card-telemetry-popovers";
import type {
  SessionTurnTrailingContext,
  SessionTurnTrailingRenderer,
} from "./session-card";
...
function writePlacement(next: PlacementMap): void {
  ...
  fetch(
    ...
    console.warn(`[session-placement-experiment] PUT failed:`, err);
  });
}
...
export interface SessionCardPlacementSlots {
...
export interface UseSessionPlacementSlotsInput {
  codeSessionStore: CodeSessionStore;
  sessionMetadataStore: SessionMetadataStore;
  /**
   * Forwarded to {@link SessionTelemetryStatusRow} when it occupies Z2 —
   * lets the status row's Time / Tokens popovers scroll the transcript
   * on a `#NNNN` entry-number click. The session card supplies it from
   * the transcript's imperative handle.
   */
  onScrollToRow?: ScrollToRowHandler;
  /**
   * Forwarded to {@link SessionTelemetryStatusRow} when it occupies Z2 —
   * gives the session card a handle to open the CONTEXT popover for the
   * `/context` slash command. Null where the row isn't the current Z2
   * datum (the handle's calls then no-op).
   */
  statusRowRef?: React.Ref<SessionTelemetryStatusRowHandle>;
  /**
   * Authors the Z2 status cells into the session card's focus cycle ([P10]
   * revised) — forwarded to {@link SessionTelemetryStatusRow} when it occupies
  ...
  /**
   * Forwarded to {@link SessionTelemetryStatusRow} when it occupies Z2 — feeds
 ...
 * session card. Returns empty slots in production builds (or whenever
 ...
 * the slot quiet.
 */
export function useSessionPlacementSlots(
  input: UseSessionPlacementSlotsInput,
): SessionCardPlacementSlots {
  ...
  const sessionNode = useCallback(
    (datum: SessionDatum | null | undefined): React.ReactNode => {
      ...
      switch (datum) {
        ...
        case "phase":
          return <SessionTelemetryPhase codeSessionStore={codeSessionStore} />;
        case "statusRow":
          return (
            <SessionTelemetryStatusRow
              ref={statusRowRef}
  ...
  const renderTurnTrailing = useMemo<SessionTurnTrailingRenderer | undefined>(
    () => {
      const datum = mapping.Z1;
      if (datum === null || datum === undefined) return undefined;
      return (ctx: SessionTurnTrailingContext): React.ReactNode => {
        if (ctx.half !== "assistant" || ctx.turn === undefined) return null;
        switch (datum) {
          case "perTurnDuration":
            return <SessionTelemetryPerTurnDuration turn={ctx.turn} />;
          case "perTurnCost":
            return <SessionTelemetryPerTurnCost turn={ctx.turn} />;
          case "perTurnTtft":
            return <SessionTelemetryPerTurnTtft turn={ctx.turn} />;
        }
      };
    },
    [mapping.Z1],
 ...
 */
export interface SessionPlacementGlobal {
...
interface WindowWithPlacement {
  tugSessionPlacement?: SessionPlacementGlobal;
}

/**
 * Install the dev-only `window.tugSessionPlacement` shim. Idempotent —
 * the second call no-ops. Call once from `main.tsx` inside an
 * `import.meta.env.DEV` guard.
 */
export function installSessionPlacementGlobal(): void {
  if (typeof window === "undefined") return;
  const w = window as unknown as WindowWithPlacement;
  if (w.tugSessionPlacement !== undefined) return;
  const api: SessionPlacementGlobal = {
    get(): PlacementMap {
