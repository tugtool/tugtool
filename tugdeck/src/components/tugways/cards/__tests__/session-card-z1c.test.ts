/**
 * Pure-logic tests for the `sessionZ1CContent` helper ([D19] /
 * `#spec-z1c`).
 *
 * The helper resolves the per-phase indicator content for SessionZ1C
...
import { sessionZ1CContent } from "../session-card-z1c";
import type { CodeSessionPhase } from "@/lib/code-session-store/types";

describe("sessionZ1CContent — active phases", () => {
  test("submitting → 'Submitting…'", () => {
    expect(sessionZ1CContent("submitting", false)).toEqual({
      label: "Submitting…",
    });
  });

  test("awaiting_first_token → 'Thinking…'", () => {
    expect(sessionZ1CContent("awaiting_first_token", false)).toEqual({
      label: "Thinking…",
    });
  });

  test("streaming → 'Streaming…'", () => {
    expect(sessionZ1CContent("streaming", false)).toEqual({
      label: "Streaming…",
    });
  });

  test("tool_work → 'Tool work…'", () => {
    expect(sessionZ1CContent("tool_work", false)).toEqual({
      label: "Tool work…",
    });
  });

  test("waking → 'Waking…'", () => {
    expect(sessionZ1CContent("waking", false)).toEqual({
      label: "Waking…",
    });
  });
});

describe("sessionZ1CContent — non-indicator phases (slot collapses)", () => {
  test("awaiting_approval → null (the pending dialog is the affordance)", () => {
    expect(sessionZ1CContent("awaiting_approval", false)).toBeNull();
  });

  test("idle → null", () => {
    expect(sessionZ1CContent("idle", false)).toBeNull();
  });

  test("replaying → null", () => {
    expect(sessionZ1CContent("replaying", false)).toBeNull();
  });

  test("errored → null", () => {
    expect(sessionZ1CContent("errored", false)).toBeNull();
  });
});

describe("sessionZ1CContent — interruptInFlight precedence", () => {
  test("interruptInFlight=true overrides every phase to null (interrupt is instant; Z1B paints end-state)", () => {
    ...
    for (const phase of phases) {
      expect(sessionZ1CContent(phase, true)).toBeNull();
    }
  });
});
