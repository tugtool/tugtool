...
function ActionEventDemo() {
  ...
  return (
    <div className="cg-section" data-testid="action-event-demo">
      ...
      <TugLabel size="2xs" color="muted">Click the button to dispatch directly to a local responder node via sendToTarget. The handler receives the full ActionEvent and displays its fields below.</TugLabel>
      ...
      <TugLabel size="2xs" color="muted" data-testid="action-event-demo-status">{lastEventText !== null ? lastEventText : "No event received"}</TugLabel>
    </div>
  );
}
