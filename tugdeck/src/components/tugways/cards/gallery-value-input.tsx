...
export function GalleryValueInput() {
  ...
  // `value-input-md`; its parent React state must also seed from the
  // saved value at mount, otherwise the controlled `value={mdValue}`
  // prop would overwrite the saved value on captureState. See AT0030
  // fixture contract.
  const savedValueInputMd = useSavedComponentState<{ value?: number }>(
    "value-input-md",
  );
  ...
  const [mdValue, setMdValue] = useState<number>(
    () =>
      typeof savedValueInputMd?.value === "number"
        ? savedValueInputMd.value
        : 50,
  );
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
            ...
            <TugValueInput
              ...
              step={0.01}
            />
            <TugLabel size="2xs" emphasis="muted">{`Value: ${percentValue}`}</TugLabel>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
            ...
            <TugValueInput
              ...
              step={0.1}
            />
            <TugLabel size="2xs" emphasis="muted">{`Value: ${decimalValue}`}</TugLabel>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
            ...
            <TugValueInput
              ...
              step={1}
            />
            <TugLabel size="2xs" emphasis="muted">{`Value: ${integerValue}`}</TugLabel>
          </div>
        </div>
      </div>
