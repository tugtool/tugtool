 ...
 * so the owner can begin a reorder drag. It fills the trailing-most `grip`
 * slot of {@link BlockStrip} (the Lens section bands) and of `TugListRow`
 * (the Lens list rows), so every handle in a stack lines up on the right
 * edge. The handle lives in the shared affordance library (not a section
 * one-off) so a future entry-level reorder reuses it ([P04]).
 *
