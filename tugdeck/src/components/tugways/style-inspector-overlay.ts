...
// ---------------------------------------------------------------------------
// Extracted standalone exported functions
// ---------------------------------------------------------------------------
 ...
 *
 * Extracted from StyleInspectorOverlay for use by the card component.
 */
...
export class StyleInspectorOverlay {
  ...
  inspectElement(el: HTMLElement, cursorX: number, cursorY: number): void {
    ...
    // Resolve token chains for key properties (delegates to extracted functions)
  ...
  // ----- Token Chain Resolution (delegates to module-level functions) -----

  /**
   * Resolve the full token chain for a CSS property on an element.
   * Delegates to the module-level `resolveTokenChainForProperty` function.
   */
  resolveTokenChainForProperty(
    ...
    return resolveTokenChainForProperty(el, property, computedValue);
  }

  /**
   * Walk var() references to build the full token resolution chain.
   * Delegates to the module-level `resolveTokenChain` function.
   */
  resolveTokenChain(startProperty: string): TokenChainHop[] {
    return resolveTokenChain(startProperty);
  }

  /**
   * Extract TugColor provenance from a palette variable name.
   * Delegates to the module-level `extractTugColorProvenance` function.
   */
  extractTugColorProvenance(tokenName: string): TugColorProvenance | null {
    return extractTugColorProvenance(tokenName);
  }
  ...
  /**
   * Try to convert an oklch() color string to --tug-color() notation.
   * Delegates to the module-level `tryFormatTugColor` function.
   */
  private tryFormatTugColor(colorStr: string): string | null {
    return tryFormatTugColor(colorStr);
  }
  ...
  private renderChainSection(
    ...
    if (result.chain.length > 0) {
      ...
      if (result.endsAtPalette && result.paletteVar) {
        const tugColor = extractTugColorProvenance(result.paletteVar);
        if (tugColor) {
   ...
   *
   * Delegates to the extracted `buildFormulaRows` function, passing `this.reverseMap`.
   * The null guard for reverseMap is retained here.
   *
  ...
  private buildFormulaSectionForInspection(
    ...
    const allRows = buildFormulaRows(bgChain, fgChain, borderChain, formulasData, this.reverseMap);
