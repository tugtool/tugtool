 ...
 * slot and moves that slot between host card content elements via
...
describe("CardPortal", () => {
  ...
  it("re-roots into the new element when the registry updates", () => {
    ...
    act(() => {
      registry.register("card-1", host2);
    });
  ...
  it("does not unmount children when hostCardId changes to a new registered card", () => {
    const hostA = makeDiv();
    const hostB = makeDiv();
    registry.register("card-A", hostA);
    registry.register("card-B", hostB);
  ...
  it("does not unmount children when the host unregisters temporarily", () => {
    ...
    act(() => {
      registry.register("card-1", hostLater);
    });
    expect(counters.mount).toBe(1);
