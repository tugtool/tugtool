/**
 * Vite integration tests for postcss-cita plugin wiring.
 *
 * Verifies that postcssCita() is correctly wired into the PostCSS pipeline
 ...
 * time — this is the programmatic equivalent of "add a temporary --cita()
...
describe("postcss-cita Vite integration: plugin processes --cita() in CSS", () => {
  it("--cita(blue, 50, 50) expands to the correct oklch() value", () => {
    const css = "a { color: --cita(blue, 50, 50); }";
  ...
  it("expands --cita() in a realistic CSS rule (background declaration)", () => {
    const css = "body { background: --cita(cobalt, 3, 18); }";
    const result = processCSS(css);
    expect(result).not.toContain("--cita(");
  ...
  it("CSS without --cita() passes through the plugin unchanged (no transformations)", () => {
    const css = [
      ":root {",
  ...
  it("plugin instance returned by postcssCita() carries postcssPlugin name", () => {
    const plugin = postcssCita();
    expect(plugin.postcssPlugin).toBe("postcss-cita");
  });

  it("postcssCita.postcss static flag is true (marks plugin as PostCSS-compatible)", () => {
    expect(postcssCita.postcss).toBe(true);
  });
});

describe("postcss-cita Vite integration: coexistence with Tailwind v4", () => {
  it("CSS containing Tailwind @apply rules passes through without error", () => {
    ...
    // appear in the PostCSS pipeline. This test verifies postcss-cita does
    // not corrupt CSS it doesn't recognise.
    const css = [
      ".btn {",
      "  color: --cita(blue, 50, 50);",
      "  font-weight: bold;",
      "}",
    ].join("\n");
    const result = processCSS(css);
  ...
  it("CSS custom property declarations (non-cita) pass through unchanged", () => {
