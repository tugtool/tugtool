...
describe("theme production link swap (thin)", () => {
  afterEach(() => {
    ...
    if (link) link.remove();
    document.body.style.removeProperty("--tug-host-canvas-color");
  });

  it("adds/updates override link for non-brio themes", async () => {
    document.body.style.setProperty("--tug-host-canvas-color", "#e7eaf0");
  ...
  it("removes override link when switching back to brio", async () => {
    ...
    document.body.style.setProperty("--tug-host-canvas-color", "#16181a");
