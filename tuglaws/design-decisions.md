...
**D97.** The tide card is partitioned into **six placement zones, `Z0`-`Z5`, numbered spatially top-to-bottom**, with `Z1` subdivided into per-turn `Z1A` / `Z1B` and a transcript-level `Z1C`, and `Z4` subdivided into two prompt-entry toolbar sub-slots, `Z4A` and `Z4B`. Each zone is an addressable region the telemetry renderers and the assistant-rendering registry target by ID. Most are `ReactNode` slot props on a host component; `Z1C` and `Z5` are state-coordinated interactive areas rather than content slots. `Z4A` and `Z4B` are *layout positions* — a leading-fixed slot and a centred-floating slot — whose occupants the toolbar assigns and is free to swap (below). Spatial numbering means any zone is findable by intuition, and a future zone inserts at its position with a clean downstream renumber — `Z0` is unambiguously "what you see first."
...
│   └──────────────────────────────────────────────────────────────────────┘   │
│                                                                              │
│   ┌─ Z1C  transcript-level in-flight indicator chrome ──────────────────┐    │
│   │ ◌◌◌ Thinking…                                                       │    │
│   │ (chrome below TugListView; sibling of TideJumpToBottomButton;       │    │
│   │  visible only when activeTurn !== null; never moves with content)   │    │
│   └─────────────────────────────────────────────────────────────────────┘    │
├──────────────────────────────────────────────────────────────────────────────┤
...
| `Z1B` | per-turn end-state aggregate within the assistant-side trailing slot — the OK / interrupted / error badge, per-turn token count, whole-turn COPY. Rendered by `TideZ1B` | rendered when the committed turn has an assistant-side Message (per Step 5.6 [Decision F](../roadmap/tugplan-tide-session-wake.md#step-5-6)). NOT rendered for a turn that committed with only a `user_message` (interrupted-before-response) — the user_message row's own static "OK" footer is the only chrome. NEVER multiplexes the in-flight indicator (Z1C owns that) | shipped under Step 5.6 |
| `Z1C` | `TideTranscriptHost` — sibling of `TideJumpToBottomButton`, rendered as chrome BELOW `<TugListView>`. NOT a list row; does not participate in row count math, does not compete for scroll-to-row targets, does not churn React keys. | `TideZ1C` — phase-driven in-flight indicator: TugThinkingIndicator with "Submitting…" / "Thinking…" / "Streaming…" / "Tool work…" / "Waking…" labels by `snapshot.phase`; caution-tone pulse with "Awaiting approval" or "Interrupting…" by `snapshot.interruptInFlight`. Visibility toggled via `data-visible` per [L06] (visible when `snapshot.activeTurn !== null`; hidden between turns). Always mounted; never moves as new Messages mint within a turn. | shipped under Step 5.6 |
...
The transcript pane (the upper `TugSplitPanel`) is a flex column — `Z0` header, `TugListView` (`flex 1 1 auto`, scrolls), with `Z1C` and `TideJumpToBottomButton` as floating chrome anchored at the list's bottom edge, and `Z2` status bar — so a telemetry update in `Z2` grows into space the list cedes without repositioning the scroll, and `Z2`, living *outside* `TugListView`, never scrolls with the transcript. `Z1` is per-turn rather than card-level: one trailing slot wired twice per turn (`half: "user" | "assistant"`), spatially inside the transcript pane between `Z0` and the `Z2` row. `Z1B` (the per-turn end-state aggregate) and `Z1C` (the transcript-level in-flight indicator) are intentionally separate sub-zones — `Z1B` is a per-turn anchor that may or may not exist on a given turn (driven by the substrate's `messages` content per Step 5.6 Decision F); `Z1C` is a transcript-level anchor that never moves and is always available. The pre-Step-5.6 `Z1B` multiplexed both responsibilities ("indicator ↔ end-state display") inside one component; under per-Message rows the multiplex is structurally awkward — the in-flight indicator would have to hop between rows as new Messages mint mid-turn — so the two responsibilities split into two zones with distinct anchors. `Z5` is the submit button — a single DOM node whose label / `disabled` / `data-mode` are driven by the lifecycle state machine and which is never swapped across mode transitions, per [L26].
...
A zone's *location* is contract; its *occupant* is not — every zone is a generic addressable slot. `Z0`, the `Z1` user half, and `Z3` are reserved: the slot exists in the API, default content is `null`, and the row collapses to zero height until something fills it (the tide card leaves `Z3` empty today, so its row has no height). `Z1C` is a state-coordinated visibility slot (visible / hidden by `data-visible` per [L06]) rather than a content slot — its occupant `TideZ1C` is fixed; only the indicator's content and visibility vary with `snapshot.phase` / `snapshot.activeTurn`. Established by the Step 20.x turn-surface work (archived as `roadmap/archive/tide-assistant-turns.md`); `Z3`'s `cautionContent` slot was added by the assistant-rendering drift-detection work; the `Z4A` / `Z4B` split and the `Z2` chrome relocations came from the prompt-entry-zones work (`roadmap/tugplan-tide-prompt-entry-zones.md`); the `Z1B` / `Z1C` split (separating per-turn end-state from transcript-level in-flight indicator) came from Step 5.6 of the tide-session-wake work (`roadmap/tugplan-tide-session-wake.md`) under the per-Message render-layout refactor. [L06], [L26]
