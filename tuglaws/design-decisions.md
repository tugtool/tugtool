...
**D103.** PULSE — app-wide color commentary — is a **general facility**, not a Claude Code feature: a route-agnostic fact bus carried by tugcast, narrated by one persistent Haiku commentator, displayed as a one-line strip beneath the [D97] `Z2` status row. The architecture is a three-role split. **Producers** emit `pulse_fact` frames — `{ source, scope, kind, fact, at }` where `fact` is one producer-written plain-language sentence; integrating any future route (shell, external assistants) means emitting facts and nothing else. The first producer is tugcode (`pulse-facts.ts`): an observer on the outbound `writeLine` stream (muted across the `replay_started`/`replay_complete` bracket so reconnects never re-narrate history) plus a turn-start hook, with source throttles where an event class is unbounded. **The bus** is tugcast (`feeds/pulse.rs`): the session relay diverts `pulse_fact` stdout lines to the app-scoped pulse bridge (they never reach the deck-bound `CODE_OUTPUT` broadcast), which lazily spawns and supervises the daemon, persists its lines in a capped (~200) `pulse_lines` ledger table, and broadcasts them on the `PULSE` feed (`0x80`; `PULSE_FACT` = `0x81`). A reconnecting deck fetches the recent tail via the `list_pulse_lines` → `list_pulse_lines_ok` CONTROL pair (the `list_session_state_changes` shape — never feed replay), and the daemon re-seeds its inner session from the same tail at spawn. **The narrator** is `tugpulse`, a tugcode-codebase sibling binary driving one persistent `claude --model claude-haiku-4-5` stream-json subprocess that rides the user's existing CLI auth (the decisive constraint — OAuth users have no API key; `--bare` is unusable because it never reads OAuth). The spike-pinned posture (`stream-json-catalog/v2.1.173-pulse-spike/`) is exact-model-id + default permission mode + the release's full tool vocabulary in `--disallowedTools` (plan mode was REJECTED: it permits read-only tools and the model ran investigations) + `--setting-sources ""` + `MAX_THINKING_TOKENS=0` (the user's global `alwaysThinkingEnabled` cost 6–14s per one-line beat; the pinned posture measures p50 ≈ 0.7–1.0s). Beat discipline is pure logic (`pulse/scheduler.ts`): coalesce ~1.5s, fire at most every ~6s while facts flow, ONE beat in flight, replies past ~4s dropped not queued, digest capped to the newest facts, idle silent; the prompt contract is one line ≤ 110 chars (clipped defensively) with a DEFAULT-TO-PASS escape so uneventful beats emit nothing — under-narration is deliberately the chosen failure mode. **One-way isolation is structural**: commentator output flows only deck-ward; no code path leads from the pulse bridge into any session bridge's stdin. **Default ON**, killed by the `dev.tugtool.pulse/enabled` tugbank default (consulted per spawn opportunity — a flip takes effect without a tugcast restart; absent reads enabled). Deck-side, the app-scoped `pulse-store` ([L02]) folds the tail + live frames (rolling cap 20, line-identity dedupe) and surfaces the toggle on its snapshot; `TidePulseStrip` renders the newest line at fixed height with a fade keyed on line identity ([L06]), a dimmed placeholder before the first line, hidden entirely when disabled. Display is PER-CARD: the strip filters the app-wide log to the card's own session via `latestLineForScope` — a new session must never wear another session's commentary (live-walk correction; one narrator, per-card display). Multi-scope woven lines show on every card they cover; `"app"`-scoped (or scope-less) lines are ambience for all cards. The Shelf/Rack productionization (`roadmap/z2-status-redesign.md`) consumes this store for its PULSE lane and row item. Originating plan: `roadmap/pulse.md`. [D97], [D102], [L02], [L06], [L19], [L20], [L26]
---
