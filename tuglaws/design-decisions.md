...
**D112.** **Attribution is recorded at the point of change, never reconstructed.** File authorship for a session is captured in a `file_events` table in `sessions.db` from tugcast's `agent_bridge` relay loop — the one supervised point every tool frame already flows through. Write/Edit/NotebookEdit attribute exactly from tool inputs on a successful `tool_result`; only `Bash` needs working-tree fingerprint bracketing (snapshot on `tool_use`, delta on `tool_result`), and cross-session overlap on one checkout marks rows `ambiguous` rather than guessing. The invariant: persisting a file event is an upsert keyed `(tug_session_id, tool_use_id, file_path)`, so replay/resume converges idempotently and attribution failures never affect frame delivery (degrade to "forward unchanged"). Ambiguous/shared rows are excluded from the card's default commit selection — one session's commit never silently sweeps another's file. `roadmap/changesets-plan.md` [P01]–[P06], [P15].
