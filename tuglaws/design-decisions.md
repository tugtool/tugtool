...
│                                                                              │
├──────────────────────────────────────────────────────────────────────────────┤
│ Z2  [grip] Z2A pinned-todo (per [D100], when active) · Z2B  STATE · TIME ·   │
│            TOKENS · CONTEXT                                       [maximize] │
...
| `Z2A` | `TideCard` · `statusBarLeadingContent` | `TideCardPinnedTodo` — the active task list (`reduceTaskListState` over the code-session store; collapses to zero height when no active todos exist) | shipped (per [D100]) |
| `Z2B` | `TideCard` · `statusBarContent` | `TideTelemetryStatusRow` — STATE · TIME · TOKENS · CONTEXT, flanked by the leading sash grip and the trailing maximize toggle | shipped |
...
The transcript pane (the upper `TugSplitPanel`) is a flex column — `Z0` header, `TugListView` (`flex 1 1 auto`, scrolls), `Z2` status bar (`Z2A` leading + `Z2B` trailing in one flex row) — so a telemetry update in `Z2B` grows into space the list cedes without repositioning the scroll, and `Z2`, living *outside* `TugListView`, never scrolls with the transcript. `Z1` is per-turn rather than card-level: one trailing slot wired twice per turn (`half: "user" | "assistant"`), spatially inside the transcript pane between `Z0` and the `Z2A` / `Z2B` row. `Z5` is the submit button — a single DOM node whose label / `disabled` / `data-mode` are driven by the lifecycle state machine and which is never swapped across mode transitions, per [L26].
...
**D100.** The active task list lives in a single pinned slot — `Z2A`, the leading sub-slot of the [D97] `Z2` status bar — not inline in the assistant turn(s) that issued each call. Anthropic's `claude` ≥ `v2.1.148` replaces the old `TodoWrite` tool (single call carrying the canonical list) with a per-item CRUD family — `TaskCreate` (one call per task; the server assigns a monotonic `taskId`) and `TaskUpdate` (`{ taskId, status }` flips one task's status). A typical session produces ~3× more events than the old `TodoWrite` did (4 × `TaskCreate` up front + 8 × `TaskUpdate` interleaved for a 4-item list is representative), so inline-per-turn rendering would litter the transcript with a status-flip per row; pinning the assembled state keeps the live picture one fixed glance away. `Z2A` is leading-fixed and `Z2B` is the existing trailing status-row content (`TideTelemetryStatusRow`); the leading sash grip and the trailing maximize toggle remain card chrome and bookend the pair, matching D97's `Z4A` / `Z4B` sub-slot convention. The slot is fed by a pure reducer — `reduceTaskListState(toolCalls): TaskListState` — that folds the event stream by walking `transcript[].toolCalls[]` then the in-flight `toolCallMap`'s values in order, appending one `TaskItem` per `TaskCreate` (with `status: "pending"` and `taskId` parsed from the `tool_result.content` "Task #N created successfully:" echo, falling back to monotonic count) and mutating-by-id for each `TaskUpdate`; only terminal (`status: "done"`) calls fold so an in-flight `TaskCreate` with no assigned id yet is skipped until its `tool_result` lands. The `Z2A` renderer reads through `useSyncExternalStore` ([L02]) and composes `TodoListBlock` in standalone mode. The slot is *active* — and therefore visible — only when the reducer returns a non-empty list with at least one non-completed item; otherwise the renderer returns `null` and `Z2A` collapses to zero height via `:empty` ([L06]). The `TaskItem`'s `subject` is the row's primary text; the optional `description` surfaces in a `TugTooltip` on hover so each row stays one line. No wire-level clear event exists; the list lives for the session, and the active rule is the sole visibility gate. Session clear (`/clear`) resets `Z2A` by way of resetting the store. The per-call `TaskCreate` / `TaskUpdate` events render as a null `ToolBlockFactory` in the transcript so `Z2A` is the sole surface (the dispatch registers `taskcreate` and `taskupdate` to a `NullToolBlock` that returns `null`, suppressing both the prior `AUDIT_CONFIRMED_DEFAULT_TOOLS` default-block fallback and any `unknown_tool` caution); the dead `registerToolBlock("todowrite", …)` entry is removed by Step 24.2. The original Step 24's inline `TodoWriteToolBlock` was already silent on every `v2.1.148+` session — `TodoWrite` is no longer emitted — so this rework is also a correctness fix, not just a UX one. [D97], [L02], [L06]
