...
**D95.** Engine focus and framework focus are two distinct axes inside a content-owning card. A content-owning card (`bag.content !== undefined` — every tide card is one) captures `bag.focus` for any focusable target except the engine's contenteditable; the engine's `onCardActivated` callback (via `paintMirrorAsActive`) is authoritative for engine focus. The two never overlap by construction: `captureFocus` classifies engine focus as `kind: "component-owned"` and the framework-axis save site filters that case out for content-owning cards, leaving `bag.focus` absent so the engine's activation hook runs as the explicit default. Symmetric on restore: the cold-boot path and `resolveActivationTarget` both honour `bag.focus` only when the kind is `dom` or `form-control`, falling through to `dispatch-activated` (engine fallback) otherwise. This boundary is what lets a tide card host transient in-card UI (find row, future inline parameter editor, future question widget) whose focus survives activation transitions without breaking the engine's selection-restore machinery. See [state-preservation.md § `FocusSnapshot` in depth](state-preservation.md#focussnapshot-in-depth), `card-host.tsx` save/restore sites, and `focus-transfer.ts#resolveActivationTarget`. Previously documented inline as `[D07]` in `card-host.tsx` comments — that local tag has been normalized to reference this decision (the canonical D07 is the JSX-composition rule above; the focus-boundary rule is D95). [L23]
