...
The laws cited inline above are summarised below in citation order. Each link opens the full law text in [tuglaws.md](tuglaws.md), which is the authoritative source.

- **L24** — State is partitioned into three zones: appearance, local data, and structure. [full text](tuglaws.md#l24)
- **L02** — External state enters React through `useSyncExternalStore` only. [full text](tuglaws.md#l02)
- **L07** — Every action handler must access current state through refs or stable singletons, never stale closures. [full text](tuglaws.md#l07)
- **L11** — Controls emit actions; responders own state that actions operate on. [full text](tuglaws.md#l11)
- **L03** — Use `useLayoutEffect` for registrations that events depend on. [full text](tuglaws.md#l03)
- **L06** — Ephemeral appearance state goes through CSS and DOM, never React state. [full text](tuglaws.md#l06)
- **L08** — Live preview in mutation transactions is appearance-zone only; commit crosses zone boundaries. [full text](tuglaws.md#l08)
- **L04** — Never measure child DOM inline after triggering child `setState` from a parent effect. [full text](tuglaws.md#l04)
- **L05** — Never use `requestAnimationFrame` for operations that depend on React state commits. [full text](tuglaws.md#l05)
