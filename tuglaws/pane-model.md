...
**A mode flip is a fade plus one true height change, never a stretch** ([D135]). The settle's transform tween may carry a scale only within a capped raster distortion (`MAX_FLIP_SCALE_DISTORTION`), and a rail flip halves or doubles a member's height, which no cap admits. So the front member crosses by a real `height` tween — its subtree laying out truthfully on every frame, through TugAnimator, never rAF ([L13]) — while every buried member fades: in at its final tile on split, out behind the front member on stack, concurrently in the one settle window. Width presets past the cap cross by a real `width` tween the same way; only the adjacent step still rides `scaleX`.
