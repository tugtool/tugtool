...
**And what it publishes for a line it cannot fill is a STAND-IN, never `null`.** The tier is a fixed three lines, so an absent description is not a shorter masthead — it is a hole between two filled lines, with the quieter fact stranded below it. This is the session masthead's description ladder ([D132]) applied to documents: a Text card that is not a file yet says *Not saved to a file*, a Project Diff still reading says *Reading the diff…*, one with no repository behind it says *No repository here*. A stand-in is published as such (`descriptionStandIn`) and paints a step quieter than a real one, so a reader can tell a document that named its place from one that is merely being placed. Its third line follows the same discipline: a buffer nothing has ever written says **Draft**, because "Saved" is a claim about an act, and clean is not the same fact for a draft as it is for a file.
