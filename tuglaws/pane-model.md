...
**Width is chosen, not derived.** A content Pane's width comes from the user through one of three doors — a resize handle, the title-bar width popup, or the deck-wide **Card Width** default — and the last two write it through the same `movePane` a drag uses, held between the Pane's stack floor and ceiling and stamped as `widthPreset` so a picker can show which named width it is at ([D128]). A manual resize clears the stamp: a settled control shows what the geometry holds, never what it was last told. The imposer sees none of this; it places whatever width it finds.
...
| `tugdeck/src/components/lens/sections/layouts-section.tsx` | The Lens **Layouts** section — the arrangement, the deck's content width, and each sidebar's side |
