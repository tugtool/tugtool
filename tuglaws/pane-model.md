...
**Imposed is a slot.** The deck's `imposition.kind` (`"two-up" | "three-up" | "four-up"`) defines numbered slots across the span — the canvas minus the Lens's width and one gap. A slot is an anchor at a fixed fraction of the band: a Pane of width *w* in slot *k* of *N* sits `k / (N − 1) × max(0, band − w)` from the edge farthest from the Lens. Slot 0 hugs that edge, the last slot hugs the Lens, and the ones between space evenly.

The property that rule exists for: **a Pane's place depends on its own width and nothing else's.** No slotted Pane can see another, so closing, opening, widening, or restacking any card leaves every other card exactly where it was. A slot is a place in the arrangement, never a place in a queue. Slack therefore spreads evenly between cards rather than pooling in one margin beside the Lens; an arrangement that holds still is worth more than one whose margins collect in one place.
