...
5. **Card activation ([P21]) — two halves.** The first responder is the chain's *single* register, but focus is *per card* (see [focus-language.md](focus-language.md), the per-card key-window model). [P21] therefore has a framework half and a content half:
   ...
   **Framework half — the activation-moment reconcile.** `FocusManager.reconcileFirstResponder`, run from `setKeyCard` (the universal activation signal: click, tab switch, pane activation / frontmost change, pane cycle, cross-pane move, cold boot), settles the first responder on a responder that serves the key card — the key view's responder; for a never-focused card, the responder owning its **default-focus target** (the card-author-tagged priority chain `traceApplyDefaultFocus` walks — `data-tug-focus-key="primary"` first, generic focusable last); as a last resort, the card's own container responder (Cmd-W must still land). Its yield rules use *registry* containment (`nodeIsWithin`, the `parentId` walk), never DOM containment, so a pane-modal sheet portaled outside the card's DOM but chain-parented into it keeps first responder; a finer or sibling in-card promotion (a clicked editor) is never clobbered; only a coarse chain-*ancestor* container (the pane/card responder a chrome click or a pre-mount `addCard` promotion left) is overridden — a coarse container must not mask the card's content accelerators behind the upward-only walk.

   **Content half — the em-card lifecycle-delegate contract.** An engine-managed card's content owns its focus lifecycle ([lifecycle-delegates.md](lifecycle-delegates.md)): it registers a `TugCardDelegate` and **reclaims its focus destination on `cardDidActivate`** — whose initial-sync covers a card created-and-activated before it even mounted (Open Quickly, File ▸ Open) — and again at its own **editor-attach moment** (an async file bind that completes after activation). The reclaim goes through the sanctioned gates (`adoptKeyCard` for a trapped dialog, else `focusResponder(editorResponderId)`), and is self-gated on the card still being the first-responder card so an inactive card's late bind never steals focus. The Dev and Text cards are the reference implementations; **every new em card must implement this contract** — it is what makes accelerator routing uniform across card types with no engine-side special cases.

   This is the one engine-owned promotion path; component code still must not call `makeFirstResponder`. The DOM-focus half of activation (the caret) remains the separate `applyBagFocus` single channel — accelerator routing needs only the chain register.
...
- **Per-cell floating surfaces.** A `TugListView` cell renderer that mounts its own popover, alert, sheet, context menu, or any other transient floating surface. The N parallel responder lifetimes interact unpredictably with virtualization, data updates, and Radix portal cleanup — when a row unmounts on a confirm-action, the popover's cleanup cascade collides with the cell's React-tree teardown, and listeners leak / focus restoration runs against detached triggers / subsequent clicks find the chain in an inconsistent state. Hoist the surface to the responder above the list (typically the form), keep one floating-surface instance, and address the right anchor at request time via a `data-id` lookup or a callback ref. The Dev picker's session-forget flow is the canonical case study — see [tugplan-tide-picker-redesign §D14](../roadmap/tugplan-tide-picker-redesign.md#d14-no-per-cell-popovers). Cell renderers themselves should be pure functions — see the cell-renderer rule in [component-authoring.md](component-authoring.md).
