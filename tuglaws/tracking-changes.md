...
**Contention is read per region, not per path.** Most cross-session overlaps are coincidences of the file — two sessions touching disjoint parts of one thing — and file-level `shared` over-warns on every one of them. So a path with two or more live proof owners is resolved one level down (`tugchanges_core::contention`, one pure function over `(hunks, per-owner anchors)` that both readers call, because a rule this intricate cannot be hand-mirrored in two places): each owner's recorded anchors are matched against the file's *current* hunks, and `shared` is true iff two owners' claimed hunk sets intersect. The wire carries each owner's `own_hunks` and the `contested_hunks` beneath them, so the card can default-elect a session's own regions and mark the bands where the overlap actually is.

**Every uncertainty widens to the whole file**, which is what makes this safe to have built: an owner with no spans, a `whole` anchor, an anchor that matches nothing, or an anchor that matches two hunks claims the entire file — reproducing the file-level answer exactly. Two sessions that wrote identical text into one region genuinely *are* contending, and matching both marks it shared, which is the correct reading. The failure direction is over-report of `shared`, never a false sole claim.

**Spans annotate; `git diff` decides** — the same two-layer split one level down. An anchor is content, never a line number ([P11]): line numbers die the moment any other edit lands above them, while written text still matches against the current diff whenever it is read. The cost is bounded by contention rather than by the dirty set: the extra `git diff` runs only for paths with two or more proof owners *and* at least one owner holding spans, which is normally no paths at all. Wall-clock overlap between sessions is never evidence: the retired design cross-marked rows `ambiguous` whenever two sessions' Bash brackets were open on the same repo root at the same moment, which false-positived on every unrelated concurrent command while adding nothing real. Files that genuinely contend are excluded from every default commit set (the card's one-click commit and `tugutil commit` alike) and included only by explicit election (`--all`, `--tree`, `--paths`); the claimant sessions are named alongside the flag. (Accepted gap: two sessions genuinely editing one file *only* via Bash — no proof row on either side — is not flagged `shared`; both see the same hinted-unattributed file, and the failure direction is under-report, never a false claim.)
...
| Changeset card / feed | `feeds/changeset.rs`, `feeds/changeset_all.rs` ([D113]) | Composes live ledger rows per project (same liveness rule); resolves multi-owner paths through the hunk-aware verdict and marks `shared` only on true overlap, carrying each owner's `own_hunks`/`contested_hunks`; the card's default selection is `!shared` for session files and **OFF for unattributed** (inclusion is an explicit per-file election — the card mirror of the exit-3 refusal), and on a contended file the default *election within* the file is this session's own hunks |
...
6b. **Sub-file evidence narrows contention and never widens ownership.** Spans say where inside a file a proof row wrote; they can only turn a `shared` file into two disjoint co-owned ones, never turn a non-owner into an owner. Every uncertainty — no spans, a `whole` anchor, an anchor matching nothing or matching two hunks — claims the whole file, which is the answer the file-level rule already gave.
