...
- **Summary:** The pane title bar carries only the width popup and close controls — the per-pane `…` card-settings button is retired (settings live in the app-level Settings card). DOM-only, no native CGEvents.
...
#### [AT0194] Window-shade collapse — RETIRED
- **Status:** ⛔ retired with the feature.
- **Tests:** none (`at0194-window-shade-collapse.test.ts` deleted).
- **Summary:** Window-shade collapse is gone — it was pointer-only, with no command, chord, or menu item, and its title-bar slot went to the width popup ([AT0156]). The number is retired rather than reused. Of its two claims, the chrome's border-box painting is still stated where it is enforced (`tug-pane.css`), and the height-preserving drag commit is moot now that no frame ever renders at a stub height.
...
- **Status:** ✅ open (new feature gate).
- **Tests:** `at0215-composer-route-chrome.test.ts` (renamed from `at0215-route-chrome.test.ts`).
- **Summary:** The `code | shell | btw` Session card. The per-route Z4B chrome manifest mounts exactly its Table T01 chip set on each route (chips a route drops UNMOUNT — code shows Mode/Model/Effort, shell shows Project/Cwd, btw shows Session/Project; the code route gave up its Session and Project chips to the Z4B diet, both names already standing in the pane title bar), verified across a choice-group click (→ Shell) and the ⇧⌘B keybinding (→ btw). Risk R04: the leading Z4A choice group and trailing Z5 submit button stay pixel-fixed while the centred-floating Z4B cluster swaps width. The `?`-route submission is a native side question ([P02]): submitting on btw opens the overlay and the ask + settled answer never change the transcript entry count (the [D108] invariant, beside AT0211). Guards [P01]/[P02]/[P03], Table T01. Native CGEvents (click + type + key) + a synthetic ⇧⌘B keydown.

#### [AT0216] Shell route — exchange e2e, live cwd, restore interleave
- **Status:** ✅ open (new feature gate).
- **Tests:** `at0216-shell-exchange.test.ts` (renamed from `at0216-shell-route.test.ts`).
...
- **Status:** ✅ open (regression gate).
- **Tests:** `at0290-jot-delete-confirm-anchor.test.ts`.
- **Summary:** A Jots row's ✕ is a hover reveal and the confirm popover it raises covers the row it came from, so a ✕-anchored popover loses its anchor while open, re-resolves, and hops away from the pointer about to answer it. The anchor is the ROW, which outlives the question: the popover's rect is sampled repeatedly and holds (within a pixel of subpixel height rounding), it is centered on the row's midpoint and sits above it (`data-side="top"`, arrow pointing down), and its center is nowhere near the trailing-edge ✕ — the assertion that actually catches an anchor regression, since the harness leaves the clicked row SELECTED and a selected row keeps its accessories revealed, so the collapse never occurs under test. Also gates the size: a controlled-mode confirm carries `data-anchored="external"` and drops the trigger-width floor, so the box is narrower than the full-width row it is anchored to.

#### [AT0296] The Lens row is the handle
- **Status:** ✅ open (regression gate).
- **Tests:** `at0296-lens-row-is-the-handle.test.ts`.
- **Summary:** There is no reorder grip on a carry-bearing row: a press anywhere on a row that is not one of its controls arms a carry, and travel past the threshold engages it. The gate asserts the absence directly — every other assertion here would still pass with a grip present — and then pins the hard case, the Jots row, whose incipit is ALSO a native HTML5 drag source into a session prompt. One surface, two drags, told apart by axis: a vertical drag reorders (the DOM order moves AND `jots.json` commits it), a horizontal one of the same length leaves the order untouched, which is the assertion that fails if the axis arbitration is dropped and the carry starts swallowing the drag-out.
...
#### [AT0350] ⌥⌘] / ⌥⌘[ rotate a slot's stack as a ring, and the directions are inverses
- **Status:** ✅ open (new feature gate).
- **Tests:** `at0350-cycle-stack-ring.test.ts`.
- **Summary:** The no-look half of the slot-stack surface makes one promise a menu does not have to: that repeated presses form a ring — every pane in the slot, then home — because nothing is put on screen to be aimed at. The obvious rule breaks that. Raising the pane one below the front ping-pongs between the top two forever and never reaches the third, since each raise rewrites the very order the "next" was computed from; raising the BOTTOM-most pane instead sends the outgoing front pane exactly one place back, so a depth-N slot is home again after N presses. So this needs three panes to say anything — at depth 2 every candidate rule looks identical — and it checks the whole front-to-back order after each of three presses rather than only which pane came up, because a rule that raised the right pane while scrambling the ones behind it would pass a front-only assertion and fail on press four. The three panes are given descending widths on purpose: a fully covered pane is stamped `data-occluded="true"` and hidden, which would make "where is it now" unreadable for the two that are not in front. The second test pins the reverse direction ([D129]): ⌥⌘[ must be ⌥⌘]'s exact inverse — the front pane goes all the way to the back via `sendPaneBehind`, not a raise of the second-from-top, which is the ping-pong rule wearing the other label — so it asserts previous-then-next is a no-op and that three previouses walk the ring backwards to home, whole order every time.

#### [AT0341] The Lens's arrows point where they say
- **Status:** ✅ open (regression gate + drift gate).
- **Tests:** `at0341-lens-cross-section-arrows.test.ts`.
- **Summary:** The liveliness net is a *linear* walk — Down and Right both advance — so a surface that rides it has one direction, wearing four keys. The Lens looked like the net's ideal subject (a rail of stacked sections walked top to bottom) and was not, because a section band is a ROW: the band, its filter field, the section's own controls, its fold cue. The walk therefore ran across each band before it ran down into the rows the band is a header for, and a Down on a band stepped sideways into the filter field. This tours the declared plane that replaced it, end to end, on the real Lens with real keystrokes, reading the engine key view rather than a count of presses. Down on a band lands in that section's rows; Down off a list's last row crosses into the next section's band; Right runs the band in the order it reads and wraps at its end rather than spilling into the rows; Left off a body returns to its own band; Up retraces the column and enters each row at its LEADING member, so coming back up from the Layouts band reaches the Cards list rather than the Cards fold cue. The empty-field release is checked in the same tour (an empty `TugFilterField` spends its arrows on movement, a field holding a query keeps them for its caret), and the whole thing closes on a zero-steal focus-invariant report, so a landing that only *looked* right because something wrote focus behind the engine's back fails here.

  The second test is the **drift gate**, and it names no section at all: it asks the running Lens which sections it is rendering and holds each of them to the two claims the plane makes — Down LEAVES a band (into that section's own body, or onto the next section when this one is folded or empty, but never onto another stop of the same band, which is exactly what the linear walk gave), and Right STAYS on it. It reaches every band by walking the column and letting it wrap, so it does not depend on where the ⌘L seed lands. This pairs with the plane being derived from the engine's live registrations rather than from a maintained table: a section added later, or a control added to an existing band, joins the plane by registering and is held to it here on the same day, with no edit to either file. The failure it is written against is the quiet one — a stop off the plane still *works*, because the liveliness net catches its arrows, so the only symptom is that the direction stops meaning anything.
...
- **Summary:** Nothing in the Lens is reachable by arrow except by being registered, so "the band is in the walk" is a claim about a registration, and the only way to check it is to walk there with real keystrokes and read where the engine says the ring is. The first test does exactly that, and reads the ring's ADDRESS (`data-tug-focus-key`) rather than counting presses, because the band's stops are ordered fractionally between the filter field and the list and a count would pass on the wrong one. Arrowing up off the top of the Cards list crosses the list's edge and lands on the band — the list's own cursor owns the first press or two, so this walks rather than stepping once — and the band SHOWS it, which is a separate assertion because the ring is painted by an attribute the registration does not itself guarantee. Arrowing RIGHT then reaches the fold chevron without passing through the list; the direction is itself the assertion (the band is a row, its controls are beside it, and Down means the rows below — see at0341), and "the route does not contain the list's key" is what would catch a chevron left an un-authored native button and simply stepped over. Space there folds and unfolds. A folded section renders no body at all, so the last assertion is that its band and chevron survive their own fold: they are the only stops it has left, and the only way back in. The second test pins the fold KEY across the rail's two levels: Space on a Cards group header (where the cursor is the list's, and Space is the list delegate's to route) and Space on the band itself (where the band is a leaf, and the engine would otherwise complete the synthesized press as a click — which on a band means *enter the section*, the opposite of folding it). It also pins the group header's shape, because that is what makes one key read as one idea: the header's fold is a `BlockFoldCue` in the row's trailing slot, the band's own affordance at the band's own edge, and its leading column is the group's kind glyph rather than a second style of chevron.

### Layouts rework — sidebar cards and content widths (AT0357)

#### [AT0357] The deck's content width reaches every content pane
- **Status:** ✅ open (new feature gate).
- **Tests:** `at0357-content-width-default.test.ts`.
- **Summary:** **Card Width** in the Lens Layouts section is a deck-wide statement, and this is the difference between it and the per-pane popup ([AT0156]). Choosing slim lands BOTH open content panes on 675 — not the active one, and not the next one created — and leaves the Lens rail at the width the seed gave it, because a rail's width is the space allocator's unknown and a content width that dragged it along would be overwriting the imposer's own arithmetic from outside. A card opened *after* that choice arrives at 675 too, which is the registration change the phase made: Session, Text, File, Diff, and DevTools no longer carry a literal opening width but resolve one from `imposition.contentWidth` at `addCard` time. Choosing comfy then puts everything back on 800 including the card opened since — the default is not a one-shot seed, so re-asserting it is a real gesture. Slim and comfy are what it asserts because both fit any canvas the harness launches at; wide is the same code path with a bigger number, and asserting it would only be asserting the canvas.

#### [AT0358] The slim preset actually fits
- **Status:** ✅ open (regression gate).
- **Tests:** `at0358-slim-width-regression.test.ts`.
- **Summary:** Slim (675) is a width the Session card's chrome was put on a diet to reach, and every part of that diet is one commit away from being undone by somebody adding a cell or a chip back. The failure is silent in both directions, which is why this gate asserts three things rather than one: Z2 degrades by HIDING cells through a container ladder (an overfull row goes quiet, and an instrument that stopped reporting looks like one with nothing to report), while Z4B has no degradation machinery at all (only its spacers flex, so it pushes its flanks off their edges instead). So: the card lands on 675, all five cells are laid out at the same widths they had at comfy and share one row, and Z4A/Z5 keep their distance to the toolbar's edges. Measured from rects throughout — `scrollWidth - clientWidth` answers 0 here whatever the children do, because neither the strip nor the toolbar is a scroll container, and "one distinct `top`" is false even on a good row because STATE sits a pixel off its neighbours. The guard was checked against a real regression, not just written: a probe widening one Z2 cell far enough to break the strip fails it at 221px of spill. Because the row centers its cells between two flexing margins, it absorbs some growth before spilling — so this catches a cell ADDED, not a cell nudged.
