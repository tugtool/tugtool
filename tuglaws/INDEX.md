...
- [dev-card-unsupported-slash-commands.md](dev-card-unsupported-slash-commands.md) — Why some of Claude Code's slash commands are hidden from the dev card's `/` popup. The maintained mirror of `HIDDEN_SLASH_COMMANDS` in `slash-supported.ts`; the discoverable answer to "why isn't `/vim` here?". Doctrine lives in `slash-commands.md`.
