...
- [scroll-intent.md](scroll-intent.md) — The scroll attribution doctrine: unattributed scrolls belong to the user, no deferred scroll write survives a user gesture. The WKWebView attribution inventory (including the event-silent native scrollbar), the supersede table, and the rule new `scrollTop` writers must satisfy. Grounds [D93]'s phase guard.
