...
Computing the live chain requires the whole file, since the walk starts at the newest leaf. The disk scanner therefore streams unbuffered and restarts once as a buffered pass — re-deriving the effective sequence at EOF — only when the file holds a compaction, a duplicate uuid, or a non-linear parent edge; a linear, compaction-free session keeps the single-pass, allocation-free cost. Every parse, recounted or not, hands out a resumable frontier that carries the **effective chain uuid set** (16-byte keys, persisted in the scan cache). That set is what makes compacted sessions incrementally resumable: a re-append block that straddles a scan boundary arrives looking like ordinary linear appends, but every record in it carries an already-effective uuid and is suppressed inline — it feeds neither the count nor the triggers. On a resumed slice a full re-stream is reserved for the shapes that can *edit history*: a new compaction boundary, a mid-file segment root, an off-leaf **user submission** (a rewind), or a tail fingerprint that says the prefix was rewritten on disk. An off-leaf record that is not a user submission — a hook attachment, a `tool_result` whose sibling carried the chain forward, an abandoned API-retry spur — is absorbed inline instead, because dead branches root exclusively at user submissions and so a non-user record can neither kill nor resurrect a prefix record. A partial final line is likewise no bar: an unterminated line is deferred whole to the next scan rather than poisoning resumability, since active sessions race the scanner constantly. After a history-editing re-stream the fresh seed resumes incrementally again. **A session never re-streams in full on an ordinary append** — certified by a split-point property test over the real reference sessions.
