...
**A carry suppresses the ring.** While a block reorder is in flight, the container declares `data-tug-carrying="true"` and every focus **ring** inside it stands down until the drop. This is the language's one appearance exception, and it is narrow on purpose: the ring only stands down, it is never *moved*, because nothing about the keyboard has changed — the key view is exactly where it was and the ring is back the moment the gesture ends.

It exists because a drag introduces a second edge-drawn mark in the same hue, the drop caret, answering a different question. The ring answers *where is the keyboard*; the caret answers *where will this land*. A ring drawn around the whole list is the one shape that swallows a caret sitting at that list's top or bottom edge, and a drop the user cannot aim is worse than a focus mark they can reconstruct in a keystroke. Only the ring yields — the cursor mark stays, both because it rides one row on an axis the horizontal caret never uses and because it is what says where the keyboard will be when the carry ends.

The attribute is a **declared** state, in the `data-tug-*` family with `data-tug-placement` and `data-tug-focus`: the carrying host stamps it, and the components inside read it. `useBlockReorder` is the only writer; `TugListView`'s stylesheet is the ring's reader, because a container's ring is the container's to draw (see the authoring contract below) and a host must never hand-roll the suppression against a component's internal focus attributes.
...
- **The snippets editor's placeable re-declaration is one-shot and depends on another component's DOM.** The snippet row's chrome declares `data-tug-placement="suppress"`, and a mount-time layout effect re-stamps `"place"` on the CM6 root inside it so clicks in the *text* still place. That stamp runs once and targets an element `TugMessageEditor` owns. If the editor ever recreates its CM6 root, or mounts it after that effect, the marker is gone and clicks in the snippet text classify as suppressed. **Symptom to watch for:** the caret stops following clicks inside an open snippet while clicking its chrome behaves correctly. CM6 does not normally recreate its root, so this is latent rather than live.
