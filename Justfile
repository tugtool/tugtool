...
test-in-app:
    ...
    cd tests/in-app
    STATUS=0
    for f in _smoke.test.ts _smoke-native.test.ts _smoke-cold-boot.test.ts _smoke-app-reload.test.ts m01-tab-switch-fc.test.ts m03-pane-activation.test.ts m16-tab-close-handoff.test.ts; do
        echo "---- $f ----"
        bun test "$f" || STATUS=$?
