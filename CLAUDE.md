...
Piping it into `grep`/`head`/`tail` is worse than redundant: the pipeline's exit status becomes the filter's, so `just app-test X | grep -A 8 "Failures:"` on a **passing** run prints nothing and exits 1 — a green run reported as a silent failure. A fixed `-A N` window also truncates the second failure and drops `Diagnostics:` entirely. `tugplug/hooks/gate-app-test-output.sh` handles these pipelines: when the command is a plain app-test run feeding filters and nothing else, it strips the filter and runs the bare command; anything it cannot rewrite safely — a `&&` chain, a subshell, a redirect into a file — it denies.
