/**
 * at0016-rapid-cadence.test.ts — Three rapid back-to-back tab-close
 * handoffs preserve the same focus-handoff behavior as the slow-
 * cadence `at0016-tab-close-handoff.test.ts`. Regression gate for the
...
describe.skipIf(!SHOULD_RUN)(
  "at0016-rapid-cadence: three back-to-back closes hand focus correctly",
  () => {
    test("seed [c1..c5] active=c3, close c3→c2→c1, c4 focused", async () => {
      const app = await launchTugApp({ testName: "at0016-rapid-cadence" });
      try {
        ...
        if (tail !== "") {
          process.stderr.write(
            `\n[at0016-rapid-cadence] Tug.app log tail (last 200 lines):\n${tail}\n`,
          );
        }
        const tracePath = await app.dumpTraceToFile(
          "logs/at0016-rapid-cadence-trace.json",
        );
        if (tracePath !== null) {
          process.stderr.write(`[at0016-rapid-cadence] trace dumped to ${tracePath}\n`);
        }
        throw err;
      } finally {
        await app.close();
