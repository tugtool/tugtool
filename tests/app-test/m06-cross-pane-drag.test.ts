...
describe.skipIf(!SHOULD_RUN)("m06: cross-pane drag preserves focus + value", () => {
  test("drag A from P1 into P2's tab bar; A becomes active in P2 with focus + value preserved", async () => {
    const app = await launchTugApp({ testName: "m06-cross-pane-drag" });
    try {
      ...
      if (tail !== "") {
        process.stderr.write(
          `\n[m06-cross-pane-drag] Tug.app log tail (last 200 lines):\n${tail}\n`,
        );
      }
      const tracePath = await app.dumpTraceToFile(
        "logs/m06-cross-pane-drag-trace.json",
      );
      if (tracePath !== null) {
        process.stderr.write(`[m06-cross-pane-drag] trace dumped to ${tracePath}\n`);
      }
      throw err;
    } finally {
      await app.close();
