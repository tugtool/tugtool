/**
 * at0078-dev-engine-focus-survives.test.ts — session-card engine focus
 ...
 * session-card when the user genuinely had engine focus at save
 * time.
 *
 * ## Shape
 *
 ...
 *   6. Assert `document.activeElement` is the session-card's
 *      contenteditable.
 *
...
describe.skipIf(!SHOULD_RUN)("AT0078: session-card engine focus survives app-switch", () => {
  test(
    "cmd-tab away + back preserves focus on the session-card contenteditable",
    async () => {
    ...
    // never activates).
    foreground: true, testName: "at0078-dev-engine-focus" });
      try {
        ...
        // Focus lands back on the session-card contenteditable via the
