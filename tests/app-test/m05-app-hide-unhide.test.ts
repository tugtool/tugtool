...
describe.skipIf(!SHOULD_RUN)("m05: app hide → unhide preserves focus + value", () => {
  test("simulateAppHide + simulateAppUnhide restores focus inside first-responder card", async () => {
    const app = await launchTugApp({ testName: "m05-app-hide-unhide" });
    try {
      ...
      if (tail !== "") {
        process.stderr.write(
          `\n[m05-app-hide-unhide] Tug.app log tail (last 200 lines):\n${tail}\n`,
        );
      }
      const tracePath = await app.dumpTraceToFile(
        "logs/m05-app-hide-unhide-trace.json",
      );
      if (tracePath !== null) {
        process.stderr.write(`[m05-app-hide-unhide] trace dumped to ${tracePath}\n`);
      }
      throw err;
    } finally {
      await app.close();
