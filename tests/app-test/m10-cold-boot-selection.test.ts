/**
 * m10-cold-boot-selection.test.ts — DOM selection restore across
 ...
 *
 * See the matching note in `m14-cold-boot-scroll.test.ts`. Test
 ...
 *
 *     just test-in-app-fast m10-cold-boot-selection.test.ts
 *
 * ## Closes
 *
 * [M10] cold-boot variant. The existing `m10-markdown-selection.test.ts`
      ...
      try {
        ...
        // ── Phase A: seed deck, anchor selection, quitGracefully. ──
        {
          const app = await launchTugApp({
            testName: "m10-cold-boot-selection-A",
          ...
          // gesture (mirrors m10-markdown-selection's preflight).
          ...
          // m10-markdown-selection — the selectionchange listener
          ...
          if (seededOpt === null) {
            throw new Error("[m10-cold-boot] could not seed a selection — first block had no usable text node");
          }
          seeded = seededOpt;
        ...
        {
          const app = await launchTugApp({
            testName: "m10-cold-boot-selection-B",
            env: { TUGBANK_PATH: tugbankPath },
            persistInTestMode: true,
          });
          try {
