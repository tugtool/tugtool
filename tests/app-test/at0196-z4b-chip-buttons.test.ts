/**
 * at0196-z4b-chip-buttons.test.ts — the Z4B "Claude Code" and "AI" chips are
 * interactive buttons with their assigned click behaviors ([AT0196]).
 *
 * ## Why this exists
 *
 * The Z4B indicator cluster used to render its left chips as display
 * `TugBadge`s, then as four `TugPushButton`s ([D13]). They are two chips now:
 *
 ...
 *   - **AI Model Settings** (`ai-chip`): a left click opens the configuration
 *     mixer sheet, whose footer carries its own changelog link.
 *
 * The changelog clicks are driven for real with `window.open` stubbed to
...
describe.skipIf(!SHOULD_RUN)(
  "AT0196: the Z4B Claude Code and AI chips are interactive buttons",
  () => {
    test(
      "Claude Code: click opens the changelog, right-click the report; AI: click opens the mixer (whose footer also opens the changelog) — and the dieted chips are absent",
      async () => {
        ...
        try {
          ...
          // Both chips render as real <button>s…
          await app.waitForCondition<boolean>(
          ...
          // Left click on the AI chip → opens the configuration mixer, NOT a
          // browser tab (the count holds at the badge's one changelog open).
          ...
          expect(
            ...
            "the AI chip click opens the sheet, not a browser tab",
          ).toBe(1);

          // The mixer's footer carries its own changelog door.
          await app.nativeClickAtElement(CHANGELOG_LINK);
          await app.waitForCondition<boolean>(
            `window.__z4b.openUrls.length > 1`,
            { timeoutMs: 4000 },
          );
          expect(await app.evalJS<string>(`window.__z4b.openUrls[1]`)).toBe(
            CHANGELOG_URL,
          );
        } catch (err) {
