/**
 * at0009-em-inactive-mount.test.ts — saved-state EM card mounted in
 * an inactive tab → activated via tab switch → engine focuses
 * correctly via the dispatch-activated path ([AT0009]).
 *
 * ## Scenario
 *
 ...
 * ## Coverage
 *
 * `gallery-prompt-entry` (TugPromptEntry wrapper, what session-card
 ...
 * session-card cold-boot selection gap; see [AT0010] and related
 * in-app tests.
 *
 * ## Gating
 *
...
describe.skipIf(!SHOULD_RUN)("m09: saved-state EM card inactive-at-mount activates via dispatch-activated", () => {
  test("gallery-prompt-entry (TugPromptEntry, session-card's editor): dispatch-activated path fires + restores text", async () => {
