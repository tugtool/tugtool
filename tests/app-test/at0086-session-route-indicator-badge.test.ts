/**
 * at0086-session-route-indicator-badge.test.ts — `SessionRouteIndicatorBadge`
...
describe.skipIf(!SHOULD_RUN)(
  "AT0086: SessionRouteIndicatorBadge repaints on route flip with mount identity",
  () => {
    test(
      "Code → Shell → Code: text follows the route; badge DOM node identity is preserved",
      async () => {
        const app = await launchTugApp({
          testName: "at0086-session-route-indicator-badge",
        });
        try {
          ...
          if (tail !== "") {
            process.stderr.write(
              `\n[at0086-session-route-indicator-badge] log tail:\n${tail}\n`,
            );
          }
          throw err;
        } finally {
