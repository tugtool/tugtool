 ...
 * What has to hold:
 *
 *  1. It engages. Picking an arrangement whose exact solve is within ±10% of
 *     the user's preferred width lands the Lens on that solve, and every seam
 *     in the chain on `IMPOSITION_GAP_PX`. The Lens's crossing is a FLIP settle
 *     like any other arrangement change, not a cut.
 ...
 *  3. It declines. A solve outside the flex range is not applied at all, not
 *     applied partway: a half-corrected gap reads as nothing, while the classic
 *     even spread reads as intentional.
 *  4. Re-asserting the layout re-tunes. Re-clicking the already-active Cards
 *     option used to be a total no-op; it is now the user's way of asking for
 *     the seams back, and it drives the same entry the settled-resize observer
 *     calls. The harness cannot resize the app's window, so this gesture is
 *     what covers that entry end to end.
 *
...
describe.skipIf(!SHOULD_RUN)(
  ...
  () => {
    ...
    test(
      "a solve outside the flex range is declined outright",
      async () => {
        const app = await launchTugApp({
        ...
        try {
          ...
          // 260px above preferred is far outside ±10%, so the allocator wants
          // a width it may not have — and takes none of it.
          const canvas = await canvasWidth(app);
          ...
          // Exactly the preferred width — not the solve, and not a step
          // toward it.
          expect(await frameWidth(app, "pLens")).toBeCloseTo(PREFERRED, 0);
          // And the seams are what the classic rule produces: the cards stand
    ...
    test(
      ...
      async () => {
        ...
        try {
          ...
          // the arrangement is already Five Up, so this is the state a slot
          // assignment or a card resize leaves behind — seams gone ragged and
