...
describe.skipIf(!SHOULD_RUN)("m21: drag aborted by Escape preserves focus", () => {
  test("Escape mid-drag rolls back commit and restores focus inside source card", async () => {
    const app = await launchTugApp({ testName: "m21-drag-aborted" });
    try {
      ...
      if (tail !== "") {
        process.stderr.write(
          `\n[m21-drag-aborted] Tug.app log tail (last 200 lines):\n${tail}\n`,
        );
      }
      const tracePath = await app.dumpTraceToFile(
        "logs/m21-drag-aborted-trace.json",
      );
      if (tracePath !== null) {
        process.stderr.write(`[m21-drag-aborted] trace dumped to ${tracePath}\n`);
      }
      throw err;
    } finally {
      await app.close();
