...
describe.skipIf(!SHOULD_RUN)("m07: card detach preserves focus + value", () => {
  test("drag A from P1 to canvas void; A lands in a new pane with value preserved", async () => {
    const app = await launchTugApp({ testName: "m07-card-detach" });
    try {
      ...
      if (tail !== "") {
        process.stderr.write(
          `\n[m07-card-detach] Tug.app log tail (last 200 lines):\n${tail}\n`,
        );
      }
      const tracePath = await app.dumpTraceToFile(
        "logs/m07-card-detach-trace.json",
      );
      if (tracePath !== null) {
        process.stderr.write(`[m07-card-detach] trace dumped to ${tracePath}\n`);
      }
      throw err;
    } finally {
      await app.close();
