...
describe.skipIf(!SHOULD_RUN)("m04: app resign → return-to-active preserves focus + value", () => {
  test("simulateAppResign + simulateAppBecomeActive restores focus inside first-responder card", async () => {
    const app = await launchTugApp({ testName: "m04-app-resign-return" });
    try {
      ...
      if (tail !== "") {
        process.stderr.write(
          `\n[m04-app-resign-return] Tug.app log tail (last 200 lines):\n${tail}\n`,
        );
      }
      const tracePath = await app.dumpTraceToFile(
        "logs/m04-app-resign-return-trace.json",
      );
      if (tracePath !== null) {
        process.stderr.write(`[m04-app-resign-return] trace dumped to ${tracePath}\n`);
      }
      throw err;
    } finally {
      await app.close();
