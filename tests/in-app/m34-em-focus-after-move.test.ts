...
describe.skipIf(!SHOULD_RUN)("m34-em: focus actually lands on engine's contenteditable after cross-pane move + detach", () => {
  test("cross-pane (gallery-prompt-input): focus lands in P2's contenteditable", async () => {
    const app = await launchTugApp({ testName: "m34-em-cross-input" });
    try {
  ...
  test("cross-pane (gallery-prompt-entry): focus lands in P2's contenteditable", async () => {
    const app = await launchTugApp({ testName: "m34-em-cross-entry" });
    try {
  ...
  test("detach (gallery-prompt-input): focus lands in new standalone pane's contenteditable", async () => {
    const app = await launchTugApp({ testName: "m34-em-detach-input" });
    try {
  ...
  test("detach (gallery-prompt-entry): focus lands in new standalone pane's contenteditable", async () => {
    const app = await launchTugApp({ testName: "m34-em-detach-entry" });
    try {
