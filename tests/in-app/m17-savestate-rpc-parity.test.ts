/**
 * m17-savestate-rpc-parity.test.ts — `window.tugdeck.saveState()`
...
describe.skipIf(!SHOULD_RUN)("m17: saveState RPC captures same axes as will-phase saves", () => {
  test("saveState() bag === window-blur bag for steady state", async () => {
    const app = await launchTugApp({ testName: "m17-savestate-rpc-parity" });
    try {
      ...
      // Any axis present in one and not the other is the [M17]
      // gap.
      ...
      if (tail !== "") {
        process.stderr.write(`\n[m17-savestate-rpc-parity] log tail:\n${tail}\n`);
      }
      throw err;
    } finally {
      await app.close();
