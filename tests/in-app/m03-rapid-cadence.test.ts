...
describe.skipIf(!SHOULD_RUN)(
  "m03-rapid-cadence: four back-to-back pane-title clicks preserve caret",
  () => {
    test("type 'hello' in A1, rapid p2→p1→p2→p1, A1 focused with caret at 5", async () => {
      const app = await launchTugApp({ testName: "m03-rapid-cadence" });
      try {
        ...
        if (tail !== "") {
          process.stderr.write(
            `\n[m03-rapid-cadence] Tug.app log tail (last 200 lines):\n${tail}\n`,
          );
        }
        const tracePath = await app.dumpTraceToFile(
          "logs/m03-rapid-cadence-trace.json",
        );
        if (tracePath !== null) {
          process.stderr.write(`[m03-rapid-cadence] trace dumped to ${tracePath}\n`);
        }
        throw err;
      } finally {
        await app.close();
