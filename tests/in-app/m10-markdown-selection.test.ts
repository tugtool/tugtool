/**
 * m10-markdown-selection.test.ts — DOM selection on a
...
describe.skipIf(!SHOULD_RUN)("m10: markdown-view DOM selection round-trips through tab-switch + cmd-tab", () => {
  test("selection survives A→B→cmd-tab→A and paints in inactive-selection while dimmed", async () => {
    const app = await launchTugApp({ testName: "m10-markdown-selection" });
    try {
      ...
      // gesture. Same pattern as m14 / m35-em — without an explicit
      ...
      if (tail !== "") {
        process.stderr.write(`\n[m10-markdown-selection] log tail:\n${tail}\n`);
      }
      throw err;
    } finally {
      await app.close();
