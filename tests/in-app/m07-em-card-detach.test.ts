...
describe.skipIf(!SHOULD_RUN)("m07-em: EM card detach to new standalone pane preserves engine state + dispatches onCardActivated", () => {
  test("gallery-prompt-input (TugPromptInput): drag A out of P1 to canvas void", async () => {
    const app = await launchTugApp({ testName: "m07-em-detach-input" });
    try {
  ...
  test("gallery-prompt-entry (TugPromptEntry, tide-card's editor): drag A out of P1 to canvas void", async () => {
    const app = await launchTugApp({ testName: "m07-em-detach-entry" });
    try {
