/**
 * m20-overlay-focus-return.test.ts — context-menu dismiss returns
...
describe.skipIf(!SHOULD_RUN)("m20: overlay dismiss returns focus to previously-focused card", () => {
  test("right-click context menu + Escape returns focus to editor", async () => {
    const app = await launchTugApp({ testName: "m20-overlay-focus-return" });
    try {
      ...
      if (tail !== "") {
        process.stderr.write(`\n[m20-overlay-focus-return] log tail:\n${tail}\n`);
      }
      throw err;
    } finally {
      await app.close();
