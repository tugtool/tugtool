...
- **The orphaned-tmux-session trap.** The tugcast test harness spawns a real tmux session per probe and sometimes leaves them behind on crashes. If you see `tmux: failed to create session` / `fork failed: Device not configured` errors during a capture or drift run, run `tmux list-sessions -F '#S' | grep '^tug-test-' | xargs -I {} tmux kill-session -t {}` and retry. This is a known footgun — §T0.5 P1 tracks the underlying reaping bug.
