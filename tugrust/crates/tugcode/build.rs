...
//!
//! Also parses palette-engine.ts and tug-color-canonical.json to extract TugColor
...
fn main() {
    ...
    // Generate TugColor palette data from palette-engine.ts
    generate_color_palette_data();
...
// ---------------------------------------------------------------------------
// TugColor palette data generation
// ---------------------------------------------------------------------------
...
/// Parse tug-color-canonical.json to extract DEFAULT_CANONICAL_L entries.
///
...
fn parse_canonical_json(path: &Path) -> Vec<(String, f64)> {
    let content = match fs::read_to_string(path) {
        ...
        Err(_) => {
            eprintln!(
                "cargo::warning=tug-color-canonical.json not found, canonical_l will be empty"
            );
            return vec![];
        }
    };
    ...
    let root: serde_json::Value = match serde_json::from_str(&content) {
        Ok(v) => v,
        Err(e) => {
            eprintln!("cargo::warning=failed to parse tug-color-canonical.json: {e}");
            return vec![];
        }
    };
...
fn generate_color_palette_data() {
    ...
    let content = match fs::read_to_string(palette_path) {
        ...
        Err(_) => {
            ...
            eprintln!("cargo::warning=palette-engine.ts not found, generating empty TugColor data");
            write_empty_color_data();
            return;
        }
    };
    ...
    // DEFAULT_CANONICAL_L is derived from tug-color-canonical.json (the single
    ...
    let dest_path = Path::new(&out_dir).join("color_palette_data.rs");
    let mut f = fs::File::create(dest_path).unwrap();

    writeln!(
        f,
        "// Auto-generated from palette-engine.ts and tug-color-canonical.json by build.rs"
    )
    .unwrap();
