...
**Purpose:** Expand the theme derivation engine's contrast pipeline to use perceptual lightness contrast (Lc, SA98G-based) as the normative threshold standard -- covering non-text UI component visibility, focus indicator contrast, semi-transparent token compositing, and cascade-aware auto-adjustment -- so that every generated theme ships with verifiable conformance across all contrast categories, with WCAG 2.x ratio retained as informational secondary data.
...
The derivation engine currently validates contrast only for text-on-background pairs (189 entries in FG_BG_PAIRING_MAP) and uses WCAG 2.x relative luminance ratio as the normative pass/fail gate. The engine also computes perceptual lightness contrast (Lc, SA98G-based) but treats it as informational only. This plan flips that relationship: Lc becomes the normative threshold standard, and WCAG ratio is retained as informational secondary data displayed in the contrast dashboard.
...
- Phase 1: Rename symbols to reflect perceptual contrast as normative. Rename `computeApcaLc` to `computeLcContrast`, rename `apcaLc` field to `lc` in ContrastResult, add `lcPass` field as the normative pass/fail gate, demote `wcagPass` to informational. Rename `APCA_LC_THRESHOLDS` to `LC_THRESHOLDS` and update the large-text threshold from 45 to 60. Rename the fg/bg interface to element/surface across all consumers.
- Phase 2: Refactor autoAdjustContrast to be cascade-aware with convergence detection, driven by `lcPass` instead of `wcagPass`. Remove the 3-iteration cap, add oscillation detection, allow bumping any token type. This must be in place BEFORE new pairs are added so cascade effects are handled correctly from the start.
...
- Phases are ordered so the normative Lc flip and cascade-aware adjuster (Phases 1-2) are operational before new non-text pairs (Phase 3), compositing (Phase 4), and focus pairs (Phase 5) are added.
...
- T4.1 test gate passes with zero unexpected failures after autoAdjustContrast using Lc thresholds, including all new non-text, composited, and focus indicator pairs (verified by `cd tugdeck && bun test`)
- All non-text UI component tokens (toggle tracks, checkbox borders, input borders, badge borders) achieve Lc 30 against their parent surfaces in the Brio theme for both dark and light presets
- Focus indicator token (accent-cool-default) achieves Lc 30 against all nine target surfaces (including surface-screen) in both modes
...
- `lcPass` is the normative pass/fail gate for all contrast checks; `wcagPass` is computed and displayed but does not gate pipeline pass/fail

#### Scope {#scope}

1. Flip normative standard from WCAG ratio to Lc: rename `computeApcaLc` to `computeLcContrast`, rename `apcaLc` to `lc`, add `lcPass` as normative gate, demote `wcagPass` to informational
2. Rename `APCA_LC_THRESHOLDS` to `LC_THRESHOLDS`; update large-text threshold from Lc 45 to Lc 60
3. Expand ELEMENT_SURFACE_PAIRING_MAP with ~33 non-text component-visibility pairs plus ~11 focus indicator pairs
4. Refactor autoAdjustContrast for cascade-aware convergence with oscillation detection and any-token-type bumping, driven by `lcPass`
...
- Importing external perceptual contrast packages -- the existing SA98G-based implementation is used as-is
...
- Current `computeApcaLc` implementation in theme-accessibility.ts (to be renamed; algorithm unchanged)
...
- Lc (SA98G-based lightness contrast) is normative for all threshold comparisons; WCAG 2.x ratio is retained as informational secondary data
- Signed Lc polarity is handled by taking |Lc| for threshold comparison
...
- Do not import external perceptual contrast packages; use existing own SA98G-based implementation
...
- The Lc 60 large-text threshold is intentionally stricter than the previous informational Lc 45 -- this is a deliberate quality bar increase, not a typo. KNOWN_BELOW_THRESHOLD must be audited in Step 1 for any large-text pairs that passed at Lc 45 but fail at Lc 60.
...
| Large-text Lc 60 threshold causes regressions | med | med | Audit KNOWN_BELOW_THRESHOLD in Step 1 for large-text pairs that passed at Lc 45 but fail at Lc 60 | New unexpected failures in existing large-text pairs |
...
- **Residual risk:** Some extreme recipe inputs could produce genuinely unfixable pairs where the hue/chroma constraints make the Lc threshold impossible. These are reported as unfixable and documented in KNOWN_BELOW_THRESHOLD.
...
**Risk R03: Large-text Lc 60 threshold regression** {#r03-large-text-threshold}

- **Risk:** Raising the large-text threshold from Lc 45 to Lc 60 may cause existing large-text pairs that previously passed informally to now fail under the normative gate.
- **Mitigation:**
  - Audit all large-text pairs in Step 1 against Lc 60 before any other changes
...
**Decision:** Replace the MAX_ITERATIONS=3 cap with convergence detection: stop when no improvements are made in a full pass, or after a safety cap of 20 iterations. After each adjustment, re-validate ALL pairs by delegating to `validateThemeContrast(resolved, pairingMap)` (not inline contrast computation) to catch cascade breakage. Track per-token adjustment history for oscillation detection. The auto-adjuster is driven by `lcPass` (the normative gate), not `wcagPass`.
...
- Using `lcPass` aligns the adjuster with the normative standard
...
**Decision:** Allow autoAdjustContrast to bump any token in the `element` position -- fg, bg, or border tokens. Only the element-position token is bumped; surface-position tokens are never adjusted. The adjuster determines bump direction using the Lc sign from `computeLcContrast(elementHex, surfaceHex)`: positive Lc means the element is darker than the surface (dark-on-light), so bump the element darker to increase |Lc|; negative Lc means the element is lighter (light-on-dark), so bump the element lighter. If element-only bumping causes oscillation, the pair is frozen and reported as unfixable.
...
- Compositing in linear sRGB before converting to hex for Lc measurement is the standard approach
...
**Decision:** Add focus indicator pairs checking accent-cool-default against all nine surfaces it can appear on (bg-app, surface-default, surface-raised, surface-inset, surface-content, surface-overlay, surface-sunken, surface-screen, field-bg-rest). Also add focused-vs-unfocused state comparison pairs for key components as informational-only (role `decorative`) since SA98G is designed for element-on-area contrast, not border-vs-border comparison.
...
- Focused-vs-unfocused pairs (accent-cool-default vs field-border-rest) compare two thin border elements, not an element against a broad area. The SA98G lightness contrast model is designed for element-on-area measurement, making border-vs-border results unreliable for gating. These pairs are kept as `decorative` role (informational, no minimum) so they appear in the dashboard for visual review without driving auto-adjustment.

**Implications:**
- ~11-13 new pairs added to the pairing map: ~10 with `ui-component` role (Lc 30 threshold) for accent-cool-default against surfaces, ~2 with `decorative` role for focused-vs-unfocused comparisons
...
#### [D06] Lc as normative threshold, WCAG ratio as informational (DECIDED) {#d06-lc-normative}

**Decision:** Perceptual lightness contrast (Lc, computed by the existing SA98G-based `computeLcContrast` function) becomes the normative pass/fail gate for all contrast checks. WCAG 2.x relative luminance ratio is retained in `ContrastResult` as the informational `wcagRatio` field and displayed in the contrast dashboard as secondary data. The `lcPass` field (boolean, `|lc| >= LC_THRESHOLDS[role]`) replaces `wcagPass` as the field that gates pipeline pass/fail. The `wcagPass` field is removed from ContrastResult entirely -- it served no purpose beyond the normative gate which is now `lcPass`.

**Rationale:**
- Perceptual lightness contrast correlates better with human perception of contrast than WCAG 2.x relative luminance ratio
- The engine already computes Lc via its own SA98G-based implementation; this change makes the existing computation authoritative rather than advisory
...
- `computeApcaLc` renamed to `computeLcContrast` (algorithm unchanged, only naming)
- `APCA_LC_THRESHOLDS` renamed to `LC_THRESHOLDS`; large-text threshold updated from 45 to 60
...
- All test assertions that checked `wcagPass` updated to check `lcPass`
- The contrast dashboard continues to display both `wcagRatio` and `lc`; badge variant logic uses `lcPass`
- `WCAG_CONTRAST_THRESHOLDS` remains exported and available for informational display in the dashboard
- The large-text Lc 60 threshold is intentionally stricter than the previous 45; KNOWN_BELOW_THRESHOLD is audited for regressions
...
6. Measure Lc between the (possibly composited) element hex and the (possibly composited) surface hex using `computeLcContrast`.
...
export interface ContrastResult {
  ...
  lc: number;              // Perceptual lightness contrast, signed (SA98G-based)
...
Note: the large-text threshold of Lc 60 is intentionally stricter than the previous informational Lc 45. This is a deliberate quality bar increase. Any large-text pairs that passed at Lc 45 but fail at Lc 60 must be audited in Step 1 and either fixed or explicitly documented in KNOWN_BELOW_THRESHOLD.
...
| Pass/fail gate | `wcagPass` (WCAG ratio) | `lcPass` (Lc threshold) | Normative standard is now Lc |

**Table T03: Symbol renames** {#t03-symbol-renames}

| Old symbol | New symbol | Location | Notes |
|------------|-----------|----------|-------|
| `computeApcaLc` | `computeLcContrast` | theme-accessibility.ts | Algorithm unchanged, only naming |
| `APCA_LC_THRESHOLDS` | `LC_THRESHOLDS` | theme-accessibility.ts | large-text updated 45 -> 60 |
| `apcaLc` (field) | `lc` (field) | ContrastResult interface | In theme-derivation-engine.ts |
| `wcagPass` (field) | removed | ContrastResult interface | Replaced by `lcPass` |
| N/A (new) | `lcPass` (field) | ContrastResult interface | Normative gate: \|lc\| >= threshold |
...
| `computeLcContrast` | function (renamed) | theme-accessibility.ts | Renamed from computeApcaLc; algorithm unchanged |
| `LC_THRESHOLDS` | const (renamed) | theme-accessibility.ts | Renamed from APCA_LC_THRESHOLDS; large-text: 60 |
| `LC_MARGINAL_DELTA` | const | theme-accessibility.ts | Fixed 5 Lc units for marginal badge classification |
...
| `validateThemeContrast` | function (modified) | theme-accessibility.ts | Accepts ElementSurfacePairing[]; computes `lcPass` as normative gate; handles parentSurface compositing |
| `autoAdjustContrast(tokens, resolved, failures, pairingMap)` | function (modified) | theme-accessibility.ts | New 4th param; cascade-aware with convergence, oscillation detection, any-token bumping; driven by `lcPass` |
| `ContrastResult` | interface (modified) | theme-derivation-engine.ts | `apcaLc` -> `lc`, `wcagPass` removed, `lcPass` added |
...
|----------|---------|-------------|
| **Unit** | Test compositing math, oscillation detection, convergence, Lc threshold comparison | compositeOverSurface, autoAdjustContrast internals, lcPass computation |
...
#### Step 1: Rename symbols and flip normative standard to Lc {#step-1}

**Commit:** `refactor(contrast): rename to perceptual contrast (Lc normative), element/surface pairing interface`

**References:** [D01] Rename fg/bg to element/surface, [D06] Lc as normative threshold, Spec S01, Spec S04, Spec S05, Table T03, Risk R03, (#d01-element-surface-rename, #d06-lc-normative, #s01-element-surface-pairing, #s04-contrast-result, #s05-lc-thresholds, #t03-symbol-renames, #r03-large-text-threshold)
...
- Renamed `computeApcaLc` -> `computeLcContrast` in theme-accessibility.ts
- Renamed `APCA_LC_THRESHOLDS` -> `LC_THRESHOLDS` with large-text updated 45 -> 60
- Updated `ContrastResult`: `apcaLc` -> `lc`, `wcagPass` removed, `lcPass` added
...
- Audited KNOWN_BELOW_THRESHOLD for large-text pairs affected by Lc 45 -> 60 change
...
- [ ] Rename `computeApcaLc` to `computeLcContrast` in theme-accessibility.ts (algorithm unchanged, only naming)
- [ ] Rename `APCA_LC_THRESHOLDS` to `LC_THRESHOLDS`; update large-text threshold from 45 to 60
- [ ] Update `ContrastResult` in theme-derivation-engine.ts: rename `apcaLc` field to `lc`, remove `wcagPass` field, add `lcPass` boolean field
- [ ] Update `validateThemeContrast` in theme-accessibility.ts:
  ...
  - Remove `wcagPass` from the result object; add `lcPass`
  - Keep `wcagRatio` computation (informational)
- [ ] Update `PairingEntry` type alias re-export
- [ ] Update source consumer: `gallery-theme-generator-content.tsx` -- imports, `APCA_LC_THRESHOLDS` -> `LC_THRESHOLDS`, `result.apcaLc` -> `result.lc`, rename `apcaLabel` function to `lcLabel`, update to use `result.lc`
- [ ] Update `badgeVariant` function in `gallery-theme-generator-content.tsx`: replace `result.wcagPass` with `result.lcPass`; update marginal check to use `Math.abs(result.lc) >= LC_THRESHOLDS[role] - LC_MARGINAL_DELTA` (fixed 5 Lc units near-pass band, per Spec S04) instead of `wcagRatio` distance from `WCAG_CONTRAST_THRESHOLDS[role]`
- [ ] Update `passCount` filter in `ContrastSummaryBar` from `r.wcagPass` to `r.lcPass`
- [ ] Update summary bar text from "pairs pass WCAG AA" to "pairs pass Lc contrast"
- [ ] Update column header and tooltip text to "Lc" in the contrast dashboard table (remove any prior branding)
- [ ] Update CSS comment in `gallery-theme-generator-content.css` (line 358: `/* Summary bar — "N/M pairs pass WCAG AA" */`) to reflect Lc normative wording
- [ ] Update all test files for renamed fields: `theme-accessibility.test.ts`, `theme-derivation-engine.test.ts`, `gallery-theme-generator-content.test.tsx`, `contrast-dashboard.test.tsx`, `cvd-preview-auto-fix.test.tsx` -- all `r.wcagPass` -> `r.lcPass`, all `r.apcaLc` -> `r.lc`, all `computeApcaLc` -> `computeLcContrast`
- [ ] Update `contrast-dashboard.test.tsx` assertions: passCount/failCount filters from `r.wcagPass` to `r.lcPass`; update any assertion text matching "WCAG AA" to "Lc contrast"
...
**Tests:**
- [ ] All existing contrast tests pass with renamed interface and Lc normative gate
- [ ] T3.3 test updated: `computeLcContrast` (was `computeApcaLc`)
- [ ] T4.1 passes with `lcPass` as the gate (may have new failures from Lc 60 large-text threshold -- these are expected and handled by KNOWN_BELOW_THRESHOLD audit above)

**Checkpoint:**
- [ ] `cd tugdeck && bun test`
- [ ] Verify no references to `FgBgPairing`, `FG_BG_PAIRING_MAP`, `computeApcaLc`, `APCA_LC_THRESHOLDS`, or `apcaLc` remain in source (grep confirms zero matches)
...
**References:** [D02] Cascade-aware auto-adjust, [D03] Any-token-type bumping, [D06] Lc normative, Spec S03, Spec S04, Table T02, Risk R01, (#d02-cascade-auto-adjust, #d03-any-token-bump, #d06-lc-normative, #s03-oscillation-detection, #s04-contrast-result, #t02-auto-adjust-params, #r01-oscillation)
...
- [ ] Filter remaining failures as `results.filter(r => !r.lcPass)` -- the normative gate is `lcPass` per [D06]
...
- [ ] Change "group by element" logic to bump the element token regardless of whether it is fg, bg, or border per [D03]. Determine bump direction using Lc sign: call `computeLcContrast(elementHex, surfaceHex)` -- positive Lc (dark-on-light) means bump element darker (decrease tone); negative Lc (light-on-dark) means bump element lighter (increase tone). This replaces the raw OKLCH L comparison for correctness under SA98G polarity semantics.
- [ ] Update unfixable reporting to use element token names

**Tests:**
- [ ] T3.4 (auto-adjust fixes a deliberately failing pair) updated to use element/surface naming and `lcPass`
...
**Checkpoint:**
- [ ] `cd tugdeck && bun test`
- [ ] Verify T4.1 still passes with 0 unexpected failures (existing 189 pairs with new adjuster, Lc normative)
...
**References:** [D01] Element/surface naming, [D02] Cascade-aware auto-adjust, [D06] Lc normative, Table T01, (#t01-non-text-categories, #context, #strategy)
...
- [ ] For any failing pairs (where `lcPass` is false), evaluate whether the formula in theme-derivation-engine.ts needs a tone adjustment or the pair should be added to KNOWN_BELOW_THRESHOLD with documented rationale
...
**References:** [D04] Alpha-composite resolution, [D06] Lc normative, Spec S02, (#d04-alpha-composite, #d06-lc-normative, #s02-compositing)
...
- [ ] Note on bump-direction with composited pairs: the auto-adjuster determines bump direction using the Lc sign from `computeLcContrast` (set up in Step 2). For composited pairs, `validateThemeContrast` now composites before measuring, so the Lc sign reflects the composited contrast. However, the bump itself adjusts the raw element tone -- this is correct because increasing |Lc| always means moving the element away from the surface in lightness. No changes to bump-direction logic are needed in this step.
...
- [ ] Verify that compositing produces accurate contrast measurements by spot-checking badge-tinted-accent-bg (alpha 15%) composited over surface-default, then measuring badge-tinted-accent-fg against the composited result using `computeLcContrast`
...
- [ ] Integration test: badge-tinted-accent-fg on composited badge-tinted-accent-bg passes Lc 30 (ui-component role)
...
**References:** [D05] Focus indicator validation, [D06] Lc normative, Table T01, (#d05-focus-indicator, #d06-lc-normative, #t01-non-text-categories)
...
- [ ] Add focused-vs-unfocused pairs as informational-only: accent-cool-default as element against field-border-rest as surface, with role `decorative` (SA98G is designed for element-on-area contrast, not border-vs-border -- see [D05])
...
- [ ] Run pipeline and verify all focus-on-surface pairs pass Lc 30 (ui-component threshold); focused-vs-unfocused pairs are decorative and do not gate
...
**Tests:**
- [ ] All focus-on-surface indicator pairs pass Lc 30 in T4.1
...
**References:** [D01] Element/surface naming, [D02] Cascade-aware auto-adjust, [D04] Alpha-composite, [D05] Focus indicators, [D06] Lc normative, Table T01, Table T02, (#success-criteria, #exit-criteria)
...
- [ ] Verify T4.1 passes with 0 unexpected failures for both dark and light presets, using `lcPass` as the gate
- [ ] Run full test suite for both Brio dark preset and Brio light preset

**Tests:**
- [ ] T4.1 end-to-end: 0 unexpected failures after autoAdjustContrast for all pair categories (Lc normative)
...
**Deliverable:** The theme derivation engine validates all contrast requirements using perceptual lightness contrast (Lc) as the normative standard -- non-text UI component visibility, focus indicator contrast, and semi-transparent token compositing -- with a cascade-aware auto-adjust loop that converges deterministically. WCAG 2.x ratio is retained as informational secondary data.
...
- [ ] `lcPass` is the normative pass/fail gate; `wcagPass` is removed; `wcagRatio` retained as informational
...
**Acceptance tests:**
- [ ] T4.1: zero unexpected failures after autoAdjustContrast (all pair categories, Lc normative)
- [ ] T-BRIO-MATCH: engine output matches updated ground truth
...
- [ ] Per-recipe Lc threshold overrides (recipe-level quality bar customization)
