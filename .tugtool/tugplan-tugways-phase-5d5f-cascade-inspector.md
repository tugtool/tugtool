...
**Purpose:** Ship a dev-mode `Ctrl+Option + hover` cascade inspector overlay that shows the full token resolution chain (component tokens, base tokens, palette variables, TugColor provenance) and scale/timing readout for any inspected element, making the tugways style system navigable instead of opaque.
...
- TugColor palette provenance displays hue family name, preset name, and coordinates (intensity, value, canonical L) for any palette-derived color.
...
1. `StyleInspectorOverlay` singleton -- modifier key tracking, overlay lifecycle, target identification, token chain resolution, TugColor provenance extraction, scale/timing readout, pin/unpin, Escape to close.
...
- Phase 5d5e (Palette Engine Integration) complete -- `tug-palette.css` with pure CSS TugColor formulas and `tug-tokens.css` chromatic tokens resolving via `var(--tug-{hue}[-preset])`.
...
- Phase 5d5e is complete: `tug-palette.css` exists with pure CSS TugColor formulas and `tug-tokens.css` chromatic tokens already resolve via `var(--tug-{hue}[-preset])` references.
...
- Chain walking terminates at three points: (1) when the current property matches the PALETTE_VAR_REGEX (one of 24 known hue family names with an optional preset suffix from `accent|muted|light|subtle|dark|deep`) -- these resolve to `oklch()` expressions with nested `var()` calls to internal palette constants, which the TugColor provenance display handles; (2) when the value starts with `oklch(` (formula terminal); (3) when the value does not contain a `var()` reference (literal hex, rgb, length, etc.).
...
| TugColor provenance | The hue family name, preset name, and intensity/tone/canonical-L coordinates for a palette-derived color |
...
resolveTokenChain(element, startProperty):
  ...
  loop:
    ...
    if currentProp matches PALETTE_VAR_REGEX (see below)
      break  // palette variable reached -- TugColor provenance handles inner constants
...
**Spec S04: TugColor Provenance Display** {#s04-tug-color-provenance}
...
| File | Purpose |
|------|---------|
| `tugdeck/src/components/tugways/style-inspector-overlay.ts` | StyleInspectorOverlay singleton -- modifier key tracking, overlay lifecycle, token chain resolution, TugColor provenance, scale/timing readout |
...
| `extractHvvProvenance` | method | `StyleInspectorOverlay` | Parses hue/preset from palette token names and reads TugColor constants |
...
|----------|---------|-------------|
| **Unit** | Test token chain resolution logic, TugColor provenance extraction, modifier key state tracking | Core inspector logic in isolation |
...
#### Step 2: Token chain resolution and TugColor provenance {#step-2}

**Depends on:** #step-1

**Commit:** `feat(tugways): add token chain resolution and TugColor provenance to cascade inspector`

**References:** [D04] Textual var() chain walking, Spec S02, Spec S04, Risk R01, (#s02-token-chain-algorithm, #s04-tug-color-provenance, #r01-var-chain-fidelity)
...
- [ ] Display token chain in the panel. For a three-layer component (e.g., TugTabBar): `--tug-comp-tab-bar-bg -> --tug-base-tab-bar-bg -> #23262d`. For a two-layer component (e.g., TugButton, which references `--tug-base-*` directly): `--tug-base-accent-cool-default -> --tug-cobalt-accent` with a "(no comp token)" indicator (chain terminates at the palette variable; TugColor provenance shows cobalt/accent details). Show each hop with an arrow separator.
- [ ] Display TugColor provenance when chain terminates at a palette variable: hue family, preset, canonical L, peak C, hue angle per Spec S04.
...
- [ ] Add inspectable sample elements that exercise all token chain depths: (a) a TugDropdown instance (exercises full three-layer chain: `--tug-comp-dropdown-*` -> `--tug-base-*` -> palette), (b) a TugButton (exercises two-layer chain: `--tug-base-*` -> palette, with "(no comp token)" indicator -- tug-button.css references `--tug-base-*` directly), (c) a colored div using `--tug-base-accent-default` (exercises `--tug-base-*` -> palette chain), (d) a div using `--tug-base-surface-raised` (exercises non-chromatic base token, terminal hex value), and (e) a div with explicit inline `background: var(--tug-orange-light)` (exercises direct palette reference with TugColor provenance).
...
- [ ] Verify two-layer chain resolution: hover a TugButton, see `--tug-base-accent-cool-default -> --tug-cobalt-accent` with "(no comp token)" indicator and TugColor provenance for cobalt/accent (TugButton CSS references `--tug-base-*` directly, not `--tug-comp-button-*`).
- [ ] Verify TugColor provenance displays hue, preset, canonical L, peak C, hue angle for palette-derived colors.
...
**Deliverable:** A dev-mode cascade inspector overlay activated by Ctrl+Option+hover that shows full token resolution chains (--tug-comp-* through --tug-base-* to palette variables), TugColor palette provenance, and scale/timing readout for any hovered element, with an 11th gallery tab demonstrating the inspector.
...
- [ ] TugColor provenance shows hue family, preset, and coordinates (verified on palette-colored element).
...
| TugColor provenance | Hue, preset, coordinates displayed for palette colors |
