...
## CITA PostCSS Plugin and Theme Conversion {#cita-postcss}

**Purpose:** Ship a PostCSS plugin that expands `--cita(hue, i, t)` notation to concrete `oklch()` values at build time, add an `oklchToCITA()` reverse mapper to palette-engine.ts, and convert all hardcoded hex tokens in theme files to `--cita()` calls.
...
The Tugways design system uses an CITA (Hue, Intensity, Tone) color model backed by OKLCH. The `citaColor()` function in `palette-engine.ts` already computes `oklch()` strings from CITA parameters at runtime. However, the three theme files (tug-tokens.css, bluenote.css, harmony.css) still contain hundreds of hardcoded hex color values for non-chromatic tokens (surfaces, grays, borders, text). These hex values were originally hand-tuned but are expressible as CITA coordinates with specific hue tints and intensity/tone settings.

Converting hex tokens to `--cita()` notation makes the color system fully parametric: every color in every theme is described by hue family, intensity, and tone. This enables future features like theme generation, accessibility tooling, and palette-wide adjustments. The conversion must happen at build time (not runtime) so there is zero performance cost.

#### Strategy {#strategy}

- Build the PostCSS plugin first as a standalone module with its own tests, independent of theme conversion.
- Add the `oklchToCITA()` reverse mapper to palette-engine.ts to enable programmatic hex-to-CITA derivation.
- Write a one-time conversion script that uses `oklchToCITA()` to derive CITA parameters for every hex token, producing the rewritten theme files.
...
- All Harmony contrast-critical hex overrides (formerly marked [D06] in comments) are converted to `--cita()`: `#c46020` becomes `--cita(flame, 45, 38)`, `#8a7200` becomes `--cita(yellow, 46, 27)`, `#b89000` becomes `--cita(yellow, 55, 35)`, `#2898c8` becomes `--cita(blue, 42, 40)`, `#ffe15a` becomes `--cita(yellow, 62, 58)`. All map with delta-E < 0.02.
- `bun run build` succeeds with no errors.
- The PostCSS plugin expands `--cita(blue, 5, 13)` to `oklch(0.3115 0.0143 230)` (matching citaColor() output exactly: L = 0.15 + 13 * (0.771 - 0.15) / 50 = 0.3115).
- Raw hue angles work: `--cita(237, 5, 13)` expands correctly using binary-searched max chroma at that angle.
- All existing `var()`, `color-mix()`, and `rgba()` references in theme files are preserved unchanged.
- `oklchToCITA()` round-trips: for every named hue family, `oklchToCITA(citaColor(hue, i, t))` returns the original `{hue, i, t}` within rounding tolerance.

#### Scope {#scope}

1. PostCSS plugin (`postcss-cita.ts`) that expands `--cita()` in CSS declaration values.
2. `oklchToCITA()` and `citaPretty()` functions in palette-engine.ts.
3. One-time conversion script to rewrite theme files from hex to `--cita()`.
...
- Runtime CITA evaluation in the browser — all expansion is build-time only.
- Converting `rgba()`, `color-mix()`, or `var()` expressions to `--cita()`.
...
- `palette-engine.ts` exports: `HUE_FAMILIES`, `DEFAULT_CANONICAL_L`, `MAX_CHROMA_FOR_HUE`, `PEAK_C_SCALE`, `L_DARK`, `L_LIGHT`, `citaColor()`, `findMaxChroma()`.
...
- Number precision: 4 decimal places, trailing zeros stripped (matching `citaColor()` format function).
...
- The `oklchToCITA()` reverse mapper will be added to palette-engine.ts alongside the existing `citaColor()` function.
- Raw numeric hue angles (e.g. `--cita(237, 5, 13)`) are supported by the plugin using binary search for max chroma at the raw angle.
- The plugin source file will live in `tugdeck/` alongside `vite.config.ts` and import constants from palette-engine.ts.
- brio.css remains nearly empty and does not need `--cita()` conversion since its defaults live in tug-tokens.css body{} block.
...
|------|--------|------------|------------|--------------------|
| Color drift from hex-to-CITA rounding | med | med | Validate round-trip within delta-E threshold | Visual regression detected |
...
**Risk R01: Color drift from hex-to-CITA rounding** {#r01-color-drift}

- **Risk:** Converting hex values through oklch intermediate to CITA and back may introduce perceptible color shifts.
- **Mitigation:**
...
- **Risk:** The CITA PostCSS plugin and Tailwind's CSS processing could interfere.
- **Mitigation:**
  - Tailwind v4 uses `@tailwindcss/vite` (a Vite plugin), not a PostCSS plugin. The CITA plugin goes in `css.postcss.plugins` which is a separate pipeline stage.
...
**Decision:** Write a custom PostCSS plugin that walks the AST and expands `--cita()` patterns, rather than using `postcss-functions` or similar libraries.

**Rationale:**
- Full control over the `--cita()` syntax parsing (three-argument function with named hues or raw angles).
...
**Implications:**
- Plugin file lives at `tugdeck/postcss-cita.ts`.
...
**Decision:** Use Vite's `css.postcss` option to pass the CITA plugin inline rather than using a separate `postcss.config.js`.
...
- The CITA plugin must be importable from the Vite config (same directory).

#### [D03] Keep palette var() formulas in tug-palette.css (DECIDED) {#d03-palette-unchanged}

**Decision:** `tug-palette.css` is not modified. Only theme files (tug-tokens.css, bluenote.css, harmony.css) use `--cita()`.

**Rationale:**
- The preset formulas in tug-palette.css use `var(--tug-{hue}-peak-c)` for P3 gamut overrides via cascade.
- Replacing these with `--cita()` would lose the runtime P3 override mechanism.
- Theme tokens are the only place where hardcoded hex values need conversion.

**Implications:**
- The plugin only needs to process theme CSS files (though it will harmlessly skip files without `--cita()` calls).
- P3 gamut override via `--tug-{hue}-peak-c` continues to work unmodified.

#### [D04] Re-derive CITA mappings programmatically (DECIDED) {#d04-programmatic-derivation}

**Decision:** Build `oklchToCITA()` first, then use it in a conversion script to programmatically derive CITA parameters for every hex token.
...
**Implications:**
- `oklchToCITA()` must handle the full range of hex values in theme files, including near-achromatic grays with slight hue tints.
- The conversion script parses CSS, identifies hex values, converts each, and writes back `--cita()` calls.
- `#ffffff` is special-cased to `var(--tug-white)` (appears in harmony.css only). `#000000` does not appear in any theme file.

#### [D05] Number precision matches citaColor() (DECIDED) {#d05-number-precision}

**Decision:** The PostCSS plugin outputs oklch() values with 4 decimal places and trailing zeros stripped, identical to `citaColor()`.

**Rationale:**
- Consistency with existing programmatic output.
- The `citaColor()` format function (`parseFloat(n.toFixed(4)).toString()`) is the established convention.

**Implications:**
- The plugin reuses the same formatting logic.
- Round-trip validation compares strings directly.

#### [D06] Convert all contrast-critical hex overrides to --cita() (DECIDED) {#d06-contrast-overrides}

**Decision:** All Harmony contrast-critical hex overrides (previously marked with [D06] comments referencing a prior phase decision) are converted to `--cita()` notation. None are left as raw hex.

**Rationale:**
- Prior analysis confirmed every contrast-critical hex maps cleanly to CITA with delta-E < 0.02.
- Specific verified mappings: `#c46020` (accent-muted) maps to `--cita(flame, 45, 38)` (delta-E=0.009), `#8a7200` (various warning/function tokens) maps to `--cita(yellow, 46, 27)` (delta-E=0.009), `#b89000` (toast-warning-fg) maps to `--cita(yellow, 55, 35)` (delta-E=0.003), `#2898c8` (banner-info-fg) maps to `--cita(blue, 42, 40)` (delta-E=0.004), `#ffe15a` (field-warning) maps to `--cita(yellow, 62, 58)` (delta-E=0.018).
...
- The existing [D06] comments in harmony.css will be updated to reference the CITA notation instead.
...
The plugin walks the PostCSS AST using `Declaration` visitor. For each declaration whose value contains `--cita(`, it:

1. Extracts all `--cita(arg1, arg2, arg3)` calls via regex.
2. For each call, resolves `arg1` as either a named hue (lookup in `HUE_FAMILIES`) or a raw numeric angle.
3. Computes the oklch() string using the same math as `citaColor()`:
...
4. Replaces the `--cita()` call in the value string with the computed `oklch(L C h)`.

**Spec S01: --cita() Syntax** {#s01-cita-syntax}

```
--cita( <hue> , <intensity> , <tone> )
...
Examples:
- `--cita(blue, 5, 13)` -- named hue
- `--cita(237, 5, 13)` -- raw angle
- `--cita(cobalt, 3, 18)` -- named hue, low intensity

#### oklchToCITA() Reverse Mapping Algorithm {#reverse-mapping}

Given an oklch() color string, reverse-map to the closest CITA notation:
...
`citaPretty()` formats the result as a human-readable string: `"blue i=5 t=13"` or `"hue-237 i=5 t=13"`.

#### Hex-to-CITA Conversion Pipeline {#conversion-pipeline}
...
2. Walks only `Declaration` nodes (comments are separate AST nodes and are never visited):
   - For each declaration whose value contains a standalone `#hex` (6-digit or 3-digit):
     ...
     - Run `oklchToCITA()` to get the CITA parameters.
     - Replace the hex value with `--cita(hue, i, t)`.
...
**PostCSS Plugin Input:** CSS files containing `--cita(hue, i, t)` patterns in declaration values.

**PostCSS Plugin Output:** Same CSS with `--cita()` calls replaced by `oklch(L C h)` strings.

**oklchToCITA() Input:** An `oklch(L C h)` CSS string.

**oklchToCITA() Output:** An object `{ hue: string, i: number, t: number }` where hue is a named family or `hue-NNN`.

#### Terminology and Naming {#terminology}

| Term | Definition |
|------|-----------|
| CITA | Hue, Intensity, Tone — the three-axis color system |
| `--cita()` | CSS function notation expanded at build time by the PostCSS plugin |
...
| File | Purpose |
|------|---------|
| `tugdeck/postcss-cita.ts` | PostCSS plugin that expands `--cita()` to `oklch()` |
| `tugdeck/scripts/convert-hex-to-cita.ts` | One-time conversion script for theme files |
| `tugdeck/src/__tests__/postcss-cita.test.ts` | Tests for PostCSS plugin (imports from `bun:test`) |
| `tugdeck/src/__tests__/convert-hex-to-cita.test.ts` | Tests for conversion script (imports from `bun:test`) |
...
| `citaPretty` | fn | `palette-engine.ts` | Formats CITA as human-readable string |
| `postcssHvv` | fn | `postcss-cita.ts` | PostCSS plugin factory function |
| `convertHexToHvv` | script | `scripts/convert-hex-to-cita.ts` | One-time hex conversion script |

---

### Documentation Plan {#documentation-plan}

- [ ] Update palette-engine.ts JSDoc with oklchToCITA() and citaPretty() documentation.
- [ ] Add inline comments in postcss-cita.ts explaining the plugin architecture.
- [ ] Add comments in vite.config.ts explaining the css.postcss configuration.
- [ ] Update theme file headers to reference the --cita() notation.
...
|----------|---------|-------------|
| **Unit** | Test `oklchToCITA()` round-trips, `citaPretty()` formatting, plugin regex matching | Core logic, edge cases |
...
#### Step 1: Add oklchToCITA() and citaPretty() to palette-engine.ts {#step-1}

**Commit:** `feat(palette): add oklchToCITA() reverse mapper and citaPretty() formatter`

**References:** [D04] Re-derive CITA mappings programmatically, [D05] Number precision matches citaColor(), Spec S01, (#reverse-mapping, #inputs-outputs, #terminology)

**Artifacts:**
- Modified `tugdeck/src/components/tugways/palette-engine.ts` with two new exported functions.

**Tasks:**
- [ ] Implement `oklchToCITA(oklchStr: string): { hue: string; i: number; t: number }`:
...
- [ ] Implement `citaPretty(oklchStr: string): string`:
  - Call `oklchToCITA()` and format as `"blue i=5 t=13"` or `"hue-237 i=5 t=13"`.
- [ ] Verify round-trip: for each of the 24 hue families, confirm `oklchToCITA(citaColor(hue, i, t, canonicalL))` recovers `{hue, i, t}` within tolerance for a range of i/t inputs.

**Tests:**
- [ ] Round-trip test: for all 24 hues at i=50/t=50, oklchToCITA(citaColor(...)) recovers exact hue name and i/t.
...
#### Step 2: Create PostCSS plugin (postcss-cita.ts) {#step-2}

**Depends on:** #step-1

**Commit:** `feat(build): add postcss-cita plugin for --cita() expansion`

**References:** [D01] Bespoke PostCSS plugin, [D05] Number precision, Spec S01, (#plugin-architecture, #s01-cita-syntax, #inputs-outputs)

**Artifacts:**
- New file `tugdeck/postcss-cita.ts`.
...
- [ ] Create `postcss-cita.ts` exporting a PostCSS plugin factory function.
- [ ] Implement the Declaration visitor that:
  - Detects `--cita(` in declaration values.
  - Extracts arguments via regex: `--cita\(\s*([a-z]+|\d+(?:\.\d+)?)\s*,\s*(\d+(?:\.\d+)?)\s*,\s*(\d+(?:\.\d+)?)\s*\)`.
  ...
  - Applies the same L and C formulas as `citaColor()`.
  - Formats output with 4 decimal places, trailing zeros stripped.
  - Replaces the `--cita()` call in the value with the computed `oklch(L C h)` string.
- [ ] Handle multiple `--cita()` calls in a single declaration value (replace all occurrences).
...
**Tests:**
- [ ] Unit test: `--cita(blue, 5, 13)` expands to `oklch(0.3115 0.0143 230)` (L = 0.15 + 13 * (0.771 - 0.15) / 50).
- [ ] Unit test: `--cita(237, 5, 13)` expands correctly for raw angle.
- [ ] Unit test: multiple `--cita()` calls in one value are all expanded.
- [ ] Unit test: values without `--cita()` are left unchanged.
- [ ] Unit test: `var()`, `color-mix()`, `rgba()` values pass through unmodified.

**Checkpoint:**
- [ ] `cd tugdeck && bun test src/__tests__/postcss-cita.test.ts` passes (test imports from `bun:test`).
...
**Commit:** `feat(build): wire postcss-cita plugin into vite.config.ts`
...
**Tasks:**
- [ ] Import `postcssHvv` from `./postcss-cita.ts` in vite.config.ts.
...
- [ ] Create a small test CSS file with `--cita()` calls and verify it expands correctly in dev mode.

**Tests:**
- [ ] Manual verification: add a temporary `--cita(blue, 50, 50)` declaration and check dev server output.

**Checkpoint:**
- [ ] `cd tugdeck && bun run build` succeeds with no errors.
- [ ] Dev server (`bun run dev`) processes `--cita()` declarations correctly.

---

#### Step 4: Build hex-to-CITA conversion script {#step-4}

**Depends on:** #step-1

**Commit:** `feat(scripts): add convert-hex-to-cita conversion script`

**References:** [D04] Re-derive CITA mappings programmatically, Table T01, (#conversion-pipeline, #reverse-mapping)

**Artifacts:**
- New file `tugdeck/scripts/convert-hex-to-cita.ts`.
...
- [ ] For each hex value, convert to oklch, run `oklchToCITA()`, and replace with `--cita(hue, i, t)`.
...
**Checkpoint:**
- [ ] `cd tugdeck && bun test src/__tests__/convert-hex-to-cita.test.ts` passes (test imports from `bun:test`).

---

#### Step 5: Convert tug-tokens.css hex values to --cita() {#step-5}

**Depends on:** #step-3, #step-4

**Commit:** `refactor(tokens): convert tug-tokens.css hex values to --cita() notation`
...
**Artifacts:**
- Modified `tugdeck/styles/tug-tokens.css` with all standalone hex values replaced by `--cita()` calls.
...
- [ ] Verify that all standalone `#hex` values in the body{} block are replaced with `--cita()` calls.
...
#### Step 6: Convert bluenote.css hex values to --cita() {#step-6}

**Depends on:** #step-3, #step-4

**Commit:** `refactor(tokens): convert bluenote.css hex values to --cita() notation`

**References:** [D04] Re-derive mappings, [D05] Number precision, Table T01, (#conversion-pipeline, #t01-theme-tints)

**Artifacts:**
- Modified `tugdeck/styles/bluenote.css` with all standalone hex values replaced by `--cita()` calls.
...
#### Step 7: Convert harmony.css hex values to --cita() {#step-7}

**Depends on:** #step-3, #step-4

**Commit:** `refactor(tokens): convert harmony.css hex values to --cita() notation`
...
**Artifacts:**
- Modified `tugdeck/styles/harmony.css` with all standalone hex values replaced by `--cita()` calls, including contrast-critical overrides.
...
- [ ] Verify all contrast-critical hex overrides are converted per [D06]: `#c46020` to `--cita(flame, 45, 38)`, `#8a7200` to `--cita(yellow, 46, 27)`, `#b89000` to `--cita(yellow, 55, 35)`, `#2898c8` to `--cita(blue, 42, 40)`, `#ffe15a` to `--cita(yellow, 62, 58)`.
- [ ] Update existing [D06] comments in harmony.css to reference the new CITA values instead of hex.
...
- [ ] Dev server hot-reload works: modify an `--cita()` value and confirm it updates in the browser.
- [ ] Verify `oklchToCITA()` round-trip accuracy across all converted tokens.
...
**Deliverable:** A build-time PostCSS plugin that expands `--cita()` color notation to `oklch()`, with all theme files converted from hardcoded hex to `--cita()` calls, and a reverse mapper (`oklchToCITA()`) for developer tooling.

#### Phase Exit Criteria ("Done means...") {#exit-criteria}

- [ ] `postcss-cita.ts` plugin expands all `--cita()` calls to correct `oklch()` values at build time.
- [ ] `oklchToCITA()` and `citaPretty()` are exported from palette-engine.ts and round-trip correctly.
...
- [ ] `bun run build` succeeds with no errors.
- [ ] Dev server hot-reload processes `--cita()` changes correctly.
...
- [ ] Dev overlay tool that shows CITA values on hover (using `citaPretty()`).
- [ ] Theme builder UI that uses `--cita()` notation for user-defined themes.
- [ ] Linter rule to prevent new hex values from being added to theme files.

| Checkpoint | Verification |
|------------|--------------|
| PostCSS plugin works | `--cita(blue, 50, 50)` expands to correct oklch value |
| Build succeeds | `cd tugdeck && bun run build` exits 0 |
| No hex regression | grep for standalone hex in theme files returns empty |
| Round-trip accuracy | oklchToCITA(citaColor(hue, i, t)) recovers original params |
