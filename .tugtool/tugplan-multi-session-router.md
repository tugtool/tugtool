...
- New file `tugcast/src/feeds/agent_supervisor.rs` with:
  ...
  - `AgentSupervisor` struct holding the ledger `Mutex<HashMap<TugSessionId, Arc<Mutex<LedgerEntry>>>>`, a `SESSION_STATE` broadcast sender, a `SESSION_METADATA` **broadcast** sender (`broadcast::Sender<Frame>`, not a watch — see [D14] for why), a `client_sessions: Mutex<HashMap<ClientId, HashSet<TugSessionId>>>` map for per-client session affinity (populated by `handle_control`; used by the P5 authorization cross-check in Step 7), the supervisor's `CODE_OUTPUT` broadcast sender, a tugbank handle for `dev.tugtool.tide.session-keys`, and config. `ClientId` is `u64` matching the router's existing `client_id_counter` type.
