...
1. New `tugcast/src/feeds/agent_supervisor.rs` module: supervisor, ledger, CODE_INPUT dispatcher task, CODE_OUTPUT merger task, per-session crash budget, per-session `system_metadata` routing (`SessionMetadataRegistry`), SESSION_STATE publisher, control-action handler (`handle_control`), and tugbank write-path for the card↔session map. CODE_OUTPUT replay buffer stays shared at the router level per [D06].
...
- `AgentSupervisor` gains a `replay_metadata_for_client(client_id)` method used **only** by the startup `rebind_from_tugbank` batch helper. It is **not** called from `handle_client` — doing so would be a perpetual no-op because fresh `client_id`s have empty `client_sessions` entries at post-handshake time. The per-session event-driven replay inside `handle_control` is the mechanism that services real reconnect flows.
...
| `AgentSupervisor` | struct | `tugcast/src/feeds/agent_supervisor.rs` | Top-level supervisor; owns the ledger, dispatcher task, merger task, SESSION_STATE broadcast, **SESSION_METADATA broadcast sender** (not a watch — see [D14]), `client_sessions: Mutex<HashMap<ClientId, HashSet<TugSessionId>>>`, tugbank handle. |
...
| `ControlError` | enum | `tugcast/src/feeds/agent_supervisor.rs` | `MissingCardId`, `MissingSessionId`, `Malformed`, `PersistenceFailure`. Returned from `handle_control` on payload-validation failures and — per [D12]'s asymmetric policy — on a tugbank **write** failure during `spawn_session` (strict). Tugbank **delete** failures during `close_session` are best-effort and do NOT return an error; they are logged via `warn!` and in-memory cleanup continues. The caller (`handle_client`) converts any `Err(_)` into a CONTROL error frame on the in-scope socket. |
| `SpawnState` | enum | `tugcast/src/feeds/agent_supervisor.rs` | `Idle` / `Spawning` / `Live` / `Errored` / `Closed`. |
| `SessionMetadataRegistry` | struct | `tugcast/src/feeds/agent_supervisor.rs` | Per-session `system_metadata` routing, populated by the merger task. See [D14]. |
...
| `AgentSupervisor::replay_metadata_for_client` | fn | `tugcast/src/feeds/agent_supervisor.rs` | **Batch helper used only by `rebind_from_tugbank` at startup.** Iterates `client_sessions[client_id]`, reads each session's `latest_metadata`, publishes each onto the SESSION_METADATA broadcast as a one-shot frame. **Not called from `handle_client`** — the real event-driven replay on reconnect runs inside `handle_control("spawn_session", ...)` per [D14]. |
...
| `AgentSupervisor::merger_task` | fn | `tugcast/src/feeds/agent_supervisor.rs` | Background task pulling from N per-bridge mpscs; forwards to shared CODE_OUTPUT broadcast; on `system_metadata` lines, writes to `LedgerEntry::latest_metadata` AND publishes on the SESSION_METADATA broadcast per [D14]. |
...
- New file `tugcast/src/feeds/agent_supervisor.rs` with:
  ...
  - `LedgerEntry` struct per [#s01-ledger-entry] (imports `TugSessionId` from `tugcast_core`).
  - `SessionMetadataRegistry` — a thin wrapper around the ledger exposing `get_latest_metadata(&TugSessionId) -> Option<Frame>` and `set_latest_metadata(&TugSessionId, Frame)`. Used by Step 6's per-session metadata routing and Step 8's on-subscribe replay per [D14].
  - `AgentSupervisor` struct holding the ledger `Mutex<HashMap<TugSessionId, Arc<Mutex<LedgerEntry>>>>`, a `SESSION_STATE` broadcast sender, a `SESSION_METADATA` **broadcast** sender (`broadcast::Sender<Frame>`, not a watch — see [D14] for why), a `client_sessions: Mutex<HashMap<ClientId, HashSet<TugSessionId>>>` map for per-client session affinity (populated by `handle_control` and read by `replay_metadata_for_client`), the supervisor's `CODE_OUTPUT` broadcast sender, a tugbank handle for `dev.tugtool.tide.session-keys`, and config. `ClientId` is `u64` matching the router's existing `client_id_counter` type.
  - `AgentSupervisor::new()` constructor.
  - Public method stubs (`handle_control`, `dispatcher_task`, `merger_task`, `spawn_session_worker`, `replay_metadata_for_client`, `on_client_disconnect`) returning `todo!()` or a no-op so the scaffold compiles.
...
- Unit tests covering each action path, idempotency, the tugbank write/delete behavior, and `client_sessions` bookkeeping. Use an in-memory tugbank fake for unit tests (no disk I/O).
...
- [ ] Detect `system_metadata` frames in the merger using the existing needle-scan pattern from `session_metadata.rs`. On match, call `SessionMetadataRegistry::set_latest_metadata` (which writes to `LedgerEntry::latest_metadata`) AND publish the spliced frame on the supervisor's SESSION_METADATA `broadcast::Sender<Frame>` per [D14].
- [ ] Implement `AgentSupervisor::replay_metadata_for_client(client_id)` as a **batch helper used only by `rebind_from_tugbank` at startup** (Step 8). Read `client_sessions[client_id]` under its mutex, for each `tug_session_id` read `LedgerEntry::latest_metadata` (skip `None`), and publish each frame onto the SESSION_METADATA broadcast as a one-shot. Return silently if `client_sessions` has no entry for the client. **Do NOT call this from `handle_client` post-handshake setup** — doing so would be a perpetual no-op because a fresh WebSocket mints a new `client_id` that has no `client_sessions` entry until the subsequent `spawn_session` CONTROL frame arrives. Real reconnect replay runs event-driven inside `handle_control("spawn_session", ...)` per [D14]. Step 8's Step 8 tasks will **not** add a post-handshake call to this function.
...
- [ ] `test_replay_metadata_for_client_batch_helper_reemits_each_session` — exercises the batch helper used by `rebind_from_tugbank` (not the `handle_client` path). Register `client_sessions[ClientId::MAX] = {id_a, id_b}` (the synthetic startup sentinel — see Step 8's rebind path), set `latest_metadata` for both sessions to distinct payloads, subscribe a broadcast receiver, call `replay_metadata_for_client(ClientId::MAX)`, assert both payloads arrive on the receiver. **Note: per [F15]'s fix, the rebind path does not actually insert anything into `client_sessions`, so this test artificially populates the map solely to exercise the batch helper's iteration semantics.**
- [ ] `test_replay_metadata_for_client_with_no_sessions_is_noop` — `client_sessions` has no entry for client 99; `replay_metadata_for_client(99)` does not panic and does not publish anything on the broadcast.
- [ ] `test_replay_metadata_for_client_skips_sessions_without_metadata` — a client has two sessions in `client_sessions` but only one has `latest_metadata` set; replay publishes one frame, not two.
...
- **`feed_router.register_stream(FeedId::SESSION_METADATA, session_metadata_broadcast_tx, LagPolicy::Warn)`** — SESSION_METADATA is a **broadcast stream**, not a snapshot watch. This is a change from the plan's earlier draft which referred to a `register_watch(FeedId::SESSION_METADATA, ...)` API that does not exist — the actual router API is `register_stream` (broadcast) + `register_input` (mpsc) + `add_snapshot_watches(Vec<watch::Receiver>)` (unkeyed bag). Because [D14] moves SESSION_METADATA from a watch to a broadcast, `register_stream` is now the correct call and `add_snapshot_watches` is not used for SESSION_METADATA at all. The supervisor's merger task publishes onto `session_metadata_broadcast_tx`; cards subscribe via the normal broadcast path; on-subscribe replay is handled by `supervisor.replay_metadata_for_client(client_id)` called from `handle_client` (see below).
...
- [ ] **Do NOT add any post-handshake replay call in `handle_client`.** Per [D14] and [F13]'s fix, event-driven replay runs inside `handle_control("spawn_session", ...)` (Step 4). A post-handshake call to `replay_metadata_for_client(client_id)` with a fresh `client_id` is a guaranteed no-op because `client_sessions[client_id]` is empty at that moment — the client has not yet sent `spawn_session`. This bullet is retained as an explicit **negative requirement** so a reviewer or a future revision does not reintroduce the no-op call.
...
- [ ] **Removed:** the earlier-draft test `test_handle_client_replay_on_connect_emits_stored_metadata` is deleted. Its premise (pre-populating `client_sessions[client_id]` before invoking `replay_metadata_for_client`) does not exercise the real reconnect flow per [F13] — in practice, the fresh-connection `client_id` has an empty `client_sessions` entry at post-handshake time, and the call would be a perpetual no-op. The real reconnect path is tested by Step 4's `test_spawn_session_replays_latest_metadata_for_known_session` (unit) and Step 10's `test_session_metadata_reaches_late_subscriber` (integration).
...
- [ ] `handle_client` calls `supervisor.on_client_disconnect(client_id)` on teardown. **There is no post-handshake call to `replay_metadata_for_client(client_id)`** — event-driven replay runs inside `handle_control("spawn_session", ...)`.
