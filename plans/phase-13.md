# Phase 13: Python Refactoring Operations Infrastructure {#phase-13}
...
###### Phase B: Stdlib Module Infrastructure {#step-0-9-phase-b}

**Prerequisites:** Phase A complete

**Context:** To classify imports correctly, we need comprehensive stdlib module lists for each supported Python version. The stdlib changes between Python versions (modules added in 3.9, 3.11; modules removed in 3.12, 3.13). We use the `omnilib/stdlibs` package (MIT licensed, same data source as ruff and isort) to generate authoritative per-version module sets.

**Artifacts:**
- New `scripts/generate_stdlib_modules.py` - One-time generator script
- New `crates/tugtool-python/src/layers/stdlib_modules.rs` - Generated Rust source with per-version module sets
...
- [x] Generate `crates/tugtool-python/src/layers/stdlib_modules.rs`:
  ...
  // Per-version stdlib sets (generated content follows)
  static STDLIB_3_8: Lazy<HashSet<&'static str>> = Lazy::new(|| {
      // ~200+ modules for Python 3.8
      let mut set = HashSet::with_capacity(220);
      set.insert("abc");
      set.insert("aifc");
      // ... (full generated list)
      set
  ...
  // Similar for STDLIB_3_9, STDLIB_3_10, STDLIB_3_11, STDLIB_3_12, STDLIB_3_13
  // Notable version differences:
  // - 3.9 adds: graphlib, zoneinfo
  // - 3.11 adds: tomllib
  // - 3.12 removes: distutils
  // - 3.13 removes: aifc, audioop, cgi, cgitb, chunk, crypt, imghdr, mailcap,
...
**Code Location:**
- `scripts/generate_stdlib_modules.py`
- `crates/tugtool-python/src/layers/stdlib_modules.rs`
...
###### Phase C: Import Classifier Implementation {#step-0-9-phase-c}

**Prerequisites:** Phase A and Phase B complete

**Context:** Import classification determines which group an import belongs to (future, stdlib, third-party, local). Correct classification is essential for proper import ordering. The classifier must handle:
- Version-specific stdlib modules (e.g., `tomllib` is stdlib on 3.11+, third-party on 3.10-)
- Backport packages (e.g., `tomli` is the third-party backport of `tomllib`)
- First-party detection via filesystem checks (packages in `src/` or project root)
- Explicit overrides via configuration

**Artifacts:**
- Extended `crates/tugtool-python/src/layers/imports.rs` - ImportClassifier implementation
...
- [x] Define `ImportClassifierConfig` struct:
  ...
  use std::collections::HashSet;
  use super::stdlib_modules::PythonVersion;
...
- [x] Define backport package mappings:
  ...
  const BACKPORT_PACKAGES: &[(&str, &str, PythonVersion)] = &[
      ...
      ("importlib_metadata", "importlib.metadata", PythonVersion::new(3, 8)),
      ("importlib_resources", "importlib.resources", PythonVersion::new(3, 9)),
  ];

  /// Packages that are always third-party (never stdlib).
  const ALWAYS_THIRD_PARTY: &[&str] = &[
      "typing_extensions",
  ];
  ```
      ...
      fn is_stdlib_for_version(&self, top_level: &str) -> bool {
          ...
          for (backport, stdlib_name, since_version) in BACKPORT_PACKAGES {
              if top_level == *backport {
                  // If we're on a version where the stdlib equivalent exists,
                  // the backport is still third-party
                  return false;
              }
              if top_level == stdlib_name.split('.').next().unwrap_or(stdlib_name) {
                  // This is the stdlib module - check if available in target version
                  return self.config.target_version.minor >= since_version.minor;
              }
          }
...
###### Phase D: ImportInserter Implementation {#step-0-9-phase-d}

**Prerequisites:** Phase A, B, and C complete

**Context:** `ImportInserter` adds new import statements at the correct location in a file, respecting import groups and existing organization. Uses `ImportClassifier` to determine where new imports should be placed.
...
- [x] Implement `ImportInserter` struct with builder pattern:
  ...
  impl ImportInserter {
      pub fn new(classifier: ImportClassifier) -> Self {
          Self { classifier, mode: ImportInsertMode::default() }
      }
...
- [x] Implement `analyze_existing_imports()` to scan file for import locations:
  ...
  pub struct ImportAnalysis {
      /// Imports grouped by classification.
      pub groups: BTreeMap<ImportGroupKind, Vec<ImportInfo>>,
      /// Line number of the last import statement.
      pub last_import_line: Option<usize>,
      /// Span of the entire import section (first to last import).
      pub import_section_span: Option<Span>,
      /// Line after module docstring (if any).
...
- [x] Implement `insert()` method returning edit operation:
  ...
  impl ImportInserter {
      ...
      pub fn insert(
          ...
          let group = self.classifier.classify(&import.module_path());
          let insertion_point = self.find_insertion_point(group, analysis)?;
          // Generate appropriate text with newlines
          Ok(TextEdit::insert(insertion_point, import.render()))
      }
  }
  ```
...
###### Phase E: ImportRemover Implementation {#step-0-9-phase-e}
...
###### Phase F: ImportUpdater Implementation {#step-0-9-phase-f}
...
###### Phase G: TYPE_CHECKING Import Tracking {#step-0-9-phase-g}

**Prerequisites:** None (can run in parallel with Phases A-F)
...
##### Step 1.1.2: Lookup Observability Infrastructure {#step-1-1-2}

**Commit:** `feat(python): add lookup observability infrastructure for debugging`

**References:** [D05](#d05-rename-reference), [Step 1.1](#step-1-1), [Step 1.1.5](#step-1-1-5)

**Prerequisites:** [Step 1.1](#step-1-1) (rename hardening)

**Rationale:**

Debugging the `test_rename_walrus_in_comprehension` failure revealed that the lookup pipeline lacks observability. The root cause was an off-by-one column error (36 vs 37), but the `SymbolNotFound` error provided no diagnostic information to indicate this. The debugging process required extensive speculation about binding collection, scope resolution, and span containment when the actual problem was simply that the test specified the wrong column.

This infrastructure will:
1. Catch location specification errors at test time before they propagate
2. Provide actionable diagnostic information when lookups fail
3. Enable tracing through the lookup pipeline when debugging complex issues
4. Detect span regressions via snapshot testing

**Artifacts:**
- New: `crates/tugtool-python/src/test_utils.rs` (or extend existing test utilities)
- Modified: `crates/tugtool-python/src/lookup.rs` (enhanced error types)
