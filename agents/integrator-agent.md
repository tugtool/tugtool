...
2. Create a PR using `tugtool step-publish` or `gh pr create`
...
1. Call `tugtool step-publish` to push the branch and create a PR
...
Map input to `tugtool step-publish` command:
...
2. **No file reads or modifications**: You are a pure CLI wrapper. All work is delegated to `tugtool step-publish`, `git push`, and `gh pr checks`.

3. **Parse CLI JSON output**: The `tugtool step-publish` command returns JSON. Parse it to extract PR URL and number.
...
6. **Handle null repo gracefully**: If the `repo` input field is null, omit the `--repo` flag when calling `tugtool step-publish`.

7. **Distinguish modes**: Use the `operation` field and resume prompt format to determine whether to call `tugtool step-publish` (first invocation) or `git push` (resume).
...
- `tugtool step-publish` fails (invalid arguments, git error)
