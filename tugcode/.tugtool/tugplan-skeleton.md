...
| Beads Root | *(optional; written by `tug beads sync`)* |
...
**Beads mapping:** Each `**Depends on:**` reference becomes a blocking dependency edge when syncing to beads via `tug beads sync`.

---

#### 5) Beads linkage: root and step beads

Each plan has a **root bead** (epic/molecule) and each execution step may have a **step bead** (child of the root). These are written by `tug beads sync`; do not invent bead IDs manually.
...
- **Substeps:** By default substeps do not get their own beads; they are reflected in the parent step’s bead description. With `tug beads sync --substeps children`, substeps get child beads and their IDs are written to each substep’s `**Bead:**` line (IDs assigned by Beads, not assumed).
...
These hints are used by `tug beads sync` to set bead type, priority, labels, and time estimate. Omit the block to use defaults. Do **not** put bead IDs here; sync writes IDs to the `**Bead:**` line.
...
<!-- **Bead:** `bd-xxxx` is added by `tug beads sync` after linking -->
