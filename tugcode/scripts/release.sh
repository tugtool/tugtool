...
CURRENT=$(grep '^version = ' tugcode/Cargo.toml | head -1 | sed 's/^version = "\(.*\)"/\1/')
if [[ -z "$CURRENT" ]]; then
    echo "Error: Could not read current version from tugcode/Cargo.toml" >&2
    exit 1
fi
...
sed -i '' "s/^version = .*/version = \"$VERSION\"/" tugcode/Cargo.toml

# Verify sed actually changed something
AFTER=$(grep '^version = ' tugcode/Cargo.toml | head -1 | sed 's/^version = "\(.*\)"/\1/')
if [[ "$AFTER" != "$VERSION" ]]; then
    echo "Error: Failed to update version in tugcode/Cargo.toml (got '$AFTER', expected '$VERSION')" >&2
    exit 1
fi
