...
"#;
        ...
        let new_content = fs::read_to_string(&log_path).unwrap();
        assert!(new_content.contains("step: step-1"));
        assert!(new_content.contains("## step-1: Test implementation"));
        assert!(new_content.contains("- .tugtool/tugplan-13.md"));

        // Verify entry is after the separator
        let separator_pos = new_content.find("---\n\n").unwrap();
        let entry_pos = new_content.find("step: step-1").unwrap();
    ...
    fn test_log_prepend_multiple_entries() {
        ...
        let step2_pos = new_content.find("step: step-2").unwrap();
        assert!(step2_pos < step1_pos, "Newest entry should be first");
    }
