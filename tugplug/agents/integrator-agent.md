...
2. Create a PR using `tugutil open-pr` or `gh pr create`
...
1. Call `tugutil open-pr` to push the branch and create a PR
...
Before calling `tugutil open-pr`, verify that a remote origin is configured:
...
Map input to `tugutil open-pr` command:
...
2. **No file reads or modifications**: You are a pure CLI wrapper. All work is delegated to `tugutil open-pr`, `git push`, and `gh pr checks`.

3. **Parse CLI JSON output**: The `tugutil open-pr` command returns JSON. Parse it to extract PR URL and number.
...
6. **Handle null repo gracefully**: If the `repo` input field is null, omit the `--repo` flag when calling `tugutil open-pr`.

7. **Distinguish modes**: Use the `operation` field and resume prompt format to determine whether to call `tugutil open-pr` (first invocation) or `git push` (resume).

8. **Detect no-remote before push/PR**: On first invocation, run `git -C {worktree_path} remote get-url origin` before any push or PR creation. If the command fails, return ESCALATE immediately. Do not attempt `tugutil open-pr` or `git push` without a remote.
...
- `tugutil open-pr` fails (invalid arguments, git error)
...
"No remote origin configured. This repository uses local-mode workflow. Use 'tugutil merge <plan>' from the main worktree to merge locally."
