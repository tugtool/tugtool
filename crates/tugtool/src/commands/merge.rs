...
fn is_infrastructure_path(path: &str) -> bool {
    path.starts_with(".tugtool/") || path.starts_with(".beads/")
}
...
/// Shows .tugtool/ and .beads/ files that differ between main and the branch.
...
/// Check if all merge conflicts are in infrastructure files (.tugtool/, .beads/)
...
fn try_auto_resolve_conflicts(repo_root: &Path) -> Result<bool, String> {
    ...
    let is_infrastructure =
        |path: &str| -> bool { path.starts_with(".tugtool/") || path.starts_with(".beads/") };
...
fn save_infra_to_temp(repo_root: &Path, infra_files: &[&str]) -> Result<PathBuf, String> {
    ...
    let temp_dir = std::env::temp_dir().join(format!("tug-merge-{}-{}", timestamp, nanos));
...
mod tests {
    ...
    fn test_squash_merge_auto_resolves_infrastructure_conflicts() {
        ...
        let log = fs::read_to_string(p.join(".tugtool/tugplan-implementation-log.md")).unwrap();
        assert!(
            log.contains("step-0"),
    ...
    #[test]
    fn test_check_infra_diff_with_tug_changes() {
        use tempfile::TempDir;
        ...
        Command::new("git")
            .arg("-C")
            .arg(temp_path)
            .args(["checkout", "-b", "tug/infra-20260210-120000"])
            .output()
            .expect("git checkout");

        fs::create_dir_all(temp_path.join(".tugtool")).unwrap();
        fs::write(
            temp_path.join(".tugtool/tugplan-implementation-log.md"),
            "# Log\nstep-0",
        )
        .unwrap();
        fs::create_dir_all(temp_path.join(".beads")).unwrap();
        ...
        let result = check_infra_diff(temp_path, "tug/infra-20260210-120000");
        assert!(result.is_some(), "Should detect infrastructure diffs");
        let msg = result.unwrap();
        assert!(
            msg.contains(".tugtool/"),
            "Should mention .tugtool/ files: {}",
            msg
        );
        assert!(
            msg.contains(".beads/"),
    ...
    fn test_save_infra_to_temp() {
        ...
        fs::write(repo_path.join(".tugtool/config.toml"), "test config").unwrap();
        fs::write(
            repo_path.join(".tugtool/tugplan-implementation-log.md"),
            "# Log\nentry1\nentry2",
        )
        .unwrap();
        ...
        assert!(temp_backup.join(".tugtool/config.toml").exists());
        assert!(
            temp_backup
                .join(".tugtool/tugplan-implementation-log.md")
                .exists()
        );

        // Verify content
        let config_content = fs::read_to_string(temp_backup.join(".tugtool/config.toml")).unwrap();
        assert_eq!(config_content, "test config");

        let log_content =
            fs::read_to_string(temp_backup.join(".tugtool/tugplan-implementation-log.md")).unwrap();
    ...
    fn test_copy_infra_from_temp() {
        ...
        fs::create_dir_all(repo_path.join(".tugtool")).unwrap();
        fs::write(repo_path.join(".tugtool/config.toml"), "original").unwrap();

        // Save to temp
        let infra_files = vec![".tugtool/config.toml"];
        let temp_backup = save_infra_to_temp(repo_path, &infra_files).unwrap();

        // Modify original
        fs::write(repo_path.join(".tugtool/config.toml"), "modified").unwrap();
        assert_eq!(
            fs::read_to_string(repo_path.join(".tugtool/config.toml")).unwrap(),
            "modified"
        );
        ...
        let restored = fs::read_to_string(repo_path.join(".tugtool/config.toml")).unwrap();
    ...
    fn test_save_restore_round_trip() {
        ...
        fs::write(repo_path.join(".tugtool/config.toml"), original_content).unwrap();
        ...
        fs::write(
            repo_path.join(".tugtool/config.toml"),
        ...
        let final_content = fs::read_to_string(repo_path.join(".tugtool/config.toml")).unwrap();
        assert_eq!(
    ...
    fn test_save_infra_nested_dirs() {
        ...
        fs::create_dir_all(repo_path.join(".tugtool/archive")).unwrap();
        fs::write(
            repo_path.join(".tugtool/archive/old.md"),
            "archived content",
        )
        .unwrap();
        ...
        // Save nested files
        let infra_files = vec![".tugtool/archive/old.md", ".beads/beads.jsonl"];
        let temp_backup = save_infra_to_temp(repo_path, &infra_files).unwrap();

        // Verify nested directories were created in temp
        assert!(temp_backup.join(".tugtool/archive").is_dir());
        assert!(temp_backup.join(".tugtool/archive/old.md").exists());
        assert!(temp_backup.join(".beads/beads.jsonl").exists());

        // Verify content
        let archived = fs::read_to_string(temp_backup.join(".tugtool/archive/old.md")).unwrap();
    ...
    fn test_merge_allows_infra_only_dirty_files() {
        ...
        fs::create_dir_all(repo_path.join(".tugtool")).unwrap();
        fs::write(repo_path.join(".tugtool/config.toml"), "test").unwrap();
        fs::create_dir_all(repo_path.join(".beads")).unwrap();
        fs::write(repo_path.join(".beads/beads.jsonl"), "{}").unwrap();
    ...
    fn test_targeted_infra_discard() {
        ...
        fs::create_dir_all(repo_path.join(".tugtool")).unwrap();
        fs::write(repo_path.join(".tugtool/config.toml"), "infra content").unwrap();
        fs::create_dir_all(repo_path.join("src")).unwrap();
        ...
        let _ = Command::new("git")
            .arg("-C")
            .arg(repo_path)
            .args(["checkout", "--", ".tugtool/config.toml"])
            .output();
        let _ = Command::new("git")
            .arg("-C")
            .arg(repo_path)
            .args(["clean", "-f", "--", ".tugtool/config.toml"])
            .output();

        // Verify infra file is gone but non-infra file remains
        assert!(
            !repo_path.join(".tugtool/config.toml").exists(),
    ...
    fn test_infra_restored_on_merge_failure() {
        ...
        fs::write(repo_path.join(".tugtool/config.toml"), "corrupted").unwrap();
