...
4. **App reload**: Tugcard reads state bags from tugbank during initialization (two-phase fetch in `main.tsx`). Same restore path as tab activation — content first via persist path or direct-apply fallback, all within `useLayoutEffect`. **Prerequisite:** `SelectionGuard.attach()` must run in `useLayoutEffect` (Rule 3) so CSS Highlight objects exist before any Tugcard restore effects fire.
