...
2. **Tab activation** (switching to): Tugcard reads the state bag from the in-memory cache. **Restore order is critical — content first, then scroll, then selection:**
   ...
   - **Scroll and selection restore** are deferred to `requestAnimationFrame`. By this point the browser has completed layout on the restored content, so `scrollTop`/`scrollLeft` assignments are not clamped and text nodes exist at their final positions for `setBaseAndExtent`. Flash suppression: `visibility:hidden` is applied in the `useLayoutEffect` body (before RAF) when scroll state exists, and cleared after scroll is set in the RAF callback.
   - **Cleanup**: the RAF handle is stored in a local variable captured by the cleanup closure (not a ref). On rapid tab switches, the previous effect's cleanup cancels the pending RAF before the new effect schedules its own — no cross-contamination between tabs.
3. **App save** (visibility change + beforeunload): Each Tugcard registers a save callback with DeckManager on mount via `useLayoutEffect` (Rule #3). The callback captures scroll, selection, and content state for that card's currently-active tab — identical to the tab-deactivation save, but triggered externally rather than by a tab switch. DeckManager listens for `visibilitychange` (`document.visibilityState === 'hidden'`) and `beforeunload`. On either event, DeckManager calls all registered save callbacks, then flushes all dirty tab states to tugbank immediately (bypassing the normal debounce). This ensures the currently-active tab's state in every card is persisted even if no tab switch occurred during the session. Unregistration happens in the `useLayoutEffect` cleanup (Rule #3). **Transport note:** On `beforeunload`, the page is unloading and normal `fetch` calls may not complete. Use `fetch` with `keepalive: true` for the flush — this guarantees the request is dispatched even during page teardown, and unlike `navigator.sendBeacon()` it supports PUT requests (which tugbank's `putTabState` endpoint requires). The `visibilitychange` handler uses the normal `fetch` path since the page remains alive.
4. **App reload**: Tugcard reads state bags from tugbank during initialization (two-phase fetch in `main.tsx`). Same restore path as tab activation — content first in `useLayoutEffect`, then scroll and selection via RAF.
...
**Lifecycle:** SelectionGuard owns the highlight lifecycle. On `attach()`, it creates a `Highlight` object and registers it via `CSS.highlights.set("inactive-selection", highlight)`. On each `pointerdown` (capture phase, before the browser clears the global Selection), a highlight-management loop runs before the existing tracking loop:
...
**[D75] HVV extends to neutrals and alpha via `--tug-neutral-*` and CSS relative color syntax.**

The HVV system covers chromatic colors. For a complete, coherent color system with no behavioral dropoffs, it extends to neutrals and transparency:
...
The neutral val-to-L mapping uses a fixed canonical L of 0.555 (the minimum allowed by the system). This produces a perceptually uniform gray ramp that behaves identically to chromatic hues — surface tokens, foreground tokens, and border tokens can all reference neutral presets using the same `var()` pattern.
