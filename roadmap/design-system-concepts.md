...
15. **Interactive controls get 2.5D elevation; content areas stay flat.** Every interactive control (button, input, switch, slider, toggle, checkbox, radio) uses the three-state elevation pattern: box-shadow with `inset 0 1px` top-edge highlight and bottom shadow at rest, deeper shadow on hover, collapsed shadow + `translateY(1px)` press on active, no shadow when disabled. Inputs use the inverted inset pattern (recessed, no bottom shadow). Content areas (text, labels, cards, separators, badges, status indicators) are flat. Elevation is expressed through three token tiers: `--tug-base-elevation-*` (per-theme base), `--tug-<component>-*` (per-component, maps from base), CSS rules. No gradients — shadow and highlight geometry provide sufficient depth. [D04, Phase 8]
