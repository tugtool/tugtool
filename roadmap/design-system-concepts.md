...
15. **Interactive controls use token-driven control states; content areas stay static.** Every interactive control (button, input, switch, slider, toggle, checkbox, radio) uses the control state token pattern: `--tug-base-control-{variant}-{property}-{state}` where properties are bg, fg, border, icon and states are rest, hover, active. States lighten progressively (rest darkest → hover → active lightest). Content areas (text, labels, cards, separators, badges, status indicators) have no state transitions. Control tokens are defined in `tug-base.css`; component CSS wires `var()` references. No box-shadow elevation, no translateY press-down, no gradients — color transitions provide all interaction feedback. Toggle controls (checkbox, switch, radio) use a two-axis token pattern: `--tug-base-toggle-{part}-{value}[-{interaction}]` where *part* is `track` or `thumb`, *value* is `off`, `on`, or `mixed`, and *interaction* is `hover` or omitted for rest. Disabled uses `--tug-base-toggle-{part}-disabled` (value-independent). The value axis is primary; interaction modifiers layer on top. Never borrow a value-state token (e.g. `mixed`) to serve as an interaction-state token (e.g. hover) — each axis gets its own tokens. [D04, Phase 8]
