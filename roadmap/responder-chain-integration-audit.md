...
- **No `cancel` phase for Radix sliders.** Radix does not expose a pointer-cancel or Escape-mid-drag hook, so `TugSlider` never dispatches `phase: "cancel"`. Parents that need rollback-on-cancel semantics would have to either switch to a raw pointer-event slider (see the `gallery-mutation-tx` pattern) or accept that every interaction ends in `"commit"` or `"discrete"`. Flagged in the `TugSlider` module docstring so consumers know the limitation up front.
...
- **Unit test coverage.** `src/__tests__/tug-slider.test.tsx` covers the phase disambiguation end-to-end against a real `ResponderChainManager` with `observeDispatch`: (1) keyboard-only dispatches exactly one `"discrete"` event (verifying `handleSliderCommit` no-ops when `draggingRef` is false — otherwise every keystroke would double-dispatch), (2) bare pointerdown emits `"begin"` with the prop value, (3) a full pointerdown + keyboard-step sequence yields the ordered `begin → change → commit` phase trajectory, (4) disabled sliders swallow pointerdown, (5) auto-derived sender ids are stable across a drag, (6) two sliders in one tree produce distinct sender ids, (7) the nested `TugValueInput` commits under the slider's sender id. Value payloads are only loosely asserted because happy-dom returns a 0×0 bounding rect — Radix's pointerdown position math snaps to the minimum — but the phase sequence is the load-bearing invariant and it's verified deterministically.
- **Verification.** `bun run check` clean, `bun test` → 1859 pass / 0 fail (1852 baseline + 7 new TugSlider phase tests).
