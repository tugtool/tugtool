...
**Status:** _in progress. Audit complete (7 ✓ / 3 ✗: the polish-15 question variant, [replay-1], [replay-2]). **[replay-2] landed + tested** — `TurnTelemetry` carries an optional `turnEndReason`; `buildTurnEntry` prefers the persisted reason over the `interruptInFlight` re-derivation (`effectiveReason`); `buildRecordTelemetryEffect` persists it; 3 new tests in `reducer.telemetry-replay-inline.test.ts`; `tsc` clean, 336 code-session-store tests green. **[replay-1]** approach decided (tugcode `runReplay` synthesizes a terminal `turn_complete` for a dangling cold-resume turn). Remaining gap-close: `tide-question-dialog` (polish-15) + wiring; the `[replay-1]` tugcode change._
...
4. **[replay-1] Cold-resume in-flight strand.** _Audit finding — the reducer-side split is not viable: the discriminator is "will this dangling turn get a live `turn_complete`?", which depends on whether tugcast's claude subprocess is alive and mid-turn. Neither `handleReplayComplete` nor the tugdeck resume path (`fireRestore`) has that fact — only tugcode does. Decision: take the cross-component fix._ tugcode's replay translator (`runReplay` in `tugcode/src/session.ts`) walks the resumed JSONL; when it reaches end-of-JSONL with a turn still open (a `user_message_replay` emitted, no matching `turn_complete` — a refused / abandoned final turn) AND the session is not a live continuation, it emits a synthetic terminal `turn_complete` for that turn so the reducer's existing replay commit path closes it. The `result` is chosen so the reducer derives a non-`error` terminal reason (the turn was cut off, not a protocol error). `handleReplayComplete`'s chain-link-13 is left intact for the genuine live-continuation case (the wire keeps streaming after the bracket). No reducer heuristic, no timer, no provenance flag; the "every committed row has a `turn_complete`" invariant holds.
...
- [ ] **[replay-1]** Fix the cold-resume in-flight strand — a dangling final turn commits as a terminal `TurnEntry` instead of stranding an animated in-flight row. _Decided: tugcode `runReplay` / `translateJsonlSession` synthesizes the terminal `turn_complete`._
...
- [ ] **[replay-1]** A replayed session whose JSONL ends mid-turn (no final `turn_complete`) commits that turn as a terminal `TurnEntry`; after `replay_complete` no row has `turn === undefined` and no `TugThinkingIndicator` is mounted.
