...
| AWAITING_FIRST_TOKEN | reserved | Z1A: live model + submit timestamp. Z1B: `TugThinkingIndicator` (`animating={true}`). | status row · STATE "Waiting" · TIME live (ticking) | project badge | "Awaiting first token" indicator | **Stop** |
| STREAMING | reserved | Z1A: live model + submit timestamp. Z1B: `TugThinkingIndicator` (`animating={true}`) — bars pulse for the duration of the live phase. (The earlier "freeze while text is actively streaming" derivation from the deferred [Step 20.4.11](#step-20-4-11) is not wired — `TideZ1B` hardcodes `animating={true}` in its in-flight-row mode, so the bars pulse for the whole live phase with no streaming-vs-thinking sub-state.) | status row · STATE "Streaming" · TIME live · TOKENS / CONTEXT climb off the live frame | project badge | "Claude is thinking" indicator | **Stop** |
| TOOL_WORK | reserved | Z1A: live model + submit timestamp. Z1B: `TugThinkingIndicator` (`animating={true}`). | status row · STATE "Working" · TIME live (ticking) | project badge | "Running {tool_name}" | **Stop** |
...
**Matrix-edit policy.** [Step 20.5.C](#step-20-5-c) is **superseded** by [Step 20.4.10](#step-20-4-10) through [Step 20.4.13](#step-20-4-13); [Step 20.4.15](#step-20-4-15) authoritatively wired the production Z1 asst-half cells per the polish-plan above. Every Z1 asst-half cell in the matrix now says (a) which content the two-line stack's status row (Z1B) shows for that state — `TugThinkingIndicator` on the in-flight row, `EndStateDisplay` on committed rows; `TideZ1B` ([Step 20.4.15](#step-20-4-15)) drives this per-row off `turn === undefined` (row not yet committed), NOT the session `phase`, so committed rows in a multi-turn transcript never re-derive in-flight-ness from a session-wide signal — and (b) what the model row (Z1A) shows (always present; model name + timestamp from the active or committed turn). Earlier framings ("per-turn live elapsed", "TugProgress agent") are retired — Z1B is the canonical surface for both signals. (REPLAYING is the lone exception — the transcript is not painted during replay, so its matrix row carries no Z1 cell content; see [DT10](#dt10-replay-transcript-suppression).) [Step 20.5.D.5](#step-20-5-d-5) re-authored the Z2 column to describe the shipped four-cell `TideTelemetryStatusRow` — see the **Z2 status row** note above.
...
| [20.5.D.5](#step-20-5-d-5) | Cross-zone coordination — Z4 prompt-entry footer consumes the lifecycle hook (Z2 audited already conformant) | `tide-card.tsx`, _new_ `tide-card-phase-indicator.tsx` |
...
**Status:** _complete. Began with the audit the scope mandates. **Z2** — the shipped `TideTelemetryStatusRow` (the four-cell STATE·TIME·TOKENS·CONTEXT row, always mounted [L26]) already satisfies the matrix's Z2 intent: the STATE cell reflects every lifecycle state inline, `deriveTimeCellMs` already does the live-tick / freeze-on-pause behaviour per cell, and the REPLAYING / ERRORED / TRANSPORT_DOWN "badges" the pre-20.4.x Z2 column listed are redundant with the `TideRestoring` backdrop + the `TugPaneBanner`. Re-implementing them as Z2 content would duplicate those surfaces and break the row's [L26] mount identity — so Z2 ships **no new code**; the stale matrix Z2 column was re-authored instead to describe the shipped four-cell row (the new "Z2 status row" note below the [20.5.A](#step-20-5-a) matrix). **Z4** — empty in production until now; wired through `useLifecycleState()` as the new `TidePhaseIndicator` ("Awaiting first token" / "Claude is thinking" / "Running {tool}"), installed as the production default for the prompt-entry footer slot in `tide-card.tsx` (an explicit `footerContent` prop or a dev placement-experiment Z4 assignment still overrides it). `resolvePhaseIndicatorView` + `currentToolNameFromCalls` are the pure projection — the running tool name comes off the streaming document's `turn.${turnKey}.tools` path. `tsc` clean; `bun test` green (2350 pass); `audit:tokens lint` 0 violations._
...
- **Z4 (prompt-entry footer)** — per the matrix: the phase indicator — "Awaiting first token" (AWAITING_FIRST_TOKEN), "Claude is thinking" (STREAMING), "Running {tool_name}" (TOOL_WORK); default content otherwise. This is the genuine gap — empty in production today.
...
**Conformance.** [L02] — Z4 reads the matrix row via `useLifecycleState()`, never `phase` directly (Z2 ships no new code — audit finding). [L06] — `TidePhaseIndicator` publishes the live state as a `data-state` attribute; CSS owns any per-state visual. [L23] / [L26] — the Z4 indicator mounts in the prompt-entry's always-present footer-content slot; content swaps inside that stable container with no remount.

**Artifacts.**

- `tugdeck/src/components/tugways/cards/tide-card-phase-indicator.tsx` — _new_ — the Z4 prompt-entry-footer phase indicator: the pure projection (`resolvePhaseIndicatorView`, `currentToolNameFromCalls`, `PhaseIndicatorView`) co-located with the `TidePhaseIndicator` component, the way `tide-card-telemetry-renderers.tsx` co-locates its pure formatters with its renderers.
- `tugdeck/src/components/tugways/cards/tide-card-phase-indicator.css` — _new_ — the indicator's type treatment; one rule, `data-state` styling hook ([L06]), no new token slots ([L20]).
- `tugdeck/src/components/tugways/cards/tide-card.tsx` — wiring: `TidePhaseIndicator` becomes the production default for the Z4 `footerContent` slot.
- `tugdeck/src/components/tugways/cards/__tests__/tide-card-phase-indicator.test.ts` — _new_ — pure-logic tests for both projection functions.
- Z2 — no artifact: the audit found `tide-card-telemetry-renderers.tsx`'s status row already conformant. The Z2 work was the matrix-column re-author in [Step 20.5.A](#step-20-5-a).

**Tasks.**

- [x] Audit the shipped Z2 status row and the prompt-entry footer / Z4 chrome against the matrix's Z2 / Z4 columns; record what already matches vs. the genuine gaps. _Done — Z2 conformant (see Status); Z4 the genuine gap (empty in production)._
...
- [x] Wire the Z4 phase indicator through `useLifecycleState()` — "Awaiting first token" / "Claude is thinking" / "Running {tool_name}". _Done — `TidePhaseIndicator`, the production default for the Z4 footer slot._

**Tests.**

- [x] Pure-logic — `resolvePhaseIndicatorView` (`state → Z4 indicator`, one case per matrix row) + `currentToolNameFromCalls` (the running-tool pick). `tide-card-phase-indicator.test.ts`, 11 cases. No `state → Z2 content` projection — Z2 ships no new code (audit finding).
- [ ] The rendered Z4 behavior is asserted by the [Step 20.5.D.5.C](#step-20-5-d-5-c) end-to-end matrix test; the HMR vet below is the human confirm.
...
- [x] `bun run audit:tokens lint` exits 0.
- [ ] **HMR vet (manual, user-gated)** — drive a session through the live phases / AWAITING_USER / a reconnect; confirm Z4 shows "Awaiting first token" → "Claude is thinking" → "Running {tool}" and collapses cleanly otherwise, and Z2 still tracks the matrix.
