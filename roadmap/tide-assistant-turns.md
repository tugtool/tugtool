...
- [x] **20.4.7.D.2 — Tugcode: settings reader + tokenizer integration + frame emitter.** `tugcode/src/claude-code-settings.ts` reads `~/.claude/settings.json` with a documented fallback policy. `tugcode/src/context-breakdown.ts` houses the pure helpers (`tokenizeFileCached`, `computeStaticCategories`, `computeCalibrationRatio`, `computeMessagesTokens`, `buildContextBreakdownFrame`, `extractObservedInputTokens`, `extractContextMax`) and the stateful `ContextBreakdownEmitter` class. `SessionManager.handleClaudeLine` invokes the emitter on `system:init` / `result` / `system:compact_boundary` and writes the resulting `context_breakdown` IPC frame to the wire; `SessionManager.handleUserMessage` feeds user text to the emitter for calibration anchoring. `main.ts` reads settings once at startup, builds the emitter, and threads it through the new optional `SessionManager` constructor option (existing callers and tests continue to work with no emitter; the popover then hits its 20.4.7.C fallback view). New `ContextBreakdown` / `ContextBreakdownCategory` / `ContextBreakdownCategoryId` types in `types.ts`, added to the `OutboundMessage` union. Tests cover the settings reader (8 cases: present-with-true/false, present-with-extras, missing-file, malformed JSON, non-object-JSON, missing-field, non-boolean-field) plus the breakdown module (23 cases: file-cache hit/miss/mtime-invalidation, per-category tokenization with absent files contributing 0, calibration math including the spike-benchmark anchor case, defensive clamps, autocompact-on/off frame shape, no-MCP invariant, full emitter lifecycle).
- [ ] **20.4.7.D.3 — Tugdeck reducer + snapshot + supervisor wiring.** Decode the new wire frame in `events.ts`, reducer handler captures `lastContextBreakdown` onto state, snapshot projection with [L02] reference stability, fixture updates (`tide-card-banner-spec.test.ts` + any other affected). Reducer dispatches `record_context_breakdown` after consuming each frame; tugcast supervisor receives the CONTROL action and calls `SessionLedger.record_context_breakdown(&row)`. Bind-time inlining: supervisor reads the latest breakdown row at bind and attaches it onto the bind response so the snapshot's `lastContextBreakdown` populates before any new frame. Tests: reducer event handling, snapshot reference-stability, fixture round-trip, supervisor action dispatch (parse + persist), bind-time attach.
...
- [ ] Writer effect (`record-context-breakdown`) dispatched by the reducer on each frame. _Ledger surface (`record_context_breakdown(&row)`) landed in 20.4.7.D.1; reducer dispatch + supervisor CONTROL handler land in 20.4.7.D.3._
- [ ] Reader path: at session bind, the supervisor reads the latest breakdown row and inlines it onto the bind response so the snapshot's `lastContextBreakdown` is populated before the popover opens. _Ledger surface (`get_context_breakdown(session_id)`) landed in 20.4.7.D.1; supervisor bind-attach + reducer projection land in 20.4.7.D.3._
