...
- [ ] **20.4.7.D.2 — Tugcode: settings reader + tokenizer integration + frame emitter.** New `tugcode/src/claude-code-settings.ts` (S5; reads `~/.claude/settings.json`), new `tugcode/src/context-breakdown.ts` (S3 tokenizer + calibration math + S4 cache by `(path, mtime)`). Hook into `session.ts` at session_init, each `cost_update`, and `compact_boundary`. Add the new `ContextBreakdown` IPC frame type to `tugcode/src/types.ts`. Tests: settings reader (file present / missing / malformed), calibration math, mocked-SDK wire-emission for both autocompact-on and autocompact-off.
