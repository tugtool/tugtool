...
**Bottom line:** the work is real engineering, not just movement. Both projects ship a better foundation than the one they replaced — the lifecycle is properly delegate-shaped, the portal model preserves session identity in ways the old code could not, and the invariants are enforced rather than wished for. The weaknesses are the kind that always accumulate when two projects ship together under pressure: vocabulary drift, near-duplicate helpers, a god-component starting to form, one semantic inversion under deferred delivery. **Deck/pane/card vocabulary drift (P1–P3) is now addressed** — see [`tugplan-vocabulary-rename.md`](tugplan-vocabulary-rename.md) and the **pane correction** in [`tugplan-vocabulary-pane-rename.md`](tugplan-vocabulary-pane-rename.md) (§ Vocabulary decisions appendix below). Remaining items (L1–L10, P4–P13) still reward attention. None blocks the next layer. Addressing the top three *remaining* recommendations before moving on will make the next project noticeably easier to land.
...
- **Also in that plan (cross-references for readers of this audit):** `DeckState` v3 (`windows` / `activeWindowId`), store API `*Window` mutators, `ADD_CARD_TO_ACTIVE_WINDOW`, Swift `focus-window` + `windowId`, menu copy ("Add Card to Active Window"; dynamic "Close Window" / "Close Card") — all aligned with that plan's **window**-as-deck-container vocabulary.
...
### Subsequent correction: pane vocabulary (post–window interim)

The **window** naming above was the deliberate vocabulary of [`tugplan-vocabulary-rename.md`](tugplan-vocabulary-rename.md), but **window** overloads Apple's `NSWindow` and the browser DOM `window` global. **`TugWindow` → `TugPane`**, data model **`windows` → `panes`**, DOM **`data-pane-id`**, CSS **`.tug-pane`**, registries **`pane-content-registry` / `pane-root-registry`**, wire **`focus-pane` / `paneId`**, **`add-card-to-active-pane`**, Swift menu **"Add Card to Active Pane"** / **"Close Pane"**, and related identifiers are documented and grep-gated in [`tugplan-vocabulary-pane-rename.md`](tugplan-vocabulary-pane-rename.md). **Pane** is the final in-canvas container term; treat earlier **window**-as-deck-frame wording in this audit as historical unless this appendix says otherwise.
