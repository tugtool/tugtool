...
| 7B | Harness Step 6 | harness | Stub-transcript replay + tooling: tugcode `--stub-transcript=<fd>` CLI flag with stream-json replay engine, version handshake recorded in startup, `scripts/capture-tugcode-transcript.ts` + `scripts/reapprove-transcript.ts`, three new error classes (`TugcodeLaunchError`, `TugcodeVersionSkewError`, `TugcodeTranscriptMismatchError`), harness wrappers `seedTugcodeTranscript` / `seedTugcodeError`, transcript round-trip unit tests, resolve [Q04]. Permitted split if diff sprawls: 7B-tugcode (CLI flag + replay engine) + 7B-harness (scripts + errors + wrappers). | Per harness plan Step 6; one commit (or two if split). |
| 7C | Harness Step 7 | harness | EM-card surface + first stub-mode end-to-end smoke: `__tug.*` gains `getEmCardState` / `getEngineSelection` / `awaitEngineReady` / `drainTugcodeTurn` plus the lifecycle delegates (`startTugcode` / `stopTugcode` / `seedTugcodeTranscript` / `seedTugcodeError`). Two new deck-trace events `engine-ready` and `engine-activation-dispatched` per [L01]; wire them at three EM factories (`tide-card`, `tug-prompt-input`, `gallery-prompt-entry`). Bump tugdeck `SURFACE_VERSION` `1.1.0`→`1.2.0`. New `tests/in-app/fixtures/tugcode/em-smoke.transcript.json` + sidecar; `tests/in-app/_smoke-em.test.ts`. Retire scratch `_smoke-tugcode-lifecycle.test.ts`. | Per harness plan Step 7; one commit. |
| 7D | Harness Step 8 | harness | Live-mode smoke: `tests/in-app/_smoke-em-live.test.ts` against real tugcode + real model, gated behind `TUGCODE_LIVE=1` env var so the default sweep skips it. README addition documenting the opt-in flag. | Per harness plan Step 8; one commit; small. |
