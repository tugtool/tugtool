...
**Tasks:**
- [ ] Gap 2 (fresh-card resolver): pick (5) registry-tag-based or (6) selector-priority-tweak from the triage shortlist. Spike: add a quick `engineKind` field to the registry (option 5) or reorder DEFAULT_FOCUS_SELECTORS (option 6); land the fix; verify `m33-em-fresh-card-activation.test.ts` passes for both `gallery-prompt-input` AND `gallery-prompt-entry`.
- [ ] Gap 1 (cold-boot selection): add `cold-boot-restore-snapshot` and `engine-restore-applied` deck-trace events; instrument the relevant call sites; do a real quit-relaunch on a tide-card with text+selection; dump the trace; identify which of triage items 1–4 (or a combination) actually drops the selection. Author the fix per diagnosis.
- [ ] Author both `m32-em-cold-boot-selection.test.ts` and `m33-em-fresh-card-activation.test.ts`. Add both to the default sweep.
- [ ] Drift-prevention cycle: revert each production fix locally → confirm the corresponding new test fails → restore → confirm green. Document in the commit message.
