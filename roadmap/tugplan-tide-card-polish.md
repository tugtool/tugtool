...
**Persistence regression surfaced after Commit 1 — investigation and fix (Commit 1A).** After Commit 1 landed (document-level pane activation listener, `5c178f46`) — and its neighboring commits Commit 0 (`df004681`, DOM-authority `data-focused` controller) and Commit 1b (`f2de80c6`, `ResponderChainProvider` listeners moved to `useLayoutEffect`) — a user-visible regression appeared: **text content in text inputs, text areas, `TugPromptInput`, `TugPromptEntry`, text selections in any of those widgets, and scroll positions in cards are no longer preserved across app reloads or relaunches.** This is a direct [L23] violation (user-visible state that must survive reload).

*Static analysis findings (Commit 1A scoping pass).* The save/restore pipeline is architecturally intact — no file in Commits 0/1/1b touched `card-host.tsx`, `use-card-persistence.tsx`, `use-card-content-restore.ts`, `use-card-dirty-state.ts`, `settings-api.ts`, `deck-manager.ts`'s save surfaces (`saveCallbacks`, `flushDirtyCardStates`, `handleVisibilityChange`, `handleBeforeUnload`, `saveAndFlushSync`, `prepareForReload`), or `main.tsx`'s startup restore path. Yet persistence is broken. That narrows the suspect space to *indirect* effects of the new click-path on the save pipeline. Three concrete hypotheses — all are falsifiable with small, targeted instrumentation and none require speculative code changes ahead of repro:

1. **H1 — Every-click `activateCard` schedules pathological churn that drops final writes.** The new listener fires `store.activateCard(pane.activeCardId)` on *every* pointerdown inside any pane (including text inputs). Same-bit flip still runs `_commitStandardFirstResponderFlip → putFocusedCardId(newFR) + notify() + scheduleSave()` every time. Each click issues a `PUT /api/defaults/dev.tugtool.deck.state/focusedCardId` and schedules (or re-schedules) a layout save. On app-quit the browser cancels in-flight `fetch` requests that weren't dispatched with `keepalive: true`; `putFocusedCardId` and `putLayout` are fire-and-forget and do not set `keepalive`. If the click immediately preceding reload is still in-flight, its cancellation is harmless — but if `scheduleSave`'s rearmed timer means the layout write never actually fired before unload, the card-state flush that rides on `handleBeforeUnload` still runs (sync XHR), so this alone should not lose text. *Likely contributor but probably not root cause on its own.*

2. **H2 — Capture-phase `notify()` races the browser's native text-input focus / selection commit.** The new listener runs in the `pointerdown` capture phase *before* the browser commits focus and selection to the clicked input. `store.activateCard` synchronously runs `notify()`, which forces a `useSyncExternalStore` re-render (external store updates outside React's event system commit synchronously). If that re-render causes any React subtree inside `CardHost` to re-commit in a way that touches the clicked input's DOM node — replaces it, re-parents it, or clears an ancestor's `contenteditable` / `value` attribute — the browser's pending focus/selection never lands, and the subsequent `saveCurrentCardStateRef.current()` on unload reads the wrong state. **This is the most likely class of bug**, because it explains why a *persistence* regression traces to a *click-path* change: the text was never accepted into the input's state in the first place. Per-field symptom prediction: (a) scroll works on initial page load (restore) but isn't remembered if the user scrolled just before quit, (b) text typed and then clicked elsewhere before quit is lost, (c) selections set programmatically are lost because the click's selection-set races the re-render.

3. **H3 — `ResponderChainProvider`'s `useLayoutEffect` move reorders registrations relative to `CardHost`'s save-callback registration.** Commit 1b moved six document listeners and three cross-module registrations (`registerActionDispatch`, `registerCardLifecycleManager`, `selectionGuard.attach`) from `useEffect` to `useLayoutEffect`. Both `CardHost.useLayoutEffect([cardId, store], register/unregister SaveCallback)` and the chain provider's registration are now in the layout phase; React runs layout effects in child-before-parent order, but across React roots / sibling subtrees the order depends on commit order. If on some path the chain provider now attaches *before* CardHost registers its save callback, and something in the attach sequence reads `saveCallbacks` and caches a snapshot, subsequent `forEach`-on-unload could iterate an incomplete map. This is a less-likely variant: neither `registerActionDispatch` nor `selectionGuard.attach` reads the save-callbacks map today, so it would require a latent coupling. *Check, but deprioritize vs. H2.*

*Fix approach by hypothesis.* Each hypothesis has a concrete, bounded fix, but the fix must follow the repro — guessing which to apply without a bisect would be exactly the half-measure this plan rejects:

- **If H2 (most likely):** move the listener's `store.activateCard` call from capture phase to bubble phase, or defer the call to a microtask (`queueMicrotask(() => store.activateCard(...))`) so the browser's native pointerdown-target commit (focus + selection) completes before the React re-render cascades. The capture-phase invariant the current comment cites ("synchronous z-index update preserves pointer→click event sequence") remains intact because `_commitStandardFirstResponderFlip` is still synchronous relative to `pointerup`/`click` — we only deferred a microtask off `pointerdown`. Cite **[L23]** (persistence preserved) and verify the pointer→click z-order sequencing claim still holds by keeping the non-focused-pane activation test (Commit 3 / work item b) green.
- **If H1:** cheapest fix is a same-bit short-circuit at the call site in `pane-focus-controller.ts` — skip `store.activateCard` entirely when `pane.activeCardId === store.getFirstResponderCardId()`. No `notify()`, no `putFocusedCardId`, no `scheduleSave()` churn on re-click of the already-active card. Move the "safety re-sync" comment from `DeckManager.activateCard` (deck-manager.ts:262–266) into the listener's short-circuit: the listener is now the only caller that would have benefited from it, and its same-bit case is the common case.
- **If H3:** flip the order — `ResponderChainProvider`'s document-listener registration back to `useEffect`, keeping only the portions that *depend* on pre-paint timing in `useLayoutEffect`. Or, simpler, ensure `CardHost`'s `registerSaveCallback` runs before the provider's cross-module registrations by registering in a module-level effect at a higher tree position.

*Investigation plan — concrete, bisect-first.* I refused to guess the fix without repro; instead, the Commit 1A work starts with a pinpoint bisect, then ships the minimal fix the bisect identifies.

1. **Repro harness.** Type a unique marker (`PROBE_MARKER_17`) into a `TugPromptEntry` in a tide card. Scroll a card with overflow. Set a selection in a text area. Note `git rev-parse HEAD`. Quit the Tug.app (full relaunch) — this is the `applicationShouldTerminate` → `saveState()` path. Relaunch. Assert the marker, scroll, and selection are all lost. Document exact steps for the regression suite.
2. **Bisect the three commits.** In order `HEAD → HEAD^ (Commit 1b) → HEAD^^ (Commit 1) → HEAD^^^ (Commit 0) → HEAD^^^^ (pre-regression)`, `git checkout` each, run the repro. The first checkout where the repro *does not* reproduce isolates the breaking commit.
3. **Instrument the breaking commit's change-surface.** Add `console.log` at: (a) `CardHost` useLayoutEffect registration and unregistration (log `cardId`), (b) `deck-manager.ts handleBeforeUnload` entry (log `saveCallbacks.size`), (c) `saveCurrentCardStateRef.current()` (log `cardId`, `bag.content === undefined`, `bag.scroll`, `bag.selection`), (d) the tugbank `putCardState` XHR path (log cardId + body length). On quit, the log sequence tells which stage dropped the payload.
4. **Confirm the hypothesis matches the logs.** The predicted signal per hypothesis: H1 → saveCallbacks.size is correct, but no XHR reaches tugbank (unload cancels fire-and-forget). H2 → `bag.content === undefined` because `onSave()` returned undefined because the prompt-input's state never received the pre-quit text. H3 → saveCallbacks.size is lower than live card count; `persistenceCallbacksRef.current === null` for the missing cards.
5. **Apply the bounded fix** for the confirmed hypothesis. No speculative fan-out.
6. **Add a regression test** — a bun test that drives the full `handleBeforeUnload` path against a live `CardHost + TugPromptEntry` composition and asserts the saved bag contains the typed text, captured scroll, and captured selection. This is the regression guard we should have had before Commit 1 landed.

*Scope of Commit 1A.* Commit 1A lands the investigation + fix + regression test as one commit. It is explicitly scoped *before* Commit 2 (validator extension) in the execution plan so the validators land on a codebase where persistence works — otherwise a validator failure during user testing could mask persistence-regression symptoms or vice versa. Upholds **[L23]** (user-visible state preserved across reload).
...
l. **Persistence regression — investigate, fix, add regression test.** See the "Persistence regression surfaced after Commit 1" section above for the three hypotheses (H1 churn, H2 capture-phase race, H3 layout-effect reordering) and the bisect-first investigation plan. Execute the bisect to identify the breaking commit, instrument the save path to confirm the hypothesis, apply the minimal bounded fix for that hypothesis, then add a `bun test` regression test that drives the full `handleBeforeUnload` path against a live `CardHost` + `TugPromptEntry` composition and asserts the saved bag contains the typed text, captured scroll, and captured selection. Upholds **[L23]**.
...
1A. **Commit 1A — Persistence-across-reload regression (bisect → minimal fix → regression test).** Covers work item (l). A user-visible regression surfaced after Commits 0/1/1b landed: text in text inputs, text areas, `TugPromptInput`, `TugPromptEntry`, text selections in those widgets, and card scroll positions are no longer preserved across app reload / relaunch. See the "Persistence regression surfaced after Commit 1" subsection for the three hypotheses and bisect-first investigation steps. Deliverable: (i) bisect identifies breaking commit, (ii) save-path instrumentation confirms hypothesis, (iii) minimal fix applied at the source of the break, (iv) new bun-test regression test drives `handleBeforeUnload` against a live `CardHost` + `TugPromptEntry` composition and asserts text + scroll + selection round-trip through the saved bag. Upholds **[L23]**. Lands *before* Commit 2 so subsequent validator work runs on a codebase with working persistence.
