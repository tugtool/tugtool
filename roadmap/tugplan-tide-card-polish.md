...
**Persistence regression discovered 2026-04-22 — investigation and fix (Commit 1A).** A user-visible regression was discovered: **text content in text inputs, text areas, `TugPromptInput`, `TugPromptEntry`, text selections in any of those widgets, and scroll positions in cards are no longer preserved across app reloads or relaunches.** This is a direct [L23] violation (user-visible state that must survive reload).

*Introduction window is unknown.* The regression was discovered on 2026-04-22. It could have been introduced any time over the past several days — assume a window of at least two to three weeks and treat the whole save/restore pipeline as the suspect surface, not any one recent commit. Earlier drafts of this section prematurely anchored suspicion on Commits 0/1/1b (the pane-activation-listener work). That was a guess, not a finding, and is explicitly withdrawn: static analysis confirms none of those commits touched the save/restore code paths.

*Investigate by instrumentation first, not by bisect.* Manual reproduction is expensive (full app quit + relaunch per trial), and the introduction window is wide. Instrument-first is cheaper and more informative: a single repro at HEAD with logs on every stage of the save and restore pipelines tells us *which stage* is dropping the payload. Then `git log -p` on that stage's files over the past 2–3 weeks narrows the suspect commits by a far tighter filter than a blind bisect. A bisect is held in reserve as a narrow fallback if the logs prove inconclusive.

The save/restore pipeline at HEAD (for reference during instrumentation):

- **Save triggers** — `handleVisibilityChange` (document hidden, `deck-manager.ts:158`), `handleBeforeUnload` (window unload, `deck-manager.ts:170`), `saveAndFlushSync` (Swift `applicationShouldTerminate` → `window.tugdeck.saveState`, `deck-manager.ts:926` / `main.tsx:210`), `saveAndFlush` (`observeApplicationDidResignActive`, `action-dispatch.ts:486–488`), `prepareForReload` (reload action, `deck-manager.ts:937`).
- **Callback registration** — `CardHost` registers in `useLayoutEffect` (`card-host.tsx:157`), unregisters on unmount. Key is `cardId`.
- **Per-card save path** — `saveCurrentCardStateRef.current()` (`card-host.tsx:142`) reads `hostContentEl.scrollLeft/Top`, `selectionGuard.saveSelection(hostStackId)`, and `persistenceCallbacksRef.current?.onSave()`, then `store.setCardState(cardId, bag)`.
- **Persistence callback registration** — `useCardPersistence` registers `{ onSave, onRestore, restorePendingRef }` via `CardPersistenceContext` in `useLayoutEffect` (`use-card-persistence.tsx:152`).
- **Per-card state cache → tugbank** — `setCardState` (`deck-manager.ts:881`) caches and arms a 1000 ms debounce; `flushDirtyCardStates` (`deck-manager.ts:900`) PUTs each dirty bag via `putCardState` (`settings-api.ts:121`, sync XHR when `options.sync`).
- **Cold-boot restore** — `main.tsx` reads `readLayout`, `readTheme`, `readDeckState`, `readCardStates` from the TugbankClient cache, passes `cardStates` to the `DeckManager` constructor, which seeds `cardStateCache`. `useCardContentRestore` (`use-card-content-restore.ts:72`) reads `store.getCardState(cardId)` on mount and applies scroll / selection / content.

*Concrete instrumentation plan (Phase 1 of Commit 1A).* Add temporary `console.log` lines at each stage, with distinctive prefixes so the output is easy to grep and strip:

1. **`card-host.tsx:158`** — at `registerSaveCallback`: log `[probe:register] cardId=${cardId}`. At the cleanup (`unregisterSaveCallback`): log `[probe:unregister] cardId=${cardId}`. Tells us which cards are live in `saveCallbacks` when a save fires.
2. **`card-host.tsx:142`** — at the top of `saveCurrentCardStateRef.current = () => { ... }` body (so it logs each time the closure is *called*, not each time it's rewritten): `[probe:save] cardId=${cardId} hostStackId=${hostStackId} hasContentEl=${!!contentEl} scroll=${JSON.stringify(scroll)} selection=${!!selection} contentDefined=${content !== undefined}`. Also log a truncated preview of `content` if defined.
3. **`deck-manager.ts:158`** — inside `handleVisibilityChange`: `[probe:visibilitychange] hidden=${document.hidden} stateFlushed=${this.stateFlushed} saveCallbacksSize=${this.saveCallbacks.size}`.
4. **`deck-manager.ts:170`** — inside `handleBeforeUnload`: `[probe:beforeunload] stateFlushed=${this.stateFlushed} reloadPending=${this.reloadPending} saveCallbacksSize=${this.saveCallbacks.size}`.
5. **`deck-manager.ts:926`** — inside `saveAndFlushSync` (Swift quit path): `[probe:saveAndFlushSync] saveCallbacksSize=${this.saveCallbacks.size} dirtyCardIdsSize=${this.dirtyCardIds.size}`.
6. **`deck-manager.ts:900`** — inside `flushDirtyCardStates`: `[probe:flush] sync=${options?.sync} ids=${Array.from(this.dirtyCardIds).join(',')}`.
7. **`settings-api.ts:121`** — inside `putCardState`: `[probe:put] cardId=${cardId} sync=${options?.sync} bodyLen=${body.length} hasContent=${bag.content !== undefined}`.
8. **`main.tsx`** (after `readCardStates`, around line 131): `[probe:boot] cardStateKeys=[${Array.from(cardStates.keys()).join(',')}] sizes=${Array.from(cardStates.values()).map((b) => JSON.stringify(b).length).join(',')}`.
9. **`use-card-content-restore.ts:72`** — inside the effect: `[probe:restore] cardId=${cardId} hostStackId=${hostStackId} hasBag=${!!bag} bagKeys=${bag ? Object.keys(bag).join(',') : ''} hasContentEl=${!!contentEl}`.

These nine probes span every stage from save-trigger entry through tugbank write, and from cold-boot read through restore application. No stage is hidden from the log.

*Repro protocol.* Each run produces two log files (pre-quit and post-relaunch):

1. `git rev-parse HEAD` — record the repro hash (expect `dae7ca51` or its descendant at execution time).
2. Launch Tug.app. Open one card with `TugPromptEntry` (tide card). Open one card with scrollable content (any tide card with output).
3. Type `PROBE_MARKER_17` into the prompt-entry. Scroll the other card by ~200 px. Set a text selection of a few characters in a text area if one is available.
4. Save the browser/webview console log up to this point as `pre-quit.log`.
5. Fully quit Tug.app (Cmd-Q). Do not reload — fully terminate.
6. Relaunch. Capture the console log from launch through the first render as `post-launch.log`.
7. Assert: does the prompt-entry show `PROBE_MARKER_17`? Does the other card show its scroll position? Does the selection persist?
8. Attach both logs to the investigation notes.

*Log-reading decision tree.*

- **No `[probe:beforeunload]` AND no `[probe:visibilitychange]` AND no `[probe:saveAndFlushSync]` in `pre-quit.log`** → app-quit bypassed every save trigger. The Swift host is either not calling `window.tugdeck.saveState()`, or it's calling it after the WKWebView is already torn down. Look at `tugapp/` Swift code and `main.tsx:210` wiring.
- **One of those probes fires but `saveCallbacksSize === 0`** → registration was never made or was cleared. Check `[probe:register]` / `[probe:unregister]` ordering in `pre-quit.log`. Look in `git log -p` at `card-host.tsx` and `use-card-persistence.tsx` for changes over the past 2–3 weeks.
- **Probes fire, `saveCallbacksSize > 0`, but `[probe:save] contentDefined=false`** → `persistenceCallbacksRef.current?.onSave()` returned undefined. Either the prompt-entry's `useCardPersistence` registration was cleared by its own cleanup re-registering the no-op pair (`use-card-persistence.tsx:168`), or `persistenceCallbacksRef` was overwritten, or the prompt-entry's internal state genuinely had no text (H2-style: focus/selection race). Look at `git log -p` at `use-card-persistence.tsx`, `tug-prompt-entry.tsx`, `tug-prompt-input.tsx`.
- **`[probe:save]` shows real content but no `[probe:put]` for that cardId** → the cache→tugbank hop was skipped. Likely `flushDirtyCardStates` early-exited or `dirtyCardIds` was already cleared. Look at `deck-manager.ts` mutations over the window.
- **`[probe:put]` fires with real body, but `[probe:boot] cardStateKeys` on relaunch is empty or missing the cardIds** → the write reached tugbank but the read-back is failing. Look at `settings-api.ts:67` (`readCardStates`), `tugbank-client.ts`, and the tugbank domain `dev.tugtool.deck.cardstate`.
- **`[probe:boot]` shows the keys are present with real sizes, but `[probe:restore] hasBag=true hasContentEl=false`** → restore ran before the host pane's content element was registered. `useCardContentRestore` depends on `[cardId, hostStackId]` only; if `hostContentEl` is null at fire time the scroll/selection branch no-ops. This is a known rough edge but has been tolerable historically; if it's now *always* null, something changed in the pane-content-registry population order.
- **`[probe:restore] hasBag=true hasContentEl=true`, but user-visible restore fails** → restore fires but the child component is not responding to `onRestore(bag.content)`. Look at `tug-prompt-entry.tsx:787` (`onRestore`), `tug-prompt-input.tsx:357`, and changes to their `restoreState` or capture/restore implementations.

*Phase 2 of Commit 1A — fix the confirmed stage.* Once the probes identify the broken stage, `git log -p --since="3 weeks ago" -- <file>` on the files that own that stage. The breaking commit should be obvious within a handful of diffs; if not, run `git bisect` narrowed to the file set the instrumentation identified (bisect becomes cheap when the suspect set is already three or four files). Apply the minimal fix at the source of the break — no speculative fan-out, no refactoring rides.

*Phase 3 of Commit 1A — regression test.* Add a `bun test` that drives the full `handleBeforeUnload` (or `saveAndFlushSync`) path against a live `CardHost` + `TugPromptEntry` composition: type a marker string, capture scroll, set a selection, fire the save path, then read the saved `CardStateBag` via `store.getCardState(cardId)` and assert all three fields round-trip. This is the regression guard we should have had before the regression was introduced.

*Commit shape.* Commit 1A may land as a short series:

- **Commit 1A.1** — instrumentation added (`[probe:...]` console.log lines across the nine sites). Commit message labels the commit as temporary; the probes come out in Commit 1A.4.
- **Commit 1A.2** — the minimal fix the logs identified. (If multiple stages are broken, prefer a single commit per cause; do not bundle independent fixes.)
- **Commit 1A.3** — `bun test` regression test.
- **Commit 1A.4** — remove the `[probe:...]` instrumentation. Keep the regression test.

If the fix is a one-line change and the logs are crystal, 1A.1 + 1A.2 + 1A.3 + 1A.4 may collapse into 1A.1 (instrumentation + fix + test + cleanup) at the author's discretion — but the bias is toward small, reviewable commits. Upholds **[L23]** (user-visible state preserved across reload).

*Scope of Commit 1A.* Lands *before* Commit 2 (validator extension) so subsequent validator work runs on a codebase with working persistence — otherwise a validator failure during user testing could mask persistence-regression symptoms or vice versa.
...
l. **Persistence regression — instrument, diagnose, fix, add regression test.** See the "Persistence regression discovered 2026-04-22" section above for the save/restore pipeline reference, nine-point instrumentation plan, repro protocol, and log-reading decision tree. Execute the instrumentation at HEAD, run the repro, read the logs to identify the broken stage, narrow `git log -p` on that stage's files over the past 2–3 weeks to find the breaking commit (bisect held in reserve as a fallback if the logs are inconclusive), apply the minimal bounded fix at the source of the break, then add a `bun test` regression test that drives the full save path against a live `CardHost` + `TugPromptEntry` composition and asserts the saved bag contains the typed text, captured scroll, and captured selection. Remove the instrumentation once the fix and regression test land. Upholds **[L23]**.
...
1A. **Commit 1A — Persistence-across-reload regression (instrument → diagnose → minimal fix → regression test).** Covers work item (l). A user-visible regression discovered 2026-04-22: text in text inputs, text areas, `TugPromptInput`, `TugPromptEntry`, text selections in those widgets, and card scroll positions are no longer preserved across app reload / relaunch. Introduction window is unknown — treat the whole save/restore pipeline as suspect, not any single recent commit. See the "Persistence regression discovered 2026-04-22" subsection for the full instrumentation plan, repro protocol, and log-reading decision tree. Deliverable: (i) nine `[probe:...]` log points spanning save triggers → callback registration → save closure → cache → tugbank write → cold-boot read → restore, (ii) repro run produces `pre-quit.log` + `post-launch.log`, (iii) logs identify the broken stage; narrow `git log -p --since="3 weeks ago" -- <file>` on that stage's files finds the breaking commit (bisect held in reserve), (iv) minimal fix applied at the source of the break, (v) `bun test` regression test drives the full save path against a live `CardHost` + `TugPromptEntry` composition and asserts text + scroll + selection round-trip through the saved bag, (vi) instrumentation removed. May land as sub-commits 1A.1–1A.4. Upholds **[L23]**. Lands *before* Commit 2 so subsequent validator work runs on a codebase with working persistence.
