...
- [ ] **Build `usePositionStableClick` + `OuterScrollportContext`.** Determine where the actual card scrollport lives (likely the entry-chrome scrollport, not the tool-wrapper itself) and publish the ref there. Hook captures pre-state rect, runs caller's mutator, then in a `useLayoutEffect` measures post rect and writes `scrollportRef.current.scrollTop += newTop - oldTop`. Guard: skip when scrollport is null OR newTop is outside [0, viewport.height]. **No rAF anywhere in this hook — `useLayoutEffect` exclusively per [L05].**
- [ ] **Wire fold cue, Find toggle, view-toggle through the hook** in `FileBlock` and `DiffBlock`. Verify by clicking each affordance with the body scrolled so the action row sits near the bottom edge — the action row stays put relative to the cursor.
- [ ] **Audit `data-tug-focus="refuse"` on every action-row button.** Fold cue, Find toggle, view-toggle, Copy across `FileBlock`, `DiffBlock`, `TerminalBlock`. Each pointerdown handler must `preventDefault()` to suppress the browser's default focus action and the implicit scroll-into-view that comes with it. The pattern exists in the codebase; this is an audit pass to confirm coverage and add it where missing.
- [ ] **Reorder the action row.** Fold cue gets pushed to the right edge via flex layout (`margin-inline-start: auto` on the fold-cue, or a two-group structure with `justify-content: space-between`). Features-group gets explicit ordering: Find → view-toggle (DiffBlock only) → Copy (E.4-dependent). Verify visually against the screenshots in the original bug report.
- [ ] **Add `widthStabilize` to `TugPushButton` using CSS Grid.** Both labels render as children of the button with `grid-area: label`. The button uses `display: inline-grid; grid-template-areas: "label"; align-items: center; justify-items: center`. The inactive label gets `visibility: hidden` + `aria-hidden="true"` and **stays in normal layout flow** (no `position: absolute`). The grid cell sizes to max-content of both children. Width is stable across toggles.
- [ ] **Apply `widthStabilize` to the view-toggle button** in `DiffBlock` with both labels declared explicitly.
- [ ] **(Optional)** apply `widthStabilize` to the Copy button across all body kinds (`"COPY"` ↔ `"COPIED"`).
- [ ] **Drop `aria-disabled` from the controlled-confirming path in TugButton.** Replace with `pointer-events: none` while `isConfirming` is true. Verify no other consumer relied on `aria-disabled` to mean "in confirming flash."
- [ ] **Add hover-companion rules for `[data-tug-confirming="true"]`** in `tug-button.css`. New tokens `--tug-button-confirming-hover-bg` and `--tug-button-confirming-hover-border` declared in both `brio.css` and `harmony.css`. Combined hover-bg uses `color-mix` against the confirming-bg.
- [ ] **Force pointer-state revalidation on the confirming revert edge.** In the `useLayoutEffect` that removes `data-tug-confirming`, set `node.dataset.tugFlush = "1"` then immediately delete it inside the same effect body. WebKit re-evaluates selector matching when the attribute set changes, which brings the resting `:hover` rule into paint without requiring user mouse motion. (The first-draft `getBoundingClientRect()` trick was inadequate — it forces layout, not hover-selector re-matching.)
- [ ] **Tests** for hook behavior (synthetic clicks against a stub scrollport with controllable height-change), button reorder, width stabilization (grid cell width equals max-content of both labels), absence of `aria-disabled` during confirming, presence of hover-companion rules in CSS, presence of the flush-attribute toggle in the revert effect.
- [ ] **Tuglaws update** documenting the position-preserving pattern and the corrected confirming-feedback contract.

**Tests (commands, Phase E.3):**

- [ ] `bun test src/components/tugways/internal/__tests__/tug-button.test.tsx` — new `widthStabilize` and `confirming-hover` cases pass.
- [ ] `bun test src/components/tugways/internal/__tests__/use-position-stable-click.test.tsx` (new file) — stub scrollport, mock rect deltas, assert `scrollTop` adjustment.
- [ ] `bun test src/components/tugways/body-kinds/__tests__/file-block.test.tsx` — button-order test passes; click-stability test passes.
...
**Why this phase exists.** Two affordances that should exist on every long-content body kind are missing:
...
Both gaps are recognized in the original Step 10.9 scope — Copy lived on FileBlock before the action-row consolidation but got dropped in Phase D — and Bash fold was deferred. Phase E.4 closes them.
...
- **[L19] Component authoring guide.** ✓ Both updated body kinds (FileBlock with new Copy, TerminalBlock with new fold cue) keep their `@tug-pairings` / `@tug-renders-on` / `data-slot` declarations in sync with the new affordances. The galleries get matrices for the new states.
...
- [ ] **Add Copy button to FileBlock.** Local `[copied, setCopied]` state; click handler does `writeText(fileText).then(() => setCopied(true)).catch(() => undefined)`; `setTimeout` clears the flag after `COPIED_FLASH_MS`. Cleanup on unmount. Button slots into the features group per Phase E.3 ordering. Uses `widthStabilize` from Phase E.3 if it landed for Copy.
- [ ] **Add fold cue to TerminalBlock.** Mirror FileBlock's structure: computed-collapse, threshold const, chevron + line-count label, controlled-via-prop, default-from-threshold. The collapsed render shows the first ~8 lines with a `mask-image` linear-gradient fade at the bottom edge; the fade is a token (`--tugx-terminal-collapsed-fade`) for theming.
- [ ] **Wire TerminalBlock as a responder parent.** Same `useResponderForm` shape as FileBlock — `useId` for the responder ID, register a `Partial<Record<TugAction, ActionHandler>>` that today only carries the existing COPY mapping but is structured to grow. Parent context published via `ResponderScope` wrapping the root.
- [ ] **Update gallery matrices** so Copy + fold cues are exercisable in standalone mode (no chrome above) — confirms the dev-warns from Phase E.2 still trigger correctly, and the standalone-affordance path renders cleanly.
- [ ] **Tests** for Copy (click → flash success, click → no-flash failure) on FileBlock; for fold cue (collapsed default at over-threshold, expand on click, controlled-collapse via prop) on TerminalBlock; for responder-parent registration on TerminalBlock.

**Tests (commands, Phase E.4):**

- [ ] `bun test src/components/tugways/body-kinds/__tests__/file-block.test.tsx` — new Copy tests pass.
- [ ] `bun test src/components/tugways/body-kinds/__tests__/terminal-block.test.tsx` — new fold-cue tests pass.
- [ ] `bunx tsc --noEmit` — clean.
- [ ] `bun run audit:tokens lint` — zero violations.
- [ ] `bun test` (full suite) — green.
