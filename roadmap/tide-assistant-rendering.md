...
- [ ] **Swap DiffBlock view-toggle to `TugChoiceGroup`.** Replace the existing `TugButton` (with its `widthStabilize` alternate-label shim) with a `TugChoiceGroup` of two segments — side-by-side and inline — each carrying a lucide icon (`Columns2`, `AlignLeft`) and a visible label. Wire `selectValue` into the DiffBlock responder form; switch on segment value to set `viewMode`. Route the dispatch through `usePositionStableClick` (Phase E.3 anchor: the action-row container or the diff header). Dispatch `tug-disengage-follow-bottom` on activation. Verify in the live transcript: the action row stays fixed under the cursor when switching modes; both segments visible at all times; the sliding indicator pill animates between them.
- [ ] **Audit `widthStabilize` callers after the view-toggle migration.** Search the codebase for `widthStabilize=` and confirm the only remaining users are controls that *change their own label* (e.g. confirming-state push buttons that flip "Copy" → "Copied"). If the DiffBlock view-toggle is the last consumer that used `widthStabilize` for a *fixed-but-swapping* label pair, leave the prop in place on `TugButton` (it remains correct for confirming flows) but note in the tuglaws section that segment-style 2-way pickers should use `TugChoiceGroup` instead.
...
- [ ] `bun test src/components/tugways/body-kinds/__tests__/diff-block.test.tsx` — view-toggle is a `radiogroup`, both segments visible, `selectValue` flips `viewMode`, `widthStabilize` wrapper is gone from the toggle, position-stable click anchor honored.
