...
- Written study investigating how to deliver delegate-method calls reliably and deterministically, escaping WebKit's gesture focus-lock without leaning on React's useEffect timing as a load-bearing mechanism. Topics to cover:
  ...
  - **Proposal.** Pick one mechanism. Justify it in-document. Commit to it in Step 11. For H1–H3, state plainly how the mechanism handles each. For H4–H5, state the follow-up work (if any) plainly so nothing is lost.
...
- Every hole H1–H5 is addressed in-document: either resolved by the chosen mechanism, documented as accepted, or flagged as a follow-up with a named next step.
