...
- Copy an image atom from the prompt entry, paste into a Text card bound to `foo.md`: `assets/<name>` appears on disk, a markdown link is inserted, and a tile renders. Copy that link back out and paste into the prompt entry: an image atom with a thumbnail appears, and `buildWirePayload` yields a non-empty `content` for it. (App-test.)
...
- The sidecar `version` stays `1` — the field is additive and optional, and readers that ignore it degrade to reference-only, which is a correct behavior rather than a broken one.
...
| Trashed-file URLs awaiting undo | structure | `useRef` map keyed by CM6 change annotation; never persisted | [L24] |
...
| `TugAtomsClipboardEntry` | interface | `tug-text-editor/clipboard-filters.ts` | Add `assetPath` / `assetName` ([P04]) |
...
- [ ] Register a CM6 `domEventHandlers.paste` on the Text card editor that extracts image files from `clipboardData.items` — the same `extractImageFiles` shape `clipboard-filters.ts` uses — uploads them with no `name` (server mints per [P11]), and inserts links in one transaction. This is the only image-paste channel: the native bridge's read result carries no image data (see #assumptions).
...
**Tasks:**
- [ ] Add optional `assetPath` / `assetName` to `TugAtomsClipboardEntry` and fill them in `serializeClipboard` through a new resolver callback ([P04]). Version stays `1` — the fields are additive and a reader that ignores them degrades to reference-only.
- [ ] Text card **copy**: build a sidecar for `assets/`-scoped links inside the selection (each entry carrying the resolved absolute path and the human name) and write it through `writeClipboardViaNative`, replacing today's always-empty sidecar. The plain-text flavor stays the literal markdown, so external apps get exactly what the document says.
- [ ] Text card **paste**: stop discarding `atoms`. For each sidecar entry — an image atom becomes a copy of its bytes into this document's asset base plus an inserted link; a link entry pointing outside this document's base is copied in (asset copy between text files, per the brief) and reference-only when it already resolves here.
- [ ] Prompt entry **paste**: when a sidecar entry carries an `assetPath` naming an image, fetch it through `/api/fs/blob` and mint a real image atom with bytes ([P05] keeps non-images as plain markup with no tile and no atom).
...
**Tests:**
- [ ] Deck unit: sidecar round-trips `assetPath`/`assetName`; a v1 payload without them parses unchanged.
