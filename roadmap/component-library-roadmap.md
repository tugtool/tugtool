...
| 40 | tug-cue | Original | Banner-shaped inline click target — a soft, full-width affordance that doubles as a passive descriptive hint and an action surface. Use case: collapsed-hint banners in body-kinds ("1,234 lines folded — click to expand"), disclosure triggers, soft prompts. Fills a gap between tug-push-button (uppercase CTA), tug-icon-button (icon-only trailing action), and tug-banner (app-modal status strip). API mirrors tug-button: `onClick` xor `action`, optional `target`, `tone`, `density`, `icon`. Designed in [#step-10-6](../roadmap/tide-assistant-rendering.md#step-10-6) of the Tide rendering plan. | High |
