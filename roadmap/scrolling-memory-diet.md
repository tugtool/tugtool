...
**Live read: pending.** The release deck still runs the pre-G1 bundle. Today's live topology has **two** fully-buried panes (x=5 column z2 under z5; x=810 column z1 under z6; the Lens covers nothing fully), so the expected drop at this arrangement is ~2 × ~78MB ≈ **~150MB**, not probe A's 3-pane 233MB. Before-ledger sampled 2026-08-02 20:40–20:44 (pid 43871, 5s cadence, unforced): graphics settled to a flat **472.8M floor** over the final two minutes, excursions to 699M in the first minute (concurrent app-test launches on the machine), mean ≈ 510M; malloc steady 366M. After: `just app-release` relaunches the release instance with the new bundle (the user's act); then re-sample unforced across 3+ cycles and write the delta here — a fresh relaunch also resets restore-churn state, so compare the settled floor and the buried panes' `data-occluded` stamps, not the first minutes.
