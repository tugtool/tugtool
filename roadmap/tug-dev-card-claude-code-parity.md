...
- [x] **Footer pinned + body scrolls** — the diff sheet's flex chain is all-flex (`.tug-sheet-content` flex column / clips → `.tug-sheet-body` flex:1 → `.diff-sheet` flex:1 → `.diff-sheet-body` flex:1 `overflow-y:auto`; no fragile `height:100%`), so under the clamp the file list scrolls and Done stays at the canvas bottom.
