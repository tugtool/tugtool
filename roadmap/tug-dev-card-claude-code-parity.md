...
- Picking a turn in the rewind sheet and choosing a restore action sends `session_rewind{msg_id, scope}` ([#q04-rewind-protocol], resolved empirically in [#step-7a]): `scope:"code"` → tugcode issues a `rewind_files` control request that reverts the files claude edited; `scope:"conversation"` → tugcode truncates the session JSONL to that turn and `--resume`s — verify end-to-end with a real-claude test asserting (code) a file claude wrote is reverted, and (conversation) the resumed session recalls only the retained turns.
...
**Implications:**
- `/resume` ([#step-8]) reuses the existing chooser as-is. `/rewind` ([#step-7]) builds a dedicated `RewindSheet`.
...
4. **Apply.** The chosen action sends `{type:"session_rewind", msg_id, scope, fork}`:
   - `scope:"code"|"both"` → tugcode issues `rewind_files{dry_run:false}` (reverts files claude edited via Edit/Write).
   - `scope:"conversation"|"both"` → tugcode truncates the session JSONL at `msg_id` and respawns (`--resume` in place, or `--fork-session` when `fork` — "the conversation will be forked"). Transcript reflects turns up to `msg_id`.
...
| `/rewind` IPC types | inbound `rewind_preview` + `session_rewind{msg_id,scope,fork}`; outbound `rewind_preview_result` | `tugcode/src/types.ts` + tugdeck `protocol.ts` | Resolved empirically in [#step-7a]. Code dim → tugcode relays the `rewind_files` control request to claude; conversation dim → tugcode truncates the session JSONL + `--resume` (no IPC to claude) |
...
| 7b | `/rewind` | ▶ TODO | turn picker + restore code/conversation (3-action confirm; Summarize out of scope). tugcode bridges `rewind_files` (both ways) + does JSONL-truncate+`--resume`. Decoupled from `SessionPickerSheet` |
...
**Commit:** `feat(dev-card): /rewind — turn picker + restore code/conversation`

**References:** [D06] protocol baseline, [D10] rewind matches terminal, [D15] overlays, [L23] preserve user-visible state, [L26] stable mount identity, Risk R03, Spec S01, (#rewind-flow), (transport-exploration.md#rewind-files-control-request)
...
**Artifacts:**
- New: `rewind-turn-source.ts` — projection over `code-session-store.transcript` → picker rows (msg_id, preview, timestamp, diff-stat). **Not** a `SessionPickerSheet` data source.
- New: `RewindSheet` component (card-scoped overlay per [D15]) — turn picker + confirm step.
- **tugcode bridge (the first task — the [#step-7a] capture was direct-to-claude; this builds the bridge path):**
  - New inbound IPC `rewind_preview` `{type:"rewind_preview", msg_id}` → tugcode issues the `rewind_files{dry_run:true}` **control request** to claude → relays an outbound `rewind_preview_result` `{msg_id, canRewind, filesChanged?, insertions?, deletions?}`.
  - New inbound IPC `session_rewind` `{type:"session_rewind", msg_id, scope:"conversation"|"code"|"both", fork:boolean}` → tugcode applies:
    - **code**/**both** → `rewind_files{dry_run:false}` control request.
    - **conversation**/**both** → **truncate the session JSONL at `msg_id`'s record + respawn** (`--resume` in place for an in-line rewind; `--fork-session` first when `fork`, to preserve the original as a registered copy then truncate+resume the copy). **Guard:** refuse/snap-to-boundary if `msg_id` falls across a `/compact` boundary (chopping past it breaks `--resume` — [#step-7a] constraint).
  - **Anchor mapping:** `msg_id` is **claude's `user_message` uuid** (from the stream / JSONL), which both `rewind_files.user_message_id` and the JSONL truncation key on. The dev-card transcript must carry that uuid per turn (it already arrives on the stream events).
  - `protocol.ts` + `tugcode/src/types.ts` gain these types; pin against a fixture per [D06]. `CLAUDE_CODE_ENABLE_SDK_FILE_CHECKPOINTING=true` is already set in `spawnClaude` ([#step-7a]).
- Modified: register `/rewind` in the [#step-1c] registry; `RUN_SLASH_COMMAND` opens `RewindSheet`.
- Modified: `code-session-store/` reducer for the conversation-truncation state transition; surgery scoped to preserve mount identity per [L26].

**Tasks:**
- [ ] **tugcode bridge for `rewind_files`** — relay the control request both ways (`rewind_preview`/`session_rewind` inbound → `rewind_files` control request → relay `control_response` outbound). Add a probe-table entry driving the round-trip end-to-end **through tugcode** (the [#step-7a] direct fixture is the expected shape).
- [ ] **tugcode conversation rewind** — on `session_rewind` with `conversation`/`both`, truncate the session JSONL to the turn boundary + respawn `--resume` (or `--fork-session` then truncate+resume the fork). Include the compaction-boundary guard.
- [ ] `rewind-turn-source.ts` projection; rows carry the claude `user_message` uuid, preview, timestamp, and a diff-stat populated from `rewind_preview_result`.
- [ ] `RewindSheet`: turn picker → confirm step with the **three** conditional actions; keyboard nav + Enter; ESC dismisses; card-scoped overlay per [D15].
- [ ] Wire `conversation` / `code` / `both` restore through the new `session_rewind` inbound.
- [ ] Empty-state: 0- or 1-turn session does not surface `/rewind` in popup.
- [ ] **Transcript truncation preserves mount identity per [L26]**: on a conversation rewind, turns after `msg_id` unmount; turns before it keep keys (`turnKey`/`msg_id`, never phase-encoded), a single phase-branching row component (no type swap), and a `useCallback`-stable renderer ref — so scroll, selection, and in-flight subscriptions survive.
...
**Tests:**
- [ ] Pure-logic: turn-source projection (row count, ordering, diff-stat mapping from `rewind_preview_result`).
- [ ] Pure-logic: reducer truncation preserves pre-fork row keys verbatim.
- [ ] Real-claude probe: `rewind_files` round-trip **through tugcode** matches the [#step-7a] fixture (`canRewind` + diff stats on dry-run; file reverted on apply).
- [ ] Real-claude: conversation rewind through tugcode — build a multi-turn session, `session_rewind{scope:"conversation"}`, assert the resumed session recalls only the retained turns (mirrors the [#step-7a] chop verification).
- [ ] Real-app: keyboard select a turn → confirm → assert (a) conversation restore truncates the transcript, (b) code restore reverts a file claude edited.
...
- [ ] `just capture-capabilities` (drift clean; `rewind_files`-through-tugcode probe present)
...
**References:** [D23] local slash-command dispatch

> **`/resume` ≠ `/rewind` (clarified 2026-05-30).** These are genuinely different operations and share **no** component: `/resume` picks among **distinct prior sessions** (switch which conversation this card is bound to); `/rewind` ([#step-7]) picks among **turns within the current session** (move this conversation backward). `/resume` uses the existing **session chooser**; `/rewind` uses its own `RewindSheet` (turns). The earlier `SessionPickerSheet` framing that lumped them is retired — `/resume` is the only consumer of [D05]'s chooser lineage.
