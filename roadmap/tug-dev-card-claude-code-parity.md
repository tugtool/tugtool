...
**Tasks:**
- [x] Send the `initialize` control_request to claude at spawn (`sendInitializeHandshake` after `installEarlyExitWatcher`, via `sendControlRequest` with a stored `initializeRequestId`); does **not** block the first user turn (the readiness gate is independent). Reset on respawn alongside `sessionInitSeen`. **New mode only** (follow-up fix): on a `--resume` spawn whose conversation is gone, injecting `initialize` perturbs the failing spawn's output so tugcode's `resume_failed` classifier misses the signature → generic-crash retries → `crash_budget_exhausted` on a reopened dead-session card. Gating to `new` keeps resume spawns byte-identical to pre-change; resumed sessions get their metadata from the [#step-2a-1] ledger replay, not the handshake. Verified against claude 2.1.158 (resume-of-nonexistent fails identically with/without initialize; fresh `--session-id` answers initialize and survives a full turn).
