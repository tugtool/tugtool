...
- **Local commands are transport-independent (A).** Recognition + dispatch happen after the empty-input guard but **before** the `canSubmit` / disabled / blocked-submit gates — a local command opens its surface even while the session is settling / reconnecting / replaying, matching `Shift+Tab` (`CYCLE_PERMISSION_MODE`), which is never gated on send-readiness. Mid-turn is unaffected either way: `canInterrupt` is true, so the non-local path reaches `send()` and the local command is intercepted before it.
- **The card-content handler is compile-time-exhaustive over the registry (B).** Key-card dispatch marks `handled` by handler *presence* (the walk stops at the first node holding a `RUN_SLASH_COMMAND` handler, regardless of what it does), so once the dev card registers one, every name the matcher dispatches is consumed. A registry entry the handler does not map would be silently swallowed — neither opened nor sent. The handler is therefore a total map keyed by the registry's command names (`Record<LocalCommandName, …>` / `never` exhaustiveness check) so drift is a compile error, not a dead command. It reads current state fresh from the store at invocation, never a stale closure ([L07]).
...
- [ ] Intercept in `performSubmit` (pure `matchLocalSlashCommand` import) **after the empty-input guard but before the `canSubmit` / disabled / blocked-submit gates** (A — local commands are transport-independent): match + no atoms → key-card dispatch `RUN_SLASH_COMMAND` → if `handled`, clear + `onAfterSubmit` + return; else fall through to the normal gated `send()` path.
...
- [ ] Register the `RUN_SLASH_COMMAND` handler on the card-content responder (no `parentId` pin) as a **compile-time-exhaustive map over the registry's command names** (B — `Record<LocalCommandName, …>` / `never` check, so a registry entry without a surface is a type error, not a silent swallow); the handler reads state fresh from the store ([L07]) and opens the command's surface.
